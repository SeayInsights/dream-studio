...
# runtime/hooks/{pack}/ for the named handler. Falls back to the legacy
...
for pack in core quality career analyze domains meta; do
  candidate="${PLUGIN_ROOT}/runtime/hooks/${pack}/${HANDLER}.py"
  if [[ -f "${candidate}" ]]; then
...
export PYTHONPATH="${PLUGIN_ROOT}:${PLUGIN_ROOT}/hooks:${PYTHONPATH:-}"
