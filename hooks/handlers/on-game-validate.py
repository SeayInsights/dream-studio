...
def classify_path(rel_path: str, file_path: Optional[Path] = None) -> set[str]:
    ...
    # Also check the filename stem (e.g., "combat_system.gd" → "combat")
    if file_path:
    ...
    # Content-based fallback for .gd files with no path match
...
def validate_gdscript(file_path: Path, domains: set[str]) -> ValidationResult:
    ...
    if is_gameplay or is_ai:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if _is_suppressed(lines, i - 1):
                continue
            # Skip enum declarations, signal declarations, class_name
            if stripped.startswith(("enum ", "signal ", "class_name ")):
                continue
            # Skip lines with @export (intentionally exposed to inspector)
            if "@export" in line:
                continue
    ...
    if is_net:
        for i, line in enumerate(lines, 1):
            ...
            if stripped.startswith("#"):
                continue
            # Client setting authoritative state
...
def validate_json_data(file_path: Path, rel_path: str) -> ValidationResult:
    ...
    # Unreasonable numeric values in balance files
    if is_balance:
    ...
    # Schema consistency: arrays of objects should have consistent keys
...
def validate_asset_naming(file_path: Path, rel_path: str) -> ValidationResult:
    ...
    # Only check assets in expected directories
...
def validate_shader(file_path: Path) -> ValidationResult:
    ...
    # Check for hardcoded visual values (should be uniforms)
    for i, line in enumerate(lines, 1):
        stripped = line.strip()
        if stripped.startswith("//"):
            continue
...
def main() -> None:
    # Fast exit: read hook payload from stdin
    try:
        payload = json.loads(sys.stdin.read())
    except Exception:
        payload = {}

    # Primary path source: CLAUDE_FILE_PATH env var, fallback to tool_input
    ...
    # Fast exit: only process game-relevant file types
    ...
    # Detect project with structural validation
    ...
    # Version staleness check (once per project, not per file — but cheap enough)
