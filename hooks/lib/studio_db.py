        ...
        with _connect(db_path) as c:
            cur = c.execute(
                ...
                (session_id, project_id, topic, plan_path, pipeline_phase,
                 current_task_id, current_task_name, tasks_completed, tasks_total,
                 branch, last_commit,
                 json.dumps(working) if working else None,
                 json.dumps(broken) if broken else None,
                 json.dumps(pending_decisions) if pending_decisions else None,
                 json.dumps(active_files) if active_files else None,
                 next_action,
                 json.dumps(lessons_json) if lessons_json else None,
                 json.dumps(gotchas_hit) if gotchas_hit else None,
                 json.dumps(approaches_json) if approaches_json else None,
                 _NOW()),
            )
