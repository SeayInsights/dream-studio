...
_PACKAGE_SHIM = '''"""Installed projection of the repo's `{rel}` package — PARTIAL BY DESIGN.

Generated by the installer; do not edit. The installed tree carries only what the
dispatcher needs to bootstrap, while handlers import many more submodules that live
in the repo. Without the __path__ append below, Python binds this name to the partial
copy, fixes its __path__, and every other submodule raises ModuleNotFoundError
(WO becfca00).
"""
...
_sidecar = _Path(__file__).resolve(){parents} / ".ds-source-root"
if _sidecar.is_file():
    try:
        _repo_pkg = _Path(_sidecar.read_text(encoding="utf-8").strip()) / "{rel}"
        if _repo_pkg.is_dir():
            __path__.append(str(_repo_pkg))
    except Exception:
        pass
'''


def _package_shim(rel: str, depth: int) -> str:
    """Source for an installed package __init__ that DEFERS to the repo's copy.

    `depth` is how many parents separate this __init__ from the hooks dir, where the
    .ds-source-root sidecar lives. Read from the sidecar rather than an environment
    variable so it survives a bare `py hooks/dispatch/hooks.py` with no environment.
    """
    return _PACKAGE_SHIM.format(rel=rel, parents="".join([".parent"] * depth))
...
def _collect_hook_file_ops(
    ...
    # control/ __init__ files are GENERATED, not copied (WO becfca00).
    #
    # Copying the repo's __init__.py made the installed tree a complete-LOOKING `control`
    # package while it carries only control/execution/dispatch_tracking.py. The plugin root
    # is inserted at sys.path[0], so Python bound `control` to that partial copy and never
    # looked at the repo: 12 handlers importing control.skills.*, control.context,
    # control.review.engine, control.analysis, control.research.memory and
    # control.execution.models.* raised ModuleNotFoundError on every dispatch, and were then
    # dropped silently, so four of them logged nothing for the life of the install.
    #
    # The shim appends the repo's matching directory to __path__, so a partial projection
    # DEFERS to the full package instead of hiding it. Written at BOTH levels because
    # control.execution is itself partial. Verified with the partial copy FIRST on sys.path,
    # which is the order production actually uses.
    for _rel, _depth in (("control", 2), ("control/execution", 3)):
        _shim_target = hooks_dir.joinpath(*_rel.split("/")) / "__init__.py"
        _shim_src = _package_shim(_rel, _depth)
        ops.append(
            FileOp(
                target=_shim_target,
                op="create",
                backup_required=_shim_target.exists(),
                source_hash=_compute_hash(_shim_src),
                source_content=_shim_src,
                reason=f"Generate {_rel}/__init__.py so the partial install defers to the repo",
                safety_notes=(
                    "Generated, not copied — a copied __init__ makes the partial projection"
                    " shadow the repo's complete package."
                ),
                backup_path=backup_base if _shim_target.exists() else None,
            )
        )
