...
def score_repo(
    ...
    total_deduction = sum(DEDUCTIONS[cat] * count for cat, count in counts.items() if count > 0)
...
FIX_TEMPLATES: dict[str, str] = {
    ...
    "non_standard_port": (
        "In `{file}:{line}`, move to port 443 (HTTPS) — " "port blocked by {proxy_type} policy"
    ),
    "websocket_no_tls": (
...
def run_analysis(
    ...
    dlp_terms: list[str] = data_cfg.get("critical", []) + data_cfg.get("sensitive", [])
...
def run_sample_mode(client: str) -> None:
    ...
    dlp_terms = profile["data"]["critical"] + profile["data"]["sensitive"]
