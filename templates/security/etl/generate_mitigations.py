...
try:
    ...
    print(
        "[generate_mitigations] ERROR: PyYAML is required. Run: pip install pyyaml", file=sys.stderr
    )
    sys.exit(1)
...
def load_mitigation_templates() -> list[dict]:
    ...
    for fname in MITIGATION_FILES:
        ...
        if not fpath.exists():
            print(
                f"[generate_mitigations] WARNING: Template file not found: {fpath}", file=sys.stderr
            )
            continue
        try:
...
def _apply_generic(finding: dict) -> dict:
    ...
    generic = GENERIC_BY_OWASP.get(owasp) or GENERIC_BY_SEVERITY.get(
        severity, GENERIC_BY_SEVERITY["medium"]
    )
...
def main() -> None:
    ...
    if args.input:
        ...
        if not input_path.exists():
            print(
                f"[generate_mitigations] ERROR: input file not found: {input_path}", file=sys.stderr
            )
            sys.exit(1)
