...
```python
# Python (hooks): from control.execution.models.selector import get_model_for_skill
tier = get_model_for_skill("ds-core think")  # → "opus"

# CLI: py -m control.execution.models.selector --skill="ds-core think"
```
...
Agent({
  ...
  model: get_model_for_skill("ds-core think"),  // "opus"
  prompt: `...`
})
```
...
When maintained context compiler and prompt assembler modules are available, use them instead of manually assembling prompts:

```bash
# 1. Compile static context (run ONCE per session)
py -m control.context.compiler --skill=build --pack=core --repo-context=repo-context.json > compiled.md

# 2. Assemble per-task prompt (run per dispatch)
py -m core.utils.prompt_assembler --template=implementer --static-context=compiled.md --task-text="Task 3: Build login form"
```
