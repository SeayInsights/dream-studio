...
- **Domain tagging**: domain-specific lessons (***REMOVED***, Power BI client-specific, etc.) must be tagged with the target skill and never promoted to core skill gotchas
...
### Sources (in order)
1. **Micro-captures** — read `~/.dream-studio/meta/today.md` when present.
