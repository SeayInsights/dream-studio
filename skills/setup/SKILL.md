...
**Trigger:** `ds-setup wizard` | keywords: `setup:`, `wizard:`, `install tools:`, `full setup:`
...
**Trigger:** `ds-setup status` | keywords: `status:`, `setup status:`, `which tools:`
...
4. If any tools are missing, append: "Run `ds-setup wizard` to install missing tools."
...
**Trigger:** `ds-setup jit` | keywords: `jit:`, `just-in-time:`, `as-needed:`
...
- `jit: firecrawl` — called by `ds-core` web research when Firecrawl is missing
- `jit: playwright` — called by `ds-security` DAST when Playwright is missing
...
3. On `yes`: `ds-setup wizard` runs to completion, then the original skill resumes.
