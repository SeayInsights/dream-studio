...
```
Next steps:
- For failed installs: check that the install tool (choco / brew / pip) is available and re-run `ds-setup wizard`.
- For verify_failed: restart your terminal, then run `ds-setup status` to confirm detection.
```
...
- **Idempotent installs:** If a tool in `completed_tools` was recorded as `skipped` or `failed` and the user wants to retry it, they should run `ds-setup wizard` with `--reset` or use `ds-setup status` to manually re-trigger a single tool. The standard resume flow does not re-prompt completed tools regardless of their prior outcome.
