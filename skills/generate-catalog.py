...
# Add the current runtime utility path for time_utils when available
...
def generate_skills_by_pack(skills):
    ...
    for pack in sorted(packs.keys()):
        ...
        for skill in pack_skills:
            ...
            if times_used > 0:
                output.append(
                    f"  Success: {success_rate:.0f}% ({times_used} uses) | Tokens: {avg_tokens:,} avg\n"
                )
            if triggers:
...
def generate_quality_metrics(skills):
    ...
    by_success = sorted(
        used_skills, key=lambda s: s.get("quality_metrics", {}).get("success_rate", 0), reverse=True
    )[:10]
    ...
    for i, skill in enumerate(by_success, 1):
        ...
        output.append(
            f"{i}. **{name}** — {success:.0f}% ({uses} uses) {'✅' if success >= 90 else '⚠️'}\n"
        )
    output.append("\n")
    ...
    by_tokens = sorted(
        used_skills,
        key=lambda s: s.get("quality_metrics", {}).get("avg_token_usage", 0),
        reverse=True,
    )[:10]
    ...
    # Sort by times used
    by_usage = sorted(
        used_skills, key=lambda s: s.get("quality_metrics", {}).get("times_used", 0), reverse=True
    )[:10]
...
def generate_dependency_graph(skills):
    ...
    if module_usage:
        ...
        for module in sorted(module_usage.keys()):
            ...
            output.append(
                f"- **core/{module}.md** → {len(users)} skills: {', '.join(sorted(users[:5]))}"
            )
            if len(users) > 5:
