"""CLI for managing the advisory local Dream Studio research cache.

The file-backed cache is local-only by default. It stores research/source
artifacts as advisory evidence, not canonical truth, and it is not a redacted
export surface unless a caller explicitly classifies and redacts the payload.

Usage examples
--------------
  python interfaces/cli/research_cache.py save "security-owasp" --sources '[{"url":"https://owasp.org","tier":"1","date":"2026-05-01","key_findings":"Top 10 list"}]'
  python interfaces/cli/research_cache.py get "security-owasp"
  python interfaces/cli/research_cache.py stale
  python interfaces/cli/research_cache.py refresh "security-owasp"
  python interfaces/cli/research_cache.py prune --days 180
  python interfaces/cli/research_cache.py list
  python interfaces/cli/research_cache.py stats
  python interfaces/cli/research_cache.py list --json
"""
...
def _calc_confidence(sources: list[dict]) -> str:
    ...
    if count == 0:
        return "low"
    tier1_count = sum(1 for s in sources if str(s.get("tier", "")).strip() == "1")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_stale.add_argument("--json", action="store_true", help="Output as JSON array.")
    ...
    # prune
    p_prune = sub.add_parser("prune", help="Remove entries older than N days (by saved_date).")
    ...
    p_stats.add_argument("--json", action="store_true", help="Output as JSON object.")
