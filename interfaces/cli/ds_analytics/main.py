...
def main() -> None:
    ...
    if hook_timing["handlers"]:
        print(
            f"  hook timing:     {len(hook_timing['handlers'])} handlers, {hook_timing['total_overhead_ms']:.1f}ms total avg"
        )
    else:
    ...
    print(
        f"  pulse trend:     {pulse_trend['trend_direction']} (slope={pulse_trend['trend_slope']:.2f})"
    )
    ...
    print(
        f"  conversion rate: {conversion_rate['rate']:.0%} ({conversion_rate['total'] - conversion_rate['orphaned']}/{conversion_rate['total']})"
    )

    # Efficiency analytics
    from efficiency_analytics import (
        parse_token_log,
        build_sessions,
    ...
    if eff_records:
        ...
        print(
            f"  efficiency:      {len(eff_sessions)} sessions, ${efficiency_data.get('cost_analysis', {}).get('total_cost_usd', 0):.2f} total cost"
        )
    else:
