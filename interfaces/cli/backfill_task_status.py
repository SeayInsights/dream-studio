...
def backfill_task_status(
    ...
    *,
    ...
    handoffs = conn.execute("""SELECT h.project_id, h.topic, h.tasks_completed, h.tasks_total,
    ...
    for h in handoffs:
        ...
        if to_mark == 0:
            if verbose:
                ...
                print(
                    f"  SKIP spec '{matching_spec}' — {already} already completed, {len(planned)} planned"
                )
            continue

        if verbose:
            print(
                f"  MARK {to_mark}/{len(tasks)} tasks as completed for spec '{matching_spec}' (handoff: {topic})"
            )
