...
# Shared utilities (also importable by command modules)
from interfaces.cli.cli_utils import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    # -----------------------------------------------------------------------
    # Delegated command groups (registered via their modules)
    # -----------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Implementation helpers for flat / system-level commands
# (dashboard, doctor, update, migrate, validate, etc.)
# These are retained here because they are tightly coupled to top-level imports
# and don't belong to any named command group.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Backward-compatible re-exports for tests that import private functions
# directly from interfaces.cli.ds.
#
# These names moved to their respective command modules but remain importable
# from this hub so existing test files do not break.  Do not remove these
# aliases without first updating every test that imports them.
# ---------------------------------------------------------------------------

# From commands/project
_project_register = project._project_register
_project_list = project._project_list
_project_status = project._project_status
_project_next = project._project_next
_project_set_active = project._project_set_active
_project_deactivate = project._project_deactivate
_project_delete = project._project_delete
_project_start = project._project_start
_project_state = project._project_state

# From commands/work_order
_work_order_close = work_order._work_order_close
_work_order_start = work_order._work_order_start
_work_order_list = work_order._work_order_list
_work_order_block = work_order._work_order_block
_work_order_unblock = work_order._work_order_unblock
_work_order_task_done = work_order._work_order_task_done
_work_order_tasks = work_order._work_order_tasks
_work_order_verify = work_order._work_order_verify
_work_order_set_order = work_order._work_order_set_order
_work_order_add_dep = work_order._work_order_add_dep
_work_order_remove_dep = work_order._work_order_remove_dep
_work_order_next = work_order._work_order_next
_work_order_executor = work_order._work_order_executor

# From commands/skill
_skill_invoke = skill._skill_invoke
_skill_list = skill._skill_list

# From commands/design_brief
_design_brief_show = design_brief._design_brief_show
_design_brief_create = design_brief._design_brief_create
_design_brief_lock = design_brief._design_brief_lock
_design_brief_update = design_brief._design_brief_update
_design_brief_set_system = design_brief._design_brief_set_system

# From commands/milestone
_milestone_close = milestone._milestone_close
_milestone_list = milestone._milestone_list
_milestone_status = milestone._milestone_status

# From commands/task
_task_set_active = task._task_set_active
_task_get_active = task._task_get_active
_task_clear_active = task._task_clear_active

# From commands/eval
_eval_dispatch = eval_cmd.dispatch
_eval_registry_dispatch = eval_cmd._eval_registry_dispatch
_eval_queue_dispatch = eval_cmd._eval_queue_dispatch

# From commands/analyze
_analyze_dispatch = analyze.dispatch
_analyze_intake = analyze._analyze_intake
_analyze_aggregate = analyze._analyze_aggregate

# From commands/config
_config_dispatch = config.dispatch

# From commands/diagnostics
_diagnostics_dispatch = diagnostics.dispatch

# From commands/integrate
_integrate_dispatch = integrate.dispatch

# Dispatch function aliases (for code that calls the old _*_dispatch names)
_project_dispatch = project.dispatch
_work_order_dispatch = work_order.dispatch
_skill_dispatch = skill.dispatch
_design_brief_dispatch = design_brief.dispatch
_milestone_dispatch = milestone.dispatch
_task_dispatch = task.dispatch
