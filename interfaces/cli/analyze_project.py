...
Examples:
  ...
  Verbose output:
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Skip research phase for faster analysis"
    )
    ...
    # Check for control.analysis.engine
    try:
        ...
        print(
            f"The intelligence mode requires Waves 0-4 of project-intelligence to be complete.",
            file=sys.stderr,
        )
        print(f"Missing module: control.analysis.engine", file=sys.stderr)
    ...
    try:
        result = analyze_project(path=project_path, run_type=run_type)
    except Exception as e:
        print(f"[ERROR] Analysis failed: {e}", file=sys.stderr)
        print(f"\nPartial results may be available in the database.", file=sys.stderr)
        print(
            f"Check logs for details: cat ~/.dream-studio/logs/analysis-errors.log", file=sys.stderr
        )
        sys.exit(1)
