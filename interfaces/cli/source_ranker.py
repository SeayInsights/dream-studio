...
Usage:
    python interfaces/cli/source_ranker.py --sources sources.json
    cat sources.json | python interfaces/cli/source_ranker.py --stdin
    python interfaces/cli/source_ranker.py --sources sources.json --json
"""
...
def rank_sources(sources: list[dict]) -> dict:
    ...
    has_dissent = any(DISSENT_PATTERNS.search(s.get("findings", "")) for s in sources)
    ...
    counter_present = any(COUNTER_PATTERNS.search(s.get("findings", "")) for s in sources)
...
def format_text(sources: list[dict], result: dict) -> str:
    ...
    lines.append(f"Triangulation: {tri:.1f} ({n} of 3 needed)")
    lines.append(f"Tier 1 sources: {tier1_count} of {n} ({tier1_pct:.0f}%)")
