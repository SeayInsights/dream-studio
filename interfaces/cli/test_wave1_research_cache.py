...
from control.research.engine import research_with_cache
...
def _require_opt_in():
    if os.environ.get(OPT_IN_ENV) == "1":
        return
    print(
        f"Refusing to run mutating legacy research diagnostic without {OPT_IN_ENV}=1. "
...
def test_cache_hit():
    ...
    conn = _connect()
    cursor = conn.execute("SELECT times_referenced FROM raw_research WHERE query LIKE '%Next.js%'")
    row = cursor.fetchone()
    conn.close()
...
def test_metrics():
    ...
    cursor = conn.execute("SELECT COUNT(*) FROM raw_metrics WHERE metric_name LIKE '%research%'")
