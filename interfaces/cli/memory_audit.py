...
def check_missing_coverage(memory_files: list[Path], session_files: list[Path]) -> list[dict]:
            ...
            gaps.append(
                {
                    ...
                    "suggested_filename": "feedback_"
                    + re.sub(r"[^a-z0-9]+", "_", correction[:40].lower()).strip("_")
                    + ".md",
                }
            )
...
def format_report(
    *,
    ...
    # Stale
    lines.append(
        "Stale entries (>60 days):" if not verbose else f"Stale entries ({len(stale)} found):"
    )
    if stale:
        for item in stale:
            lines.append(
    ...
    if not memory_dirs:
        if args.as_json:
            print(
                json.dumps(
                    {
                        "error": "No memory directories found.",
                        "dirs_checked": str(explicit_path or Path.home() / ".claude" / "projects"),
                    }
                )
            )
        else:
