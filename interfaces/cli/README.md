...
| `spec_risk_check.py` | Pre-populate spec edge cases from prior experience | `py interfaces/cli/spec_risk_check.py <topic>` |
...
Research cache entries are advisory local evidence. They are not canonical truth
or redacted exports by default.

**Examples:**
```
py interfaces/cli/research_cache.py save "security-owasp" --sources '[{"url":"https://owasp.org","tier":"1","date":"2026-05-01","key_findings":"Top 10 list"}]'
py interfaces/cli/research_cache.py get "security-owasp"
py interfaces/cli/research_cache.py stale
py interfaces/cli/research_cache.py list
py interfaces/cli/research_cache.py prune --days 180
py interfaces/cli/research_cache.py stats
py interfaces/cli/research_cache.py list --json
```
...
**Examples:**
```
py interfaces/cli/source_ranker.py --sources sources.json
cat sources.json | py interfaces/cli/source_ranker.py --stdin
py interfaces/cli/source_ranker.py --sources sources.json --json
```
...
**Examples:**
```
py interfaces/cli/spec_risk_check.py "auth token refresh"
py interfaces/cli/spec_risk_check.py "security scan integration" --json
```

**Used by:** `ds-core think` — mandated in the gotcha `run spec_risk_check.py before writing Edge Cases`.
...
### sync-plugin-cache.ps1

PowerShell script. Refreshes the local installed Dream Studio Claude plugin cache
from this checkout. It stages a versioned cache directory, copies plugin metadata,
hooks, runtime support files, and skills, excludes generated artifacts such as
`__pycache__`, and updates `~/.claude/plugins/installed_plugins.json` when the
existing schema is recognized.

Always preview first:
```powershell
powershell.exe -ExecutionPolicy Bypass -File interfaces/cli/sync-plugin-cache.ps1 -DryRun
```
...
**CLI:**
```
py -m control.execution.models.selector --skill=<name> [--default=sonnet]
```
...
**Module API:**
```python
from control.execution.models.selector import recommend_model
tier = recommend_model("build", default="sonnet")  # → "sonnet"
```

**Examples:**
```
py -m control.execution.models.selector --skill=build
py -m control.execution.models.selector --skill=think --default=opus
```
...
**CLI:**
```
py -m control.context.compiler --skill=<mode> --pack=<pack> \
...
**Module API:**
```python
from control.context.compiler import compile_context
text = compile_context("build", "core", repo_context_path=None, project_root=".")
```

**Examples:**
```
py -m control.context.compiler --skill=build --pack=core
py -m control.context.compiler --skill=debug --pack=quality --project-root=.
py -m control.context.compiler --skill=think --pack=core --repo-context=.planning/repo_context.json
```
...
Template files live in `core/utils/prompt_templates/<name>.md`.

**CLI:**
```
py -m core.utils.prompt_assembler --template=<name> --static-context=<path> \
...
**Module API:**
```python
from core.utils.prompt_assembler import assemble_prompt
text = assemble_prompt("implementer", "compiled.md", task_text="Build the API endpoint")
```

**Examples:**
```
py -m core.utils.prompt_assembler --template=implementer --static-context=compiled.md --task-text="Build the API endpoint"
py -m core.utils.prompt_assembler --template=reviewer --static-context=compiled.md --task-file=review-scope.md
py -m core.utils.prompt_assembler --template=auditor --static-context=compiled.md --task-text="Audit security" --decisions="Use OWASP top 10"
```
...
|---|---|
| `ds-core think` | `spec_risk_check.py`, `source_ranker.py`, `research_cache.py` |
| `ds-core handoff` | `resume_from_handoff.py` (output consumed in standup) |
| `ds-quality learn` | `lesson_queue.py` (promote/reject cycle) |
...
| Skill-only plugin sync | `sync-cache.ps1` (after skill-only changes) |
| Full local plugin refresh | `sync-plugin-cache.ps1 -DryRun`, then `sync-plugin-cache.ps1` |
