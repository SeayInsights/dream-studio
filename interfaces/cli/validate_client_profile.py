...
Classification: local-only optional security/client tooling. This validator
does not create org aggregation, cloud authority, or core runtime authority.
"""
...
def validate(data: dict) -> list[str]:
    ...
    if not isinstance(isolation, dict):
        ...
        if not model:
            ...
            errors.append(
                f"INVALID isolation.model '{model}' " f"(valid: {sorted(VALID_ISOLATION_MODELS)})"
            )
    ...
    if not isinstance(network, dict):
        ...
        if not isinstance(proxy, dict):
            ...
            if not proxy_type:
                ...
                errors.append(
                    f"INVALID network.proxy.type: expected string, got {type(proxy_type).__name__}"
                )
                proxy_type = None
    ...
    if not isinstance(compliance, dict):
        ...
        if not frameworks or not isinstance(frameworks, list):
            errors.append(
                "MISSING required field: compliance.frameworks (must be a non-empty list)"
            )
    ...
    if not isinstance(scan, dict):
        ...
        if not schedule:
            ...
            errors.append(
                f"INVALID scan.schedule '{schedule}' " f"(valid: {sorted(VALID_SCAN_SCHEDULES)})"
            )
    ...
    if not isinstance(dashboard, dict):
        ...
        if not tool:
            ...
            errors.append(
                f"INVALID dashboard.tool '{tool}' " f"(valid: {sorted(VALID_DASHBOARD_TOOLS)})"
            )
