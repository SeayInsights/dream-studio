...
def migrate_gotchas(dry_run=False) -> tuple[int, list[str]]:
    ...
    for file_path in files:
        try:
            ...
            skill_id = (
                extract_skill_id_from_path(file_path) if "skills" in file_path.parts else None
            )
...
def migrate_planning_specs(dry_run=False) -> tuple[int, list[str]]:
    ...
    safe_print(
        f"[.planning/] Found {len(all_files)} files ({len(md_files)} .md, {len(yaml_files + yml_files)} .yaml/.yml)"
    )
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Show what would be migrated without writing to DB"
    )
