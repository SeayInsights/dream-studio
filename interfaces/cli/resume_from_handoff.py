"""
resume_from_handoff.py — reads a handoff from database or JSON and produces a structured session briefing.

Usage:
    py scripts/resume_from_handoff.py [handoff_id | handoff.json] [--checkout] [--brief]

    handoff_id  Numeric ID to query from prd_handoffs/raw_handoffs (e.g., 199)
    handoff.json File path to legacy JSON handoff
...
If no argument is given, scans database for most recent unconsumed handoff.
"""
...
def load_handoff_from_db(handoff_id: int) -> dict | None:
    ...
    try:
        ...
        row = conn.execute(
            """
            SELECT
                id, topic, plan_path, working, broken,
                pending_decisions, next_action, lessons_json,
                branch, last_commit, tasks_completed, tasks_total,
                created_at
            FROM raw_handoffs
            WHERE id = ?
        """,
        ...
        # Parse JSON fields from raw_handoffs
        data = {
            "handoff_id": row[0],
            "topic": row[1],
            "plan_path": row[2],
            "working": json.loads(row[3]) if row[3] else [],
            "broken": json.loads(row[4]) if row[4] else [],
            "pending_decisions": json.loads(row[5]) if row[5] else [],
            "next_action": row[6] or "Continue work",
            "lessons_this_session": json.loads(row[7]) if row[7] else [],
            "branch": row[8],
            "last_commit": row[9],
            "progress": f"{row[10]}/{row[11]} tasks" if row[11] else "unknown",
            "created_at": row[12],
            "source": "raw_handoffs",
        }
        return data
        ...
        print(f"Error loading handoff from database: {e}", file=sys.stderr)
        return None


def find_latest_handoff_db() -> dict | None:
    """Find most recent unconsumed handoff from database."""
    if not _connect:
        return None
    ...
    try:
        ...
        print(f"Error finding latest handoff: {e}", file=sys.stderr)
        return None
...
def print_full_briefing(data: dict) -> None:
    ...
    # Show PRD info if available
    if prd_title:
        lines.append(f" PRD:       {prd_title}")
        lines.append(f" Status:    {prd_status or 'unknown'}")
    ...
    if lessons:
        ...
        for item in lessons:
            if isinstance(item, dict):
                lesson = item.get("lesson", str(item))
                context = item.get("context", "")
                lines.append(f" • {lesson}" + (f" ({context})" if context else ""))
            else:
                lines.append(f" • {item}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Resume a dream-studio session from a handoff.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        "handoff",
        nargs="?",
        help="Handoff ID (e.g., 199) or JSON file path. Auto-discovers latest if omitted.",
    )
    parser.add_argument(
        "--checkout",
        action="store_true",
    ...
    # Resolve handoff source
    if not args.handoff:
        # Auto-discover from database
        data = find_latest_handoff_db()
        if data:
            print(
                f"Auto-discovered: Handoff #{data['handoff_id']} from {data.get('source', 'database')}",
                file=sys.stderr,
            )
        else:
            ...
            if handoff_path:
                ...
                print("Error: no handoff found in database or .sessions/", file=sys.stderr)
                sys.exit(1)
    elif args.handoff.isdigit():
        # Handoff ID - query database
        handoff_id = int(args.handoff)
        data = load_handoff_from_db(handoff_id)
        if not data:
            print(f"Error: Handoff #{handoff_id} not found in database", file=sys.stderr)
            sys.exit(1)
        print(
            f"Loaded: Handoff #{handoff_id} from {data.get('source', 'database')}", file=sys.stderr
        )
    else:
        # File path
        ...
        if branch:
            ...
            if active != branch:
                ...
                if not git_checkout(branch):
                    print(
                        f"Warning: git checkout {branch!r} failed — continuing anyway.",
                        file=sys.stderr,
                    )
            else:
