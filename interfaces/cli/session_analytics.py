...
try:
    from control.session.parser import scan_sessions, extract_blockers
...
def compute_analytics(sessions: list[dict], window_days: int) -> dict:
    ...
    for s in sessions:
        ...
        skills_used: list[str] = s.get("skills_used", []) or ["ds-core"]
        ...
        for sk in skills_used:
            ...
            is_failed = False
            if s.get("type") == "handoff" and s.get("broken_items") or s.get("type") == "recap" and s.get("risk_flags"):
                is_failed = True
            if is_failed:
                skill_failed[sk] += 1
    ...
    common_blockers = [
        {"item": b["item"], "frequency": b["frequency"]} for b in blockers[:20]  # top 20
    ]
