...
| decision lineage | `core.decisions.emitter.emit_decision` | `business_canonical_events` (`decision.recorded` events) | `proj_decision_patterns`, decision query APIs, coverage reports | decision integrity reports and audit summaries | Decision reasoning/outcome/causal linkage (`triggered_event_id`) all live in the `decision.recorded` event payload; `decision_log`/`decision_event_link` were dropped migration 136 (WO-DBA-EVAL-DECISION T4). |
...
Projection code must not write canonical operational tables such as `raw_sessions`, `raw_workflow_runs`, `execution_nodes`, `business_canonical_events`, `memory_entries`, `activity_log`, `hook_executions`, or `canonical_events`.
