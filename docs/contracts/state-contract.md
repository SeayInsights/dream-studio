...
| execution state | `core.event_store.studio_db` write helpers and explicit execution managers | `activity_log`, `hook_executions`, `hook_findings`, `adapter_executions`, `audit_runs` | hook performance views, activity timeline views, dashboard/API summaries | validation failures, hook stdout/stderr, adapter metadata | `activity_log` is a legacy operational hub and compatibility surface; canonical events remain governed by the event contract. |
...
| governance state | governance and security write helpers plus explicit audit/risk endpoints | `risk_register`, `risk_mitigations`, `audit_runs`, `guardrail_decisions`, `guardrail_rules_audit`, `sec_sarif_findings`, `sec_manual_reviews`, `sec_cve_matches`, `sec_hook_checks` | `vw_security_summary`, `vw_risk_hotspots`, `vw_guardrail_decisions`, security API summaries | compliance scores, trend views, rule-load audit rows | Governance records may be written by explicit local governance APIs. Dashboards do not become governance authority by displaying them. |
...
- `raw_skill_telemetry`, `activity_log`, and `adapter_executions`: telemetry records usage observations; `activity_log` records normalized operational activity; `adapter_executions` records adapter normalization metadata.
...
Projection code must not write canonical operational tables such as `raw_sessions`, `raw_workflow_runs`, `execution_nodes`, `decision_log`, `memory_entries`, `activity_log`, `hook_executions`, `adapter_executions`, `risk_register`, or `canonical_events`.
