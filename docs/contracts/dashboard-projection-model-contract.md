...
<!-- Reviewed 2026-07-06 — WO-SPLIT-DASHBOARD (fe3007fc): the monolithic projections/frontend/dashboard.html inline <style> (~596 lines) and <script> (~8160 lines) were extracted byte-faithfully to frontend/static/dashboard.css and frontend/static/dashboard.js, mounted at /static (FastAPI StaticFiles). dashboard.js loads as a CLASSIC script (not type=module) so the 71 inline event handlers keep resolving against window globals. No projection contract, API response shape, or dashboard data model change — this is a static-asset extraction only. -->
