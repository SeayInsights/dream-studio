# Dashboard Projection Model Contract

## Purpose

This contract defines documentation/data shapes for dashboard projections over Work Orders, evals, reports, handoffs, approvals, operator decisions, and Security Review artifacts.

Dashboard projections are read-only views over file-backed artifacts. They are not authority. Work Orders, reports, approvals, operator decisions, eval artifacts, Handoff Packets, and Security Review artifacts remain the source of truth.

This contract does not implement a dashboard UI, dashboard API, runtime projection builder, database table, event ledger, schema migration, scan runner, profile registry, target repo access, Docker expansion, or TORII/cloud/org/global/enterprise integration.

## Projection Artifact Roles

| Artifact | Role |
| --- | --- |
| `DashboardProjectionSnapshot` | Top-level read model snapshot for a bounded set of file-backed artifacts. |
| `WorkOrderOverviewProjection` | Work Order status, readiness, verdict, decision, next action, and risk summary. |
| `EvalProjection` | Deterministic eval result summary and evidence refs. |
| `ApprovalOperatorDecisionProjection` | Approval and operator decision state used for gated execution visibility. |
| `SecurityReviewProjection` | Security Review report, release-gate, findings, evidence, accepted-risk, and next Work Order summary. |

## Authority Model

- Projections are read-only views over artifacts.
- Projections must preserve artifact refs so the source evidence can be opened directly.
- Projections must show stale, missing, incomplete, or conflicting evidence.
- Projections must not claim readiness when source artifacts are missing or stale.
- Projections must not run scans.
- Projections must not approve risk.
- Projections must not mutate repos.
- Projections must not stage, commit, push, write target artifacts, install dependencies, update lockfiles, open native runtime databases, write event ledgers, or bypass Work Order approval.
- Projections must not replace Work Order reports, operator decisions, eval artifacts, Security Review reports, or Handoff Packets.
- Projections must not replace operator decisions.
- Projections must not replace Security Review reports.
- Projections must not replace Handoff Packets.

## DashboardProjectionSnapshot

Required fields:

| Field | Required | Purpose |
| --- | --- | --- |
| `projection_id` | yes | Stable projection snapshot identifier. |
| `generated_at` | yes | Timestamp or `unknown` when generated manually. |
| `source_artifact_refs` | yes | File-backed source artifacts used for the projection. |
| `work_orders` | yes | List of `WorkOrderOverviewProjection` items. |
| `evals` | yes | List of `EvalProjection` items. |
| `approvals_and_operator_decisions` | yes | List of `ApprovalOperatorDecisionProjection` items. |
| `security_reviews` | yes | List of `SecurityReviewProjection` items. |
| `stale_or_missing_evidence` | yes | Evidence gaps, stale refs, missing artifacts, and conflict notes. |
| `non_authority_notice` | yes | Statement that the dashboard projection is not authority. |

## WorkOrderOverviewProjection

Required fields:

| Field | Required | Purpose |
| --- | --- | --- |
| `work_order_id` | yes | Work Order identifier. |
| `phase_name` | yes | Phase name from report or handoff evidence. |
| `approval_mode` | yes | Work Order approval mode. |
| `risk_level` | yes | Work Order risk level. |
| `readiness` | yes | Sequential readiness, if known. |
| `verdict` | yes | Latest report verdict, if known. |
| `final_decision` | yes | Latest final decision, if known. |
| `next_action` | yes | Bounded next action or `unknown`. |
| `report_ref` | yes | File-backed report path or `missing`. |
| `handoff_ref` | yes | Handoff Packet path or `missing`. |
| `blocking_risks` | yes | List of blocking risks, missing evidence, or authority conflicts. |

## EvalProjection

Required fields:

| Field | Required | Purpose |
| --- | --- | --- |
| `eval_artifact_ref` | yes | File-backed eval artifact path. |
| `eval_type` | yes | Eval type identifier. |
| `pass_fail` | yes | `pass`, `fail`, `incomplete`, or `unknown`. |
| `score` | yes | Numeric score, label, or `not_applicable`. |
| `evidence_refs` | yes | File-backed evidence refs used by the eval. |
| `blocking` | yes | Boolean indicating whether the eval blocks continuation. |
| `limitations` | yes | Staleness, missing evidence, or scope caveats. |

## ApprovalOperatorDecisionProjection

Required fields:

| Field | Required | Purpose |
| --- | --- | --- |
| `approval_artifact_ref` | yes | Approval artifact path or `not_applicable`. |
| `approval_status` | yes | `present`, `missing`, `not_required`, `stale`, or `invalid`. |
| `operator_decision_ref` | yes | Operator decision artifact path or `not_applicable`. |
| `selected_decision` | yes | Selected operator decision or `none`. |
| `reason_required` | yes | Boolean. |
| `reason_present` | yes | Boolean. |
| `execution_allowed` | yes | Boolean derived from source artifacts; this is display-only and not execution authority. |

## SecurityReviewProjection

Required fields:

| Field | Required | Purpose |
| --- | --- | --- |
| `security_review_report_ref` | yes | File-backed Security Review report path. |
| `target_id` | yes | Target identifier or `not_applicable`. |
| `security_pack_id` | yes | Security Review Profile Pack identifier. |
| `verdict` | yes | Security Review report verdict. |
| `release_gate_decision` | yes | Release-gate decision from the report artifact. |
| `taxonomy_coverage` | yes | Category coverage and gaps. |
| `scan_status_counts` | yes | Counts by scan status. |
| `findings_by_severity` | yes | Counts by severity. |
| `findings_by_release_impact` | yes | Counts by release impact. |
| `blocking_findings` | yes | Blocking finding IDs. |
| `accepted_risks` | yes | Accepted risk IDs and decision refs. |
| `deferred_scans` | yes | Deferred scan IDs and reasons. |
| `evidence_inventory_refs` | yes | Evidence record refs. |
| `next_work_order_recommendation` | yes | Bounded next Work Order recommendation from the security report. |

## Stale And Missing Evidence Behavior

- Missing report refs must be displayed as missing, not inferred.
- Missing approval artifacts must display execution as not allowed.
- Missing operator decisions must display risk acceptance as incomplete.
- Missing eval artifacts must display eval status as `unknown` or `incomplete`.
- Missing Security Review reports must display release-gate state as `HOLD` or `unknown`, not clear.
- Stale source artifacts must display staleness and last-known values separately.
- Conflicting source artifacts must display conflict notes and must not be resolved silently.
- Projection snapshots must preserve source artifact refs for manual review.

## Dashboard Boundary

Dashboard projections may show:

- Work Order posture, readiness, verdict, final decision, and next action.
- Eval result summaries, evidence refs, blocking state, and limitations.
- Approval and operator-decision state.
- Security Review posture, taxonomy coverage, scan summary, findings, evidence inventory, release-gate decisions, accepted risks, deferred scans, and next Work Orders.

Dashboard projections must not:

- run scans;
- approve risk;
- mutate repos;
- stage files;
- commit files;
- push;
- write target artifacts;
- install dependencies;
- update lockfiles;
- open native runtime databases;
- write event ledgers;
- create schema migrations;
- replace Work Order reports;
- replace operator decisions;
- replace Security Review reports;
- replace Handoff Packets.

## Security Projection Source Rules

- Security Review projections must read from `SecurityReviewReport` artifacts, not directly from target repositories.
- Security release-gate decisions must come from `ReleaseGateSummary` or the Security Review report.
- Accepted risks must reference `AcceptedRiskRecord` and file-backed operator decision artifacts.
- Finding counts must come from `SecurityFindingRecord` data or report summaries.
- Evidence inventory refs must point to `SecurityEvidenceRecord` data or external report refs.
- Dashboard projection data must not become remediation authority.

## Validation Expectations

Static checks for this contract should verify:

- required projection shapes and fields are documented;
- dashboard non-authority rules are explicit;
- stale and missing evidence behavior is explicit;
- security projection fields are complete;
- sample projection data remains non-executing and not target-specific;
- no dashboard UI, API, runtime builder, DB/event/schema, target repo, scan execution, Docker, TORII/cloud/org/global/enterprise, dependency, or lockfile authority is introduced.

<!-- reviewed: 2026-06-06, WO-B broken surfaces. projections/api/routes/security.py: wired parse_sarif_file() into POST /security/sarif/import (uncommented import + call, removed stub). The endpoint was already listed in ALLOWED_DASHBOARD_WRITES and validated by test_dashboard_write_like_calls_stay_on_named_api_exceptions. No projection contract shape change; no new dashboard authority introduced. -->

<!-- reviewed: 2026-06-12, WO-C full-CI fixes. core/telemetry/read_models.py: qualified fcs.project_id in _security_remediation_intelligence LEFT JOIN WHERE clause — bug fix for "ambiguous column name" SQLite error; no shape change. core/telemetry/execution_spine.py: record_security_finding now emits a status_changed event when a non-open status is supplied; added "resolved" and "closed" to resolve_security_finding valid_resolutions. Both are correctness fixes with no new projection authority, no new tables, and no contract shape changes. -->

<!-- reviewed: 2026-06-06, WO-D dead discovery route removal. discovery_external.py and discovery_research.py deleted (no live callers). Dead endpoints removed from discovery_internal.py (now only /graph/{project_id}). Dead endpoints removed from prd.py (handoffs, progress, ready-waves). No projection contract shape change; no new dashboard authority introduced. Removal of dead routes reduces the projection surface. -->

<!-- reviewed: 2026-06-06, WO-A telemetry write-path honesty. dashboard_freshness.py: removed raw_token_usage from table_counts (retire path b — zero-token writes stopped at source). legacy_backfill.py: removed raw_token_usage → token_usage_records backfill mapping (source table no longer written). token_usage_records section of dashboard_freshness.py (lines 206-225) already reads from token_usage_records for the legacy_token_metrics section — no change required there. Dashboard projection contract shape unchanged; no new tables, no removed authority paths. -->

<!-- Last reviewed 2026-06-07 — WO-F prd_* cluster drop (migration 103): migration 103 drops the entire prd_* cluster (prd_documents, prd_plans, prd_tasks, prd_sessions, prd_handoffs, session_tasks, prd_version_records, prd_amendment_records, prd_route_reconciliation_records) and the views vw_prd_progress and vw_task_details. AD-10 decision: business_projects IS what PRD was. All prd_* callers removed from studio_db.py, prd_authority.py (deleted), routes/prd.py (deleted), analytics_ingestion.py, module_contracts.py, dashboard_freshness.py, contract_atlas.py, contract_registry.py, project_intelligence.py, shared_intelligence.py, resume_from_handoff.py, and migrate_prd_schema.py (deleted). -->

<!-- Last reviewed 2026-06-07 — WO-N behavioral eval harness (18.8.3): Eval Health dashboard panel added (id="evals" tab-content). Projections API route at /api/v1/evals/health reads ds_eval_baselines and ds_eval_runs. Dashboard surfaces: total evals, pass rate, baselines table, recent runs table. No change to existing dashboard projection shape; new panel reads from new tables only. -->

<!-- Last reviewed 2026-06-07 — WO-J cache_read_tokens (migration 105): intelligence.py token panel updated to query SUM(cache_read_tokens) from token_usage_sql() subquery and surface prompt-caching wins as a 'whats_working' item. token_attribution.py canonical_token_metrics() now returns actual cache_read_total as cache_hits (was hardcoded 0). Dashboard projection contract shape unchanged: no new routes, no removed fields; cache_hits field was already in the API model (metrics.py:46). -->

<!-- Last reviewed 2026-06-07 — WO-R AI spine consolidation: DASHBOARD_MODULES in execution_spine.py updated — agent_analytics, skill_analytics, workflow_analytics, hook_analytics now declare owns_tables=["execution_events"] and source_tables without per-type invocation tables. COMPONENT_TABLES in read_models.py repointed from per-type tables to execution_events filtered by component_column IS NOT NULL. tool_analytics module declaration source_tables updated to ["execution_events"]. process_run_timeline() invocations section now queries execution_events per component instead of per-type tables. workflow_execution_graph() queries execution_events instead of workflow_invocations. Dashboard projection contract shape preserved: same route endpoints, same field shapes, same derived_view semantics. Source authority changes from 5 per-type tables to execution_events (unified). -->

<!-- Last reviewed 2026-06-08 — WO-Y findings event-spine: FindingsProjection registered in core/projections/runner.py via runner.register_spine(FindingsProjection()). Folds security_events spine into findings_current_status. DASHBOARD_MODULES security_analytics entry in execution_spine.py updated: owns_tables and source_tables now reference security_events and findings_current_status instead of the retired findings/sec_sarif_findings tables. read_models.py security rollup (_security_rollup, _security_remediation_intelligence, _security_status_counts, _security_attribution) repointed to findings_current_status LEFT JOIN security_events. Dashboard projection contract shape preserved: same route endpoints (/api/v1/security/*), same field shapes, same derived_view semantics. -->

<!-- Last reviewed 2026-06-09 — WO-TS3 DuckDB-first read path: project_intelligence.py _project_row_for_authority() now tries DuckDB analytics store first via core/analytics/duckdb_read.get_project_row_duckdb(), falling back to SQLite authority. discovery_internal.py verify_project_exists() similarly tries DuckDB first via project_exists_duckdb(), falling back to SQLite. Dashboard projection contract shape is unchanged: same /api/v1/projects/* and /api/discovery/* route endpoints, same field shapes, same derived_view semantics. The read source layer changed (DuckDB-first, SQLite fallback) but the projection model contract is unaffected. DuckDB is NEVER-AUTHORITY — SQLite business_projects remains the canonical source. -->

<!-- Last reviewed 2026-06-09 — WO-TS4 correction: reverting DuckDB-first read paths in project_intelligence.py and discovery_internal.py — both now read from SQLite business_projects directly. Dashboard projection contract shape is unchanged: same endpoints, same field shapes, same derived_view semantics. The revert fixes a 404 bug where duckdb_projects (0 rows) caused project-not-found before the SQLite fallback was reached. -->

<!-- Last reviewed 2026-06-10 — WO-DEBT-B research router: discovery_research.py added as a new FastAPI router under /api/discovery/research (POST, GET, GET/{cache_id}, DELETE/{cache_id}). Routes read and write the research_cache SQLite table only (no authority tables). Dashboard projection contract shape is unchanged: no new projection shapes, no removed fields, no new authority introduced. The discovery research router is a cache-backed lookup surface, not a dashboard projection. -->

<!-- Last reviewed 2026-06-11 — WO-FULLCI-RED findings/read_models correction: FACT_TABLES in read_models.py updated — "findings" replaced with "findings_current_status" (findings retired in migration 112). All direct "findings" table references in read_models.py updated to use findings_current_status with project-only scope (the projection table lacks milestone_id/task_id/process_run_id). record_security_finding() in execution_spine.py now eagerly calls FindingsProjection.fold_spine() after writing to security_events, keeping findings_current_status in sync at write time. Dashboard projection contract shape preserved: same /api/telemetry/* route endpoints, same field shapes, same derived_view semantics. The read source changed (findings_current_status + security_events spine) but the model contract is unchanged. -->

<!-- Last reviewed 2026-06-14 — WO-DASH-EVAL: New route GET /api/v1/eval/registry added (projections/api/routes/evals.py, registry_router; registered in main.py at prefix /api/v1/eval). Returns list of {target_id, target_type, rubric_score, baseline_score, friction_flag, last_run_at, last_run_id} from eval_registry LEFT JOIN ds_eval_runs on baseline_run_id. Returns [] gracefully when eval_registry table is absent (migration 119 not run). Eval Health panel added to dashboard Overview tab (dashboard.html): table showing Skill/Target | Type | Score | Baseline | Friction | Last Run; friction_flag=true rows highlighted in yellow. Route is a read-only derived projection over eval_registry + ds_eval_runs — no authority writes, no new scans, no schema changes. Dashboard projection contract model is unchanged. -->

<!-- Last reviewed 2026-06-14 — WO-DASH-CONFIG: New route GET /api/v1/config added (projections/api/routes/config.py; registered in main.py at prefix /api/v1). Returns list of {key, value, updated_at} from ds_config ordered by key. Returns [] gracefully when ds_config table is absent. Config tab added to dashboard with sidebar nav item (CF mark, Governance section): table showing Key | Value | Last Updated. Route is read-only over ds_config — no authority writes, no schema changes. Dashboard projection contract model is unchanged. -->

<!-- Last reviewed 2026-06-14 — WO-READMODEL-WIRING: Three read-model repoints, no new routes or schema changes. (1) skill_usage_sql() in authority_sources.py now prefers execution_events WHERE event_type='skill.invoked' (199 rows, direct skill_id column) over the dropped skill_invocations table; canonical_events remains as a fallback. (2) _fallback_hook_invocations() in hooks.py updated with accurate commentary — hook.tool_activity IS emitted (5,094 rows in execution_events); hook_executions primary path has 1,151 rows. (3) _process_run_drilldowns() in read_models.py now derives process run entries from execution_events GROUP BY process_run_id instead of the empty process_runs table. Dashboard projection contract shape is unchanged: same route endpoints, same field shapes, same derived_view semantics. Read sources corrected; no authority writes. -->

<!-- Last reviewed 2026-06-14 — WO-DASH-FRONTEND: Removed Shared Intelligence section from Overview (HTML + JS; shared_intelligence.py route re-registered for Adapter projections endpoint). Added Adapter Projections panel to Overview (GET /api/shared-intelligence/adapters/projections). Added "Development — Not Yet Wired" collapsible section with static cards for process_runs drilldown, SI learning/context/capability/models panels — each labeled with source table. Removed Goal to Action static card (overview entry from DASHBOARD_PAGE_STORIES). Moved telemetry drilldown-detail to Development section. Registered /api/v2/eval/registry and /api/v2/config routes (same handlers, v2 prefix). Dashboard fetches for eval/registry and config updated to /api/v2/*. No new authority writes. -->

<!-- Last reviewed 2026-06-14 — WO-DASH-VALIDATION-GAPS (T4): dashboard.html loadHooksData() rewritten — now uses Promise.all for parallel fetches, updates summary cards (total executions, failures, success rate, anomalies), creates hooks timeline chart (7-day bucket) and performance distribution chart via Chart.js lazy initialization from real API data, and populates per-hook status cards from statsData.by_hook. initHooksCharts() on-load call removed (was creating empty placeholder charts before data arrives). Dashboard projection contract shape unchanged: same /api/telemetry/hooks/* route endpoints, same field shapes, same derived_view semantics. -->

<!-- Last reviewed 2026-06-17 — WO-DASH-RENDER-FIX: frontend render-path fixes in dashboard.html only — NO projection-model contract change. Null-guarded the hooks-total-anomalies write (element removed from DOM); added honest empty-states for hooks cards, hooksPerformance / model-distribution / cost-over-time / workflow-success / skill-success / skill-execution-time charts (replace stuck 'Loading…'/blank canvas); initSkillsTab no-data + catch paths call populateSkillsLeaderboard([]); initWorkflowsTab early-return + initPromotedVsDraftChart show honest empty-states. Same API routes, field shapes, and derived_view semantics; the Dev "Not Yet Wired" section is unchanged. -->

<!-- Last reviewed 2026-06-17 — WO-DASH-DEVDIAG-CONSOLIDATE: dashboard.html only, NO projection-model contract change. Consolidated developer diagnostics into ONE dedicated "Developer Diagnostics" tab: moved the Tool Activity / Event Validation Failures / Raw Event Stream three-block out of the loose hooks-scoped hooks-invisible-tables div into a new id="developer-diagnostics" tab-content (same element ids, fed by the same /api/v1/hooks/tool-activity|validation-failures|raw-events routes), and removed the per-page createDeveloperDiagnostics RAW_TECHNICAL_MARKERS drawer that ensureBusinessStorySections() injected into every page. loadInvisibleTables() now loads on the new tab. Security tab still leads with its findings summary cards; the Overview "Development — Not Yet Wired" section is untouched. Same routes, field shapes, derived_view semantics. -->

<!-- Last reviewed 2026-06-17 — WO-DASH-COST-PLAN: reportable cost is unchanged and honest (read_models._token_rollup still sums only provider-backed cost_visibility rows). Added a NEW, gate-safe API-equivalent cost surface: projections/core/cost_analysis.py (api_equivalent_cost + plan_comparison) computes API-equivalent USD over all token_usage_records via core/pricing/claude_models.compute_cost, honest priced/unpriced coverage; exposed via new route GET /api/v1/cost/plan-comparison (projections/api/routes/cost_plan.py, registered in projections/api/main.py). dashboard.html adds a "Plan vs API-equivalent cost" panel on the Models tab (config-driven plan from ds_config cost.plan_name/cost.plan_monthly_usd, honest empty-state when unset; label "API-equivalent cost", never "Estimated cost") plus a coverage note on the reportable-cost KPI. Fake-cost regression gate still passes (no cost math in the four scanned files). No change to existing projection routes/shapes/derived_view semantics. -->

<!-- Last reviewed 2026-06-17 — WO-EVALS-RENDER: bug fix only, NO contract change. projections/api/routes/evals.py GET /api/v1/evals/health queried non-existent ds_eval_baselines columns (eval_version/score/passed) and 500-ed, blanking the Evals tab despite ds_eval_runs=287/ds_eval_baselines=8. Corrected to the real schema (version, baseline_score, regression_flag; pass = NOT regression_flag) per migration 092. Same route, same response shape/field names (version/score/passed in the JSON), same derived_view semantics; dashboard.html render unchanged (it was already correct once the route returns 200). -->

<!-- Last reviewed 2026-06-18 — WO-DASH-ANALYTICS-TABS: analytics-tab render fixes; one genuine backend bug fix, no contract shape change. analytics.py session-outcome-flow: 'timeout' previously absorbed 'unknown'+'in_progress' (every unresolved session looked like a timeout) — now 'timeout' counts only real timeouts and a new 'other' bucket holds unknown/in_progress (session_flow gains an 'other' key). New read-only route GET /api/v1/insights/attribution-breakouts (insights.py) aggregates token_usage_records by project/milestone/task/skill/agent (top-20 each, existence-guarded) for the Token-Attribution tab. dashboard.html: Learning/Adaptation charts now show honest empty-states when their source tables are empty (learning_event_records=0 etc.) instead of flat-zero/blank; trend-regression series key corrected (reportable_cost); Config + Memory-Surface tabs gain explanatory descriptions (kept, not removed). No fabricated cost/data; fake-cost gate still passes. -->

<!-- Last reviewed 2026-06-18 — WO-DASH-OVERVIEW-ORDER: pure Overview-tab reordering in dashboard.html, NO contract/route/shape change. The Telemetry Traceability section was rendered BEFORE the primary graphs (confusing); moved it BELOW the four primary graphs (Session Timeline, Top Skills, Model Distribution, Cost Over Time) and before the Eval Health panel. KPIs (reportable cost / success rate / active alerts) stay at the top. Same elements, ids, routes, and derived_view semantics — only DOM order changed. -->

<!-- Last reviewed 2026-06-18 — WO-CAPTURE-SKILL-DURATION: derive per-invocation skill duration READ-ONLY (no schema change, no new execution_events writer). projections/core/collectors/authority_sources.py LEFT-JOINs execution.started↔execution.completed on process_run_id and computes execution_time_s = (julianday(completed)-julianday(started))*86400; unpaired invocations stay NULL (excluded from AVG, never fabricated). skill_collector.py surfaces min/avg/max; metrics.py /skills leaderboard falls back to the derived min/max when canonical skill.executed ranges are absent, so the Skills avg-duration card + execution-time-distribution chart render real data (honest empty-state when no paired events). Same routes/shapes/derived_view; no cost math (fake-cost gate green). -->

<!-- Last reviewed 2026-06-18 — WO-DASH-ATTRIBUTION-SURFACES: Token-Attribution breakouts now enrich by_project with the project NAME (read-only JOIN to business_projects in projections/api/routes/insights.py; dashboard.html shows name not UUID). Token→skill attribution fixed at CAPTURE (core/skills/invocation.py writes active_skill.json; core/telemetry/token_capture.py stamps skill_id into the token.consumed trace) — historical rows were NOT backfilled (process_run_id is NULL across token rows → no honest correlation; not fabricated). cost-by-model verified correct (blank is data-quality: 33 of 40 token rows have NULL model_id; the 7 known-model rows compute non-zero). Project Security 'blank' is an HONEST empty state (the 15 findings belong to DreamySuite, not Dream Studio) — no code change. Read-only for projection tables; no cost math in scanned files (fake-cost gate green); same routes/shapes/derived_view. -->

<!-- Last reviewed 2026-06-19 — WO-DASH-ACTIONABILITY: dashboard.html surfaces made actionable (presentation only, no projection/read-model contract change). Evals tab leads with a "Needs Attention" panel (regressions from /api/v1/evals/health baselines + friction/pending-rerun from /api/v2/eval/registry) before the raw tables; config tab adds a "What it does" column (client-side key descriptions); memory-surface adds a why-it-matters line per surfaced memory; learning tab retains its honest empty-state. No API response shape, projection, or SQLite boundary changed. -->

<!-- Last reviewed 2026-06-20 — WO-SPLIT-PROJECT-INTEL (feat/split-project-intel-routes): project_intelligence.py (2480 lines) split into projections/api/lib/ (security_helpers, stack_helpers, project_helpers) and four route files (project_list, project_detail, project_artifacts, project_security). Pure module reorganization — no SQL queries, schema, migration, business logic, or API contract changed. -->

<!-- reviewed 2026-06-26: migration 128 dead-tables removal — no content changes required -->

<!-- Last reviewed 2026-06-27 — WO-READMODELS-DUCKDB (worktree-agent-a57e51b228124e525): dashboard read-model routes repointed from SQLite to the DuckDB read-model views in aggregate_metrics.db. hooks.py (/hooks/executions, /hooks/executions/{exec_id} now keyed on the event_id UUID, /hooks/performance), analytics.py, insights.py and intelligence.py now read the DuckDB hook_executions/validation_failures/raw_sessions views (derived from canonical events via events_fact) instead of the SQLite projection tables. The /hooks/findings route was removed (hook_findings carried 0 rows). validation_failures + hook_executions SQLite tables dropped in migration 129; vw_hook_performance retired. Dashboard projections remain derived/non-authority — these routes are read-only and do not mutate canonical runtime state. raw_sessions and token_usage_records stay on SQLite (read-write session-lifecycle authority and cost data DuckDB cannot derive, respectively). No projection-model artifact shape changed; this is a data-source repoint only. -->

<!-- Last reviewed 2026-06-27 — Wave 1 migration 130 (chore/wave1-substrate-drop): removed _research_blocker_resolution(), _artifact_lineage_lifecycle(), and _artifact_lifecycle_counts() read-model functions from read_models.py; these queried blocker_resolution_records and artifact_records (both 0-row aspirational tables with no production writers). Removed artifact_analytics dashboard module from execution_spine.py. Dashboard routes that previously returned 'research_blocker_resolution' and 'artifact_lineage_lifecycle' keys in the telemetry summary no longer include those keys; they were always empty (0-row sources). No shape change to any populated dashboard route. -->

<!-- Last reviewed 2026-06-27 — Wave 2 substrate realignment (migration 131, worktree-agent-a910d590fedb5c672): alert_history dropped migration 131 — projections/core/alerts/alert_evaluator.py.trigger_alert() now builds the triggered-alert dict in memory only (no persistence), and the read-only /api/v1/alerts/history + /alerts/analytics routes are removed from projections/api/routes/alerts.py (rule CRUD on alert_rules + SLA routes retained). Dashboard read-models that named route_decision_records (dropped) degrade via object_exists() to 0. No projection-model artifact shape change for any populated route; alert config remains advisory projection service state. -->

<!-- Last reviewed 2026-06-28 — Batch 1 canonical-first migration (migration 133): compliance_review_flags removed from production_readiness_dashboard_summary() (table dropped; field was always empty list in production). source_tables in contract_atlas.py and shared_intelligence route now reflect accurate post-133 table set. No shape change to any populated dashboard route. -->

<!-- Last reviewed 2026-07-03 — WO-DBA-EVAL-DECISION (fcd8cf14) PR B / T4: migration 136 drops ds_eval_runs, hook_eval_runs, decision_log, decision_event_link. projections/api/routes/evals.py GET /api/v1/evals/health recent_runs now reads the 20 most recent eval.run.completed / work_order.verified canonical events from business_canonical_events instead of ds_eval_runs (same response keys: run_id, eval_id, total_score, passed, started_at, model_tested — null when a key is absent from the event payload). GET /api/v1/eval/registry baseline_score now LEFT JOINs ds_eval_baselines on eval_id instead of ds_eval_runs on baseline_run_id (same response shape). projections/api/routes/intelligence.py decision-tracking win counters now COUNT(*) business_canonical_events WHERE event_type='decision.recorded' instead of decision_log. No response shape, field name, or derived_view semantics changed on any populated route. -->

<!-- Last reviewed 2026-07-03 — WO-DBA-REPOINT (e51389d3): SessionCollector (projections/core/collectors/session_collector.py) now reads the DuckDB raw_sessions compat view first (connect_analytics read_only; canonical-derived, complete) with the SQLite raw_sessions table as fallback when the analytics store has no views yet or carries no rows for the window; response shape (total_sessions/by_project/timeline/day_of_week/outcomes/avg_duration_minutes/success_rate/source_status) unchanged, source_status now names the serving source. analytics.py get_anomalies gains the missing-view guard (empty shape, never a 500, matching the other repointed routes). Token/cost, skill-telemetry, and handoff readers deliberately remain on SQLite (governance-protected NULL cost columns in the view; no views for raw_skill_telemetry/sum_skill_summary/raw_handoffs). -->

<!-- Last reviewed 2026-07-03 — WO-TOKEN-VIEW-WIDEN (8bc3731b, operator-corrected scope): the DuckDB token_usage_records view now derives model_id (COALESCE of the events_fact model_id column and payload $.model), cached_tokens ($.cache_creation_input_tokens), cache_read_tokens ($.cache_read_input_tokens), and estimated_cost (LEFT JOIN token_model_pricing — a projection of core/pricing/claude_models.py refreshed on every ensure_analytics_schema — same arithmetic as compute_cost, model suffix normalized via regexp_replace). Source is the TA3 token.consumed family in ai_canonical_events, whose payloads already carry these keys (coverage identical to the SQLite authority). Events without a model keep NULL cost — never fabricated. Response shapes of the view unchanged (columns were previously hardcoded NULL). -->

<!-- Last reviewed 2026-07-03 — WO-DBA-DROP (authority e0b90310-bf30-4146-b9df-58707a2e5d86): migration 137 drops the SQLite token_usage_records table (the 2026-06-27 WO-READMODELS-DUCKDB entry above listed it as staying on SQLite; the WO-TOKEN-VIEW-WIDEN widening above removed the reason it had to). Every token-reading dashboard route repointed to the DuckDB view, response shape unchanged: projections/api/routes/analytics.py get_trends() token/cost series now read the same DuckDB connection already open for session trends (previously a separate SQLite query via token_usage_sql()); projections/api/routes/insights.py get_attribution_breakouts() reads the DuckDB view directly with the SQLite business_projects.name enrichment kept as a second connection (same two-connection pattern as analytics.py); projections/api/routes/metrics.py and intelligence.py are unchanged at the route level because they go through projections/core/collectors/authority_sources.py::token_usage_sql(), which now materializes DuckDB rows into a connection-scoped SQLite TEMP TABLE when the SQLite table is absent (so the existing "embed as subquery" call sites keep working without a route edit). core/telemetry/read_models.py token rollups (global/project/milestone/task summaries, process_run_timeline, workflow_execution_graph) read the DuckDB view when the SQLite table is absent, with one honest shape difference: process_run_id is never populated (canonical token.consumed events carry no process_run_id — the view hard-codes NULL), so scope filters no longer key on it for token data. provider is likewise always "unknown" for DuckDB-sourced rows (canonical events carry no provider field; the old value was inferred at SQLite write time from the model name). No route was removed and no field was renamed. -->


<!-- Last reviewed 2026-07-04 — migration 139 (WO-AI-SPINE, AD-5): core/telemetry/read_models.py and dashboard_freshness.py repointed decision/outcome/attention read models from the dropped decision_records/outcome_records/dashboard_attention_items tables (0/2/0 production rows; pure duplication of the execution_events row their writers already wrote) to execution_events, filtered by event_type + outcome_status. Response dict/list shapes are unchanged; fields never persisted anywhere except the dropped tables (decision selected_option/rationale, outcome/attention summary, true attention title/severity for research/decision events) are honestly NULL or best-effort derived rather than fabricated. No dashboard projection is primary authority; this is a source-table change behind an unchanged contract. -->

<!-- Last reviewed 2026-07-04 — migration 140 (WO dff23cb0-950f-4607-bb30-e1a353a6f8ba): findings_current_status (FindingsProjection.fold_spine() upsert over security_events, 15 rows, pure derived state) dropped. core/telemetry/{read_models,dashboard_freshness,execution_spine}.py and projections/api/routes/{project_detail,project_list,project_security,security}.py repointed every `FROM findings_current_status` to core/findings/current_status.py::FINDINGS_CURRENT_STATUS_SQL, a SQL fragment reproducing the identical current_status derivation (latest finding.status_changed/finding.resolved event body per finding, else 'open') computed fresh on every query — no persisted table, no replacement table, same precedent migration 139 set for decision/outcome/attention above. Response dict/list shapes and dashboard route contracts (/api/v1/security/*, /api/v1/projects/*) are unchanged. No dashboard projection is primary authority; this is a source-table change behind an unchanged contract. -->

<!-- Last reviewed 2026-07-04 — migration 141 (WO 9f47a1a0-11db-4fcb-9d93-d350fd9a5a6f): raw_workflow_runs/raw_workflow_nodes dropped (write-orphaned since 2026-05-18; archive_workflow() silently swallowed its own INSERT failures). core/telemetry/dashboard_freshness.py's legacy_component_rows heuristic no longer counts raw_workflow_runs (removed from the table_counts scan; workflow freshness now follows workflow.completed canonical events in ai_canonical_events, not counted in this legacy set). projections/core/collectors/workflow_collector.py and projections/core/sla/tracker.py (both outside projections/api/routes but sharing this contract's data-source concerns) were repointed from the dropped tables to ai_canonical_events WHERE event_type IN ('workflow.completed', 'workflow.node.completed'), preserving their existing output shapes. No dashboard route response shape changed; this is a source-table change behind an unchanged contract, same precedent as migrations 139/140 above. -->

<!-- Last reviewed 2026-07-04 — WO-CI-460-FOLLOWUP (d033e6f8): comment-only reword in core/telemetry/dashboard_freshness.py (the boundary grep matched canonical_events inside a comment naming ai_canonical_events). No projection shape, source-table, or freshness-semantics change. -->

<!-- Reviewed 2026-07-05 — WO 6d978483 (PEP 585/604 modernization [2/2]): source files in this domain received mechanical type-annotation modernization only (PEP 585 builtin generics, PEP 604 unions, datetime.UTC) via ruff UP safe autofixes. No contract, behavior, schema, routing, API-shape, or CLI-surface change — reviewed, no doc content change needed. -->

<!-- Reviewed 2026-07-07 — WO-SPLIT-DASHBOARD (fe3007fc): projections/frontend/dashboard.html inline <style> (~596 lines) and <script> (~8160 lines) were extracted byte-faithfully to frontend/static/{dashboard.css,dashboard.js}, mounted at /static (FastAPI StaticFiles). dashboard.js loads as a CLASSIC script so the 71 inline handlers keep resolving window globals. Coupled tests + the dashboard smoke harness were migrated to read the combined source / static assets. No projection contract, API response shape, or dashboard data-model change — static-asset extraction only. -->

<!-- Reviewed 2026-07-07 — WO-DASH-GRAPHS-500 (7f8e6b83): the DuckDB raw_sessions view types started_at as a VARCHAR ISO string (= events_fact.event_timestamp). Two analytics routes assumed date/timestamp types and 500'd, leaving the Trends and Performance graphs blank. projections/api/routes/analytics.py get_trends() sessions series now slices substr(started_at,1,10) so it yields string dates matching the token series (previously DATE(started_at) returned datetime.date and mixed with the token substr str in sorted(set(...)) -> TypeError); get_performance() now CASTs started_at AS TIMESTAMP before strftime('%w'/'%H') at all three call sites (previously strftime on a VARCHAR -> DuckDB BinderException). Regression test: tests/integration/test_dashboard_analytics_endpoints.py. No response shape or field change — a query type-coercion fix behind an unchanged contract. -->

<!-- Last reviewed 2026-07-08 — WO-DASH-DUCKDB-PROJECTION (899d3f17-e9a1-4bc2-8ba6-dc04188ad9b2): core/telemetry/read_models.py component reads repointed onto the DuckDB analytics store (aggregate_metrics.db events_fact projection). The workflow component usage now derives from events_fact (workflow.completed / workflow.node.completed) and the validation rollup from events_fact (validation.result_recorded), via a new fail-open read-only _analytics_rows helper + a _component_usage_from_events_fact dispatcher (_DUCKDB_COMPONENT_EVENT_TYPES gates which components read DuckDB). component_usage_summary source_tables now names events_fact instead of the execution_events spine. hook/tool/skill/agent component usage stays on the SQLite spine until their capture reaches canonical (WO-HOOK-EXEC-STATS / WO-AGENT-TELEMETRY) — hook.tool_activity has no canonical equivalent, so repointing now would drop live telemetry rather than gain completeness. Validation is deliberately NOT conflated with the validation_failures view (event.validation.failed schema-rejections — a distinct metric); the validation.result_recorded read is honestly empty until WO-VALIDATION-CAPTURE. The dead duckdb_execution_events table (never-emitted execution.* feed) is unused; events_fact is the live projection. No dashboard route response shape, field name, or derived_view semantics changed — a read-source repoint behind an unchanged contract. -->

<!-- Last reviewed 2026-07-08 — WO-AGENT-TELEMETRY (15735272-93e8-48ab-aea2-50d61ccdc25a): subagent (Task tool) invocations now emit agent-identified events, so the agent dashboard component reads them from the DuckDB events_fact projection (the SQLite spine never carried agent_id — it was always NULL). core/telemetry/read_models.py: "agent" added to _DUCKDB_COMPONENT_EVENT_TYPES (agent.execution.completed/started/failed), so the agent component usage now derives from events_fact grouped by agent_id. Capture side (outside this contract's read surface): emitters/claude_code/emitter.py normalize_post_tool_use emits agent.execution.completed with trace.agent_id=tool_input.subagent_type for Task calls; core/telemetry/token_capture.py stamps agent_id/agent_type onto the Task call's token.consumed; config/event_type_registry.py registers agent.execution.* → ai_canonical_events. No dashboard route response shape, field name, or derived_view semantics changed — the agent component's source moves from the (empty) SQLite spine agent_id to the events_fact agent_id, behind an unchanged contract. -->


<!-- Last reviewed 2026-07-15 — WO-SCHEMALEAN (migration 147): removed GET /api/shared-intelligence/capability-routes (get_capability_routes/capability_route_summary) + its dashboard surface entry — it read the dropped, permanently-empty capability_route_records. No projection response-shape change to any populated surface; the recommendation-preview route is kept. -->

<!-- Last reviewed 2026-07-17 — WO-SCHEMALEAN (migration 149): removed the entire /api/v1/audits/* route module (projections/api/routes/audits.py) + its main.py registration — it read the dropped, permanently-empty audit_runs (and already-dropped sec_sarif_findings/sec_cve_matches). No projection response-shape change to any populated surface; the live security surface (/api/v1/security/* over security_events + scan_runs) is the equivalent. The dead dashboard "Security Audits" subtab / audit-history tab degrade gracefully (existing try/catch → empty/error) and are removed in a follow-up dead-UI cleanup WO. -->

<!-- Last reviewed 2026-07-24 — WO 76d0744e (fix/analytics-read-no-fabricate): connect_analytics(read_only=True) no longer fabricates an empty native DuckDB store when aggregate_metrics.db is absent — it raises the typed AnalyticsStoreMissingError and creates no file (only the read_only=False projection-runner path creates the store). This upholds the contract rule "projections must not claim readiness when source artifacts are missing or stale": a fabricated empty store served zero-row analytics as if real. Read routes degrade to their existing honest empty shapes via the typed error — analytics.py (/anomalies, /trends, /performance), insights_diagnostics attribution-breakouts (_EMPTY), and intelligence_overview (performance-alerts / health-snapshot / whats-working); the shared read helpers (_analytics_rows, authority_sources, session_collector) and the dashboard-truth / freshness gates already caught it via their broad excepts. No dashboard route response shape, field name, or derived_view semantics changed — same empty shapes, reached honestly. -->

<!-- Last reviewed 2026-08-05 — WO 62aa33b1 / P5 (feat/prd-sow-dashboard): new read-only route group GET /api/v1/prd-sow/active and GET /api/v1/prd-sow/{project_id} (projections/api/routes/prd_sow.py; registered in main.py at prefix /api/v1/prd-sow). Returns the PRD+Statement-of-Work panel shape (schema dream_studio.prd_sow.panel.v1: overall_score, coverage, confidence, per-capability entries, per-milestone SOW entries) via projections/api/lib/prd_sow_panel.build_prd_sow_panel, which calls the engine's NO-WRITE core.prd.compute_prd_sow — so serving the dashboard never mutates the docstore. Derived read-only over business_milestones/business_work_orders + the files.db docstore capability map; no authority writes, no new studio.db table, no schema change. /active resolves business_projects status='active' (404 when none). Dashboard projection contract model is unchanged — a new derived read surface, same derived_view semantics. -->

<!-- Last reviewed 2026-08-08 — WO-CLIENT-DASHBOARD-API (Client Layer, Phase 2): GET /api/v1/projects (projections/api/routes/project_list.py) gains a nullable `client_id` field per project row, read via a guarded column expr (p.client_id when the migration-155 column exists, else NULL) so a pre-155 live DB degrades to client_id=None rather than erroring. Purely ADDITIVE — no removed/renamed fields, no route change, derived_view semantics unchanged; the dashboard's visual client-rollup view is a later Dashboard Coherence concern, this WO ships only the data field. Read-only over business_projects; no authority write, no new table. -->

<!-- Last reviewed 2026-08-08 — WO-CLIENT-DASHBOARD-API (filter follow-up): GET /api/v1/projects gains an optional `?client=<id>` query param that scopes the listing to one client's projects (a post-inclusion filter on the client_id field; a pre-migration-155 DB has client_id=None so filtering by a client simply matches nothing rather than erroring). Additive optional query param — omitting it preserves the prior full-list response exactly; derived_view semantics, field shapes, and route path unchanged. Read-only. -->

<!-- Last reviewed 2026-08-10 — WO-DASH-COHERENCE T3 (fix/dashboard-prd-tab-rewire): the dashboard PRD tab (frontend) was rewired off the dropped /api/prd/list + /api/prd/{id} routes onto the live read-only PRD+SOW panel already documented in the 2026-08-05 P5 entry above. projections/frontend/static/dashboard.js loadPRDData() now fetches GET /api/v1/prd-sow/active (the active project's capabilities + per-milestone SOW entries) and viewPRDDetails() fetches GET /api/v1/prd-sow/{project_id} for the per-milestone capability modal; projections/frontend/dashboard.html relabels the tab "PRD + Statement of Work" with Milestone/Status/Score/Capabilities columns. The orphaned getTaskStatusColor helper (whose only caller was the removed per-PRD task table) was deleted. This is the frontend consuming an already-shipped, already-documented derived read surface — no new route, no new response shape, no new studio.db table, no schema change. Dashboard projection contract model is unchanged — a frontend data-source repoint behind an unchanged contract. -->

<!-- Last reviewed 2026-08-10 — WO-DASH-DRILLDOWN (feat/dashboard-project-drilldown): new read-only drill-down surface so the dashboard Projects modal can navigate project -> milestones -> work orders -> tasks. New route module projections/api/routes/project_drilldown.py (registered in main.py at prefix /api/v1): GET /projects/{project_id}/milestones -> {project_id, milestones:[{milestone_id,title,status}], count}; GET /milestones/{milestone_id}/work-orders -> {milestone_id, work_orders:[{work_order_id,title,status,type}], count}; GET /work-orders/{work_order_id}/tasks -> {work_order_id, tasks:[{task_id,title,status}], count}. Read-only over the SQLite authority tables business_milestones / business_work_orders / business_tasks, each object_exists-guarded so a snapshot missing a table degrades to an honest empty list rather than a 500 (unknown ids likewise return empty, never an error). No authority write, no new studio.db table, no schema/migration change. Frontend: a Milestones sub-tab in the project modal (projections/frontend/{dashboard.html,static/dashboard.js}) drills the three levels with breadcrumb back-nav (IDs only in onclick; titles cached client-side). This is a new derived read surface with the same non-authority, derived_view, honest-empty-state semantics the contract already requires — the projection model is unchanged. -->

<!-- Last reviewed 2026-08-10 — WO-DASH-COHERENCE T4 (fix/dashboard-remove-dead-panels): frontend-only removal of dead UI for retired features — no route, response-shape, or projection-model change. (1) Alert firing history + analytics were retired by migration 131 (alerts fire in-memory only; /api/v1/alerts/history + /api/v1/alerts/analytics were removed from alerts.py per the 2026-06-27 Wave-2 entry above). Removed the Alert History timeline sub-panel, the Top Triggered Rules + Resolution Time analytics charts (initAlertsTab now fetches only /sla + /rules), and the dead /api/v1/alerts/history fetch behind the Overview "Active Alerts" KPI — the KPI/bell/drawer chrome is retained but now shows an honest 0 (no live persisted alert source). (2) Project detail sub-tabs PRDs (prd_documents dropped, migration 103) and Dependencies (pi_dependencies dropped) removed — buttons, panels, loaders, and switchProjectTab branches; Overview/Security/Activity retained (Activity reads execution_events, still live). (3) The extensions effect-count (loadEffectSummary → /api/v1/intelligence/extensions/{id}/effect-summary) removed — findings was dropped so it was always empty. The live Alerts surface (SLA gauges + rule CRUD) and Project Overview/Security/Activity are unchanged. Dashboard projection contract model is unchanged — dead-UI removal behind an unchanged contract. -->

<!-- Last reviewed 2026-09-23 -- fix/collectors-read-the-store-they-were-given: metrics.py, analytics.py, intelligence_overview.py, intelligence_domains.py, and core/telemetry/{read_models_tokens,read_models_timeline}.py all call authority_sources.token_usage_sql()/fetch_token_usage_records(), which previously resolved its DuckDB aggregate_metrics.db read via connect_analytics()'s ambient default regardless of which authority the route's own conn (from core.config.database.get_connection(), scoped to DREAM_STUDIO_DB_PATH) belonged to -- a route bound to one authority could read another authority's token rows once anything had populated the ambient DREAM_STUDIO_HOME store. Each now passes core.analytics.duckdb_store.analytics_db_path_for_connection(conn), resolving the aggregate_metrics.db sibling of that conn's own file (via PRAGMA database_list) instead. No route response shape, field name, or derived_view semantics changed -- a read-source scoping fix behind an unchanged contract, same class as the WO-DBA-REPOINT/WO-DASH-DUCKDB-PROJECTION entries above. -->

<!-- Last reviewed 2026-09-24 -- fix/hooks-and-read-models-follow-the-authority-not-ambient: closes the six sites PR #807's own entry above explicitly disclosed as out of scope. hooks.py (list_hook_executions, get_hook_execution_details, get_hook_performance, list_validation_failures -- 4 sites), insights_rhythm.py's get_work_rhythm, and insights_diagnostics.py's get_attribution_breakouts all called connect_analytics(read_only=True) with no path, so each resolved the ambient DREAM_STUDIO_HOME default regardless of which authority the request's own conn belonged to; each now derives analytics_db_path_for_connection(sql_conn) from a conn opened (or already held) for that purpose. core/gates/dashboard_truth.py's _run_duckdb_token_invariants and core/telemetry/dashboard_freshness.py's _duckdb_token_count now take the db_path each already threads down from their public entry point and resolve analytics_db_path_for(db_path) instead of the ambient default. core/telemetry/read_models_shared.py's _analytics_rows (feeding read_models_components.py's _component_usage_from_events_fact and read_models_validation.py's _validation_rollup) now takes the sql_conn both its existing callers already held (one call site's own comment previously read "conn is unused") and resolves analytics_db_path_for_connection(conn). No route response shape, field name, or derived_view semantics changed -- same class of fix as the entry above, and no projection-model artifact shape changed. Separately, insights_rhythm.py's own strftime(started_at, ...) calls needed an explicit CAST(started_at AS TIMESTAMP) -- a pre-existing bug independent of this fix, uncovered only because this PR is the first to exercise that route against real analytics data. -->


