...
<!-- Last reviewed 2026-07-04 — migration 140 (WO dff23cb0-950f-4607-bb30-e1a353a6f8ba): findings_current_status dropped — it was FindingsProjection.fold_spine()'s materialized upsert over security_events (15 rows), pure derived state with no independent signal. core/security/lifecycle.py::_security_open_finding_count() now derives the open-finding count from security_events at read time via core/findings/current_status.py::FINDINGS_CURRENT_STATUS_SQL (the same status-change derivation FindingsProjection used) instead of reading a stored table. The scanner→evidence-surface boundary is unchanged: record_finding()/set_finding_status() still write only to security_events (AD-6 emit-only pattern); no writer targets findings_current_status or any replacement table. Security lifecycle gate contract unchanged. -->
...
<!-- Last reviewed 2026-07-15 — WO-SCHEMALEAN (migration 147): capability_route_records dropped (shared-intelligence table). No security-lifecycle-gate behavior/contract change — a shared_intelligence source file in this domain's glob was touched only for the table drop. -->
