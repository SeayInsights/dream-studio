...
<!-- Last reviewed 2026-07-04 — migration 140 (WO dff23cb0-950f-4607-bb30-e1a353a6f8ba): findings_current_status dropped (pure derived state — FindingsProjection.fold_spine() upsert over security_events, 15 rows); core/security/lifecycle.py::_security_open_finding_count() now derives the open-finding count from security_events at read time via core/findings/current_status.py::FINDINGS_CURRENT_STATUS_SQL. Secure production readiness gate policy unchanged: same control framework, same evidence requirements, same authority boundaries; production readiness evidence and control results remain in SQLite authority (readiness_events). Gate contract is unaffected. -->

<!-- Last reviewed 2026-07-04 — WO-SQUASH-BASELINE (5fd84891-a329-48b8-b537-f0d4fc94d1a7, operator-approved irreversible squash): migration 040 (040_production_readiness_authority.sql) was collapsed, along with migrations 001-141, into core/event_store/migrations/142_lean_baseline.sql and deleted from the working tree (available in git history). No schema or gate-contract change: the baseline's CREATE TABLE IF NOT EXISTS statements re-emit the exact final schema migration 040's descendants left behind (release_readiness_records and compliance_review_flags were already dropped by migration 133, per the entry above), and production readiness evidence/control results remain in SQLite authority (readiness_events). This is a mechanical re-emission of the already-reviewed final schema, not a new gate or contract decision; satisfies the docs-drift domain whose source_patterns matched the deleted migration path by name. See docs/MIGRATION_AUTHORITY.md and docs/architecture/aspirational-schema-debt.md for the full squash regeneration method and verification result. -->
