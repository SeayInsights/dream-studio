...
<!-- Last reviewed 2026-06-27 — Wave 2 substrate realignment (migration 131, worktree-agent-a910d590fedb5c672): no docker-module-profile change. migration 131 retires dormant tables + dead writers/readers; no module profile, image, or packaging surface changes. flake8-baseline.txt regenerated (332 → 330). -->
...
<!-- Last reviewed 2026-09-21 — feat/backlog-tier-0: format and lint consolidated into pre-push (duplicate steps removed from ci.yml and ci_gate.py). No Docker profile change: no module profile, image or dependency set is affected by where a formatting check runs. -->
...
<!-- Reviewed 2026-09-23 — chore/run-docs-drift-before-push: interfaces/cli/contract_docs_drift_gate.py's changed-files resolution changed (see docs/PUBLICATION_BOUNDARY.md's matching note) and it now also runs in pre-push. No Docker profile change: no module profile, image, or dependency set is affected by how a docs-drift check resolves which files changed. Reviewed, no doc-content change needed. -->
...
<!-- Reviewed 2026-09-26 — feat/create-security-pack (pack-split campaign, security merge): repo_publication_readiness.py's secret-scan exemption path was repointed (quality:security merged into security:review), and core/projects/adaptive_routing.py/acquisition.py/intake.py were touched for an unrelated pack-mapping fix. No Docker module-profile change: no image, dependency set, or module profile is affected by a skill-content path or a pack-name mapping. Reviewed, no doc-content change needed. -->
