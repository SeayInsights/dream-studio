...
| `.github/workflows/ci.yml` (pr-smoke) | pull request, manual | **3-platform matrix** (ubuntu-latest, macos-latest, windows-latest): docs drift, Contract Atlas lifecycle, blast-radius, **the impact set** (`interfaces/cli/impact_tests_gate.py`: every test the blast-radius gate selects for this diff -- by module reference, or by naming a changed data file -- unioned with the sweep tests that walk source trees and can never be selected by reference; measured ~2 min, never empty), dashboard truth, format, lint baseline, pip-audit, and 11 hardcoded focused test files (10 unit + 1 integration) — still NOT the full suite. Before the impact step, pr-smoke ran 105 of 6,747 unit tests and three PRs merged green in one day leaving main red with failures the blast-radius gate had already printed and nothing ran. `fail-fast: false` — all platforms run to completion even if one fails. The matrix watch on these 3 jobs is the sole merge authorization. | Required remote confidence |
