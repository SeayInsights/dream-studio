...
<!-- reviewed: 2026-06-06, WO-C orphan rot sweep. core/module_contracts.py: removed dead test file reference from 3 module validation_tests lists. No troubleshooting guidance affected. No semantic change required. -->
...
<!-- Reviewed 2026-09-23 -- fix/home-means-home: every default Dream Studio home in core/ and interfaces/ now resolves through core.config.paths.home_dir() -- DREAM_STUDIO_HOME, else ~/.dream-studio, not created by resolving -- instead of each module spelling Path.home() / ".dream-studio" itself. With DREAM_STUDIO_HOME unset nothing moves: the default is the same directory. With it set (as `ds --home` sets it) these defaults now follow it: the installed runtime's default home (core/installed_runtime.py); a wrong home now shows up as DREAM_STUDIO_HOME, not as the OS home. The live-authority check and its backup in sqlite_bootstrap deliberately still recognise the operator's real home. No schema, table or command-surface change. -->
