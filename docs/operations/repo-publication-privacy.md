...
<!-- Reviewed 2026-08-05 — WO ed3aa5db (fix/unify-credential-patterns): the pre-publication secret-content scan (core/release/repo_publication_readiness.py::SECRET_CONTENT_RULES) now derives from the single credential-pattern source core/gates/credential_patterns.py instead of a local looser 4-rule subset, and its secret-scan path exclusions now match secret_scan's shared SCANNER_EXCLUDED_PATH_SUBSTRINGS. Effect: the scan detects MORE secret types (Slack/Google/Stripe/GitHub-PAT/aws-secret) with fewer false positives, and no longer self-trips on the sibling scanner's PEM test fixture (tests/unit/test_security_baseline.py). This STRENGTHENS the privacy scan; the publication boundary, the metadata-only/never-echo-values reporting contract, and the block-on-findings policy are UNCHANGED — no doc-content change needed. -->
...
<!-- Last reviewed 2026-09-21 — feat/backlog-tier-0: format and lint now run in the pre-push gate only (duplicate steps removed from ci.yml and ci_gate.py). No privacy boundary change: neither check reads, emits or classifies private content, and removing a duplicate run changes no artifact's classification. -->
...
<!-- Reviewed 2026-09-23 — chore/run-docs-drift-before-push: interfaces/cli/contract_docs_drift_gate.py's changed-files resolution changed (see docs/PUBLICATION_BOUNDARY.md's matching note for the mechanism), and it now also runs in pre-push. No privacy-classification change: the gate does not classify content as private, it detects PRIVATE_ARTIFACT_PATTERNS matches in whatever the changed-files list already contains -- which files can appear in that list changed, not what happens once one does. Reviewed, no doc-content change needed. -->
...
<!-- Reviewed 2026-09-26 — feat/create-security-pack (pack-split campaign, security merge): repo_publication_readiness.py's `_skip_secret_scan_path()` exemption for the security skill's deliberate fake-credential rule triggers was repointed from `canonical/skills/quality/modes/security/` to `canonical/skills/security/modes/review/` (same exemption reason as the 2026-05-28 entry above, only the path moved). No privacy-classification change: nothing about what counts as private data was touched. Reviewed, no doc-content change needed. -->
