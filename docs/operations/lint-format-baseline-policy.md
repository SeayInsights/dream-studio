...
**2026-09-23:** the "Focused smoke tests" step is gone from `.github/workflows/ci.yml`. Its sixteen files -- every one this page records as added to the focused smoke suite below -- are now `ALWAYS_RUN` in `interfaces/cli/impact_tests_gate.py`, and the impact step runs them on every PR across all three matrix platforms alongside the tests the change set reaches and the sweep class. One place decides what pr-smoke runs; the entries below are history. The pr-smoke job budget went from 15 to 30 minutes: the impact step measured 12m05s on Windows for a 127-file selection, and the 15-minute budget cancelled that leg after the step had passed. No change to the flake8 baseline or to what the format and lint checks decide.
...
headless baseline; the `security:review audit` / `ds-security` skills are for on-demand deep
...
<!-- Last reviewed 2026-06-15 — WO-BLAST-RADIUS-GATE: a new blocking pr-smoke step (`python -m core.gates.blast_radius`) and the `blast-radius` check in ci_gate.py run the nothing-left-hanging detectors at merge time. Adds a blocking gate; no change to the black/flake8 baseline policy itself. -->
