...
<!-- Last reviewed 2026-09-21 — feat/backlog-tier-0: format and lint consolidated into pre-push (duplicate steps removed from ci.yml and ci_gate.py). No external validation pipeline change — both checks are scoped to this repository's own source and are not part of the pipeline that validates an external project. -->
...
<!-- Reviewed 2026-09-23 — chore/run-docs-drift-before-push: interfaces/cli/contract_docs_drift_gate.py's changed-files resolution changed (see docs/PUBLICATION_BOUNDARY.md's matching note) and it now also runs in pre-push. No external validation pipeline change -- this gate is scoped to this repository's own source and docs, not part of the pipeline that validates an external project. Reviewed, no doc-content change needed. -->
...
<!-- Reviewed 2026-09-26 — feat/create-security-pack (pack-split campaign, security merge): repo_publication_readiness.py's secret-scan exemption path was repointed (quality:security merged into security:review), and core/projects/adaptive_routing.py/acquisition.py/intake.py were touched for an unrelated pack-mapping fix. Neither touches this repo's own external-project validation pipeline -- that pipeline validates OTHER projects' repos, not this one's own publication boundary. Reviewed, no doc-content change needed. -->
