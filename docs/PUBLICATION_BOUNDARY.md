...
<!-- Reviewed 2026-09-24 — fix/home-means-home: .github/workflows/ci.yml's pr-smoke job timeout-minutes goes from 30 to 45. This branch's 44-file diff to core/config/paths.py (a foundational, widely-imported module) selects 286 dependent test files -- measured locally at ~10m48s for the impact step alone, more than double the 127-file case the 30-minute budget was calibrated against. Windows' own slower ratio for this same step (documented in the workflow's own comment: 12m05s vs 4m08s ubuntu for a smaller selection) means the job needs real margin past a larger selection's worst case. Test time budget only: no release-gate policy, publication-boundary, privacy classification, packaging, module-profile, or black/flake8 baseline change. Reviewed, no doc-content change needed. -->
