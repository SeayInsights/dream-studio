...
<!-- 2026-06-06: WO-D dead discovery route removal — flake8-baseline.txt stale-entry cleanup (deleted file entries removed, fixed E402 line number). No publication boundary change; this is dead-code removal with no new public surface. -->
...
<!-- Last reviewed 2026-06-27 — Wave 2 substrate realignment (migration 131, worktree-agent-a910d590fedb5c672): no publication-boundary change. migration 131 retires 24 empty dormant tables + dead code/tests; only end-user-facing docs, shipping code, and CI tests are affected. flake8-baseline.txt regenerated (332 → 330). -->
...
<!-- Last reviewed 2026-09-21 — feat/backlog-tier-0: format and lint consolidated into the pre-push gate only; the duplicate `Format check` and `Lint baseline check` steps were removed from ci.yml and from interfaces/cli/ci_gate.py. No change to the publication boundary, privacy classification, Docker profiles or release-gate policy — the checks themselves, their single-source pins and what they decide are unchanged; only where they run moved, and no artifact's publishability turns on it. -->
<!-- Reviewed 2026-09-23 — feat/the-watcher-owns-main: .github/workflows/ci.yml pr-smoke loses the "Focused smoke tests" step; its sixteen test files become ALWAYS_RUN in interfaces/cli/impact_tests_gate.py and run inside the existing impact step, so one place decides what pr-smoke runs. The job's timeout-minutes goes from 15 to 30 to fit the impact step's measured Windows runtime. The Windows-only Store-alias scenario step is unchanged. Test selection and a CI time budget only: no release-gate policy, publication-boundary, privacy classification, packaging, module-profile, or black/flake8 baseline change. Reviewed, no doc-content change needed. -->
<!-- Reviewed 2026-09-24 — fix/home-means-home: .github/workflows/ci.yml's pr-smoke job timeout-minutes goes from 30 to 45. This branch's 50-file diff to core/config/paths.py (a foundational, widely-imported module) selects 697 dependent test files once the import-graph closure (#805) is counted -- measured locally at 20m02s for the impact step alone. An earlier measurement in this same comment (44 files -> 286 tests, ~10m48s) predated this branch merging #805 and undercounted by 2.44x; corrected here. Test time budget only: no release-gate policy, publication-boundary, privacy classification, packaging, module-profile, or black/flake8 baseline change. Reviewed, no doc-content change needed. -->

<!-- Reviewed 2026-09-23 — chore/run-docs-drift-before-push: interfaces/cli/contract_docs_drift_gate.py's changed-files resolution changed (an unresolvable base ref now enumerates tracked files instead of going blind to already-committed content, and a genuinely empty diff is trusted rather than falling through to an unrelated working-tree file), and canonical/workflows/pre-push.yaml now also runs this gate, pinning DREAM_STUDIO_CHANGED_FILES empty in its own env. Detection-mechanism and where-it-runs changes only: no publication-boundary rule, private-artifact pattern, or what counts as publishable changed -- see docs/operations/lint-format-baseline-policy.md for the mechanism detail. Reviewed, no doc-content change needed. -->

<!-- Reviewed 2026-09-24 — fix/home-means-home: .github/workflows/ci.yml pr-smoke job timeout-minutes goes from 45 to 90. The 45 figure was measured inside the review lane's own Docker container on this machine -- a Linux container despite the host OS being Windows, so it never actually exercised a Windows Python process. Real GH CI at the same 697-test selection: windows-latest's impact step exceeded 43m14s and was still running when the 45-minute job timeout killed it (twice, at two different commits on this branch), while ubuntu-latest/macos-latest finished the identical step in 18-21m in the same runs -- a >=2.07x ratio, consistent with the ~2.8x Windows/other-platform ratio already on record in this file's own history. 90 gives real margin past a ~57-minute Windows projection. Test time budget only: no release-gate policy, publication-boundary, privacy classification, packaging, module-profile, or black/flake8 baseline change. Reviewed, no doc-content change needed. -->

<!-- Reviewed 2026-09-24 — fix/home-means-home: .github/workflows/ci.yml pr-smoke timeout-minutes comment corrected to the real measured Windows numbers (71m51s impact step, 78m18s whole job, ~12 minutes of margin under the 90-minute budget) once the PR that carried the 90-minute bump actually ran on GH CI -- the value itself (90) is unchanged, only the projection it was justified by (a ~57-minute estimate) is replaced with what was actually measured. Comment-only: no release-gate policy, publication-boundary, privacy classification, packaging, module-profile, or black/flake8 baseline change. Reviewed, no doc-content change needed. -->

<!-- Reviewed 2026-09-26 — feat/create-security-pack (pack-split campaign, security merge): core/release/repo_publication_readiness.py's `_skip_secret_scan_path()` exemption for the security skill's deliberate fake-credential rule triggers was repointed from `canonical/skills/quality/modes/security/` to `canonical/skills/security/modes/review/`, since quality:security merged into security:review in this PR (same exemption reason as the 2026-05-28 entry above, only the path moved). No publication-boundary policy change -- the same content is exempted for the same reason, just at its new location. -->


<!-- Reviewed 2026-09-26 -- feat/gate-import-registry (C12/D18): interfaces/cli/contract_docs_drift_gate.py's main() gained an explicit argv parameter (was silently reading the real sys.argv, safe only because every gate always ran in a fresh subprocess before this change) so it can be called in-process from core/gates/registry.py; behavior is unchanged when invoked the way the manifest already does (no argv). interfaces/cli/lint_baseline.py (also in this domain's source_patterns) was read but not modified -- it already took an explicit argv. No release-gate policy, publication-boundary, privacy classification, packaging, module-profile, or flake8/black baseline change: this is execution-mechanism only (subprocess vs import), not a change to what any gate checks or decides. Reviewed, no doc-content change needed. -->
<!-- Reviewed 2026-09-26 -- feat/fail-open-census (H30): a new file, runtime/config/release-gates/fail-open-baseline.txt, joins this domain's runtime/config/release-gates/** source pattern -- it is a pinned-debt ledger for the new fail-open-census gate, the exact same shape and purpose as the existing flake8-baseline.txt in the same directory (a ratchet baseline, not a policy document). No release-gate POLICY, publication-boundary, privacy classification, packaging, or module-profile change -- this is a new gate's tracked-debt file, not a change to what any existing gate enforces or to how a release is judged. Reviewed, no doc-content change needed. -->
<!-- Reviewed 2026-09-26 -- feat/enforce-native-full-port (C11): touched only because this run edited docs/HOOK_RUNTIME.md and docs/WORKFLOW_RUNTIME.md (a publication-risk source pattern for this domain), documenting on-edit-enforce.py's native port going from scaffold to complete. No release-gate policy, publication-boundary, privacy classification, packaging, or module-profile change -- the edited docs describe an existing blocking hook's internals, carry no secrets, and change no publication decision. Reviewed, no doc-content change needed here. -->
