...
Two design skills now available: ds:design and huashu-design. Each has distinct strengths and use cases.
...
## Workflow: ds:design → huashu-design

**Ideal pattern for brand projects:**

### Step 1: Establish Brand Assets (ds:design)
```
User: "brand: Linear"
...
|------|----------------|-----------------|
| Get brand assets | ds:design | `brand: Stripe` |
| Need design direction | ds:design | `design direction: landing page` |
| Apply theme to project | ds:design | `apply theme: dark mode` |
| Create generative art | ds:design | `generative art: particle system` |
| Make ad creative | ds:design | `ad creative: Instagram feed 1080x1080` |
...
1. `brand: <product>` — ds:design establishes assets
...
1. `design direction: make a landing page` — ds:design offers 3 philosophy options
...
1. `brand: <new brand>` — ds:design acquires new assets
2. `apply theme: new brand to this codebase` — ds:design maps tokens to CSS
3. `verify: brand consistency` — Check all references point to brand-spec.md

### Pattern 4: Presentation for Client
1. `brand: <client name>` — ds:design gets client logo/colors
...
- Need brand assets first? → ds:design
- Have brand assets, need interactive prototype? → huashu-design
- Vague brief, need direction? → ds:design (Design Direction Advisor)
...
A: ds:design — it will extract assets from PDF and create brand-spec.md. Then huashu-design can use that brand-spec for deliverables.

**Q: Need a quick mockup without brand assets?**  
A: huashu-design — it can work standalone with its Design Direction Advisor fallback. But quality will be higher if you run ds:design brand: first.

**Q: Both skills have "Junior Designer Workflow" and "Design Direction Advisor"?**  
A: Yes, huashu-design pioneered these patterns and ds:design extracted them. Both implement the same philosophy. Use whichever skill you're already working with.

**Q: Generative art / p5.js — which skill?**  
A: ds:design — huashu-design doesn't cover generative/computational art.

**Q: MP4/GIF/PPTX exports — which skill?**  
A: huashu-design — ds:design doesn't have export infrastructure.

---

## Version History

- **2026-04-26** — Initial integration. ds:design enhanced with huashu patterns (Fact Verification, Brand Asset Protocol, Design Direction Advisor, Junior Designer Workflow). huashu-design installed as complementary skill for specialized deliverables.
