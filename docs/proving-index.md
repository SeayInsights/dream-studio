...
|-----------|---------|--------------|---------------|------|---------|
| Python | ✓ | dream-studio | Baseline (internal) | 2026-04 | 22+ rules fire on real Python codebase; founding proving ground |
...
| Ecosystem | Proven? | Proving Repo | PR / Evidence | Date | Summary |
|-----------|---------|--------------|---------------|------|---------|
| Python | ✓ | dream-studio | Baseline (internal) | 2026-04 | Core proving ground; all cq-* rules fire on Python |
...
| Ecosystem | Proven? | Proving Repo | PR / Evidence | Date | Summary |
|-----------|---------|--------------|---------------|------|---------|
| Python | ✓ | dream-studio | Baseline (internal) | 2026-04 | All tst-* rules fire on Python test suite |
...
| Ecosystem | Proven? | Proving Repo | PR / Evidence | Date | Summary |
|-----------|---------|--------------|---------------|------|---------|
| Python | ✓ | dream-studio | Baseline (internal) | 2026-04 | typ-002, dep-007 candidate/confirm proven |
...
| Ecosystem | Proven? | Proving Repo | PR / Evidence | Date | Summary |
|-----------|---------|--------------|---------------|------|---------|
| SQLite (Python) | ✓ | dream-studio | PR #134 description | 2026-06-01 | db-013 PASS (timeout=30.0); db-022 PASS (WAL mode); db-016 PASS (backup code exists) |
...
| Ecosystem | Proven? | Proving Repo | PR / Evidence | Date | Summary |
|-----------|---------|--------------|---------------|------|---------|
| Python (FastAPI) | ✓ | dream-studio + fastapi-template | PR #142 description | 2026-06-01 | 12 rules; api-001 FIRE (no Pydantic on /api/v1/ml/recommend); api-003 FIRE (no rate limiting); api-009 PASS (JWT in projections/api/); api-007 FIRE (password-recovery/{email} exposes user existence on 404) |
...
| Ecosystem | Proven? | Proving Repo | PR / Evidence | Date | Summary |
|-----------|---------|--------------|---------------|------|---------|
| Python (backend) | ✓ | dream-studio | PR #145 description | 2026-06-02 | arch-001 FIRE: core/projections/framework.py ProjectionEngine (22 methods, >400 LOC); arch-004 FIRE ×2: core/design_briefs/mutations.py:37 + core/health/status.py:22 core imports interfaces; arch-009 FIRE: projections/api/routes/shared_intelligence.py (31 import targets) |
...
| Ecosystem | Proven? | Proving Repo | PR / Evidence | Date | Summary |
|-----------|---------|--------------|---------------|------|---------|
| Python (service) | ✓ | dream-studio | PR #146 description | 2026-06-02 | ops-001 FIRE: print() in main.py:173, spool/ingestor.py, dispatcher.py; ops-007 FIRE: config validated lazily in projections/config/settings.py; ops-011 FIRE: urllib fallback path in dispatcher.py lacks timeout; ops-012 FIRE: single-stage Dockerfile (no multi-stage); ops-005/006/008/010/013 PASS; ops-013 SILENT (no k8s manifests) |
...
| Python | NEGATIVE PROOF | dream-studio | PR #149 description | 2026-06-03 | 0 findings confirmed — has_pii_schema=False (event store has no PII columns); validates auto-skip on operator data |
| Go | DECLARED | — | rules.yml only | — | Declared in applies_to; no Go proving run |
...
| Python (developer-tool) | NEGATIVE PROOF | dream-studio | PR #150 description | 2026-06-03 | pl-001/002 SILENT (developer-tool type — correct, not broken); pl-009 FIRE T1: non-semver tags (ph-X.Y.Z format); pl-011 FIRE T2 (warning only, not blocking for dev tool); pl-007 PASS |
