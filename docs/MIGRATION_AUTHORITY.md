...
<!-- Last reviewed 2026-07-03 — WO-DBA-EVAL-DECISION (fcd8cf14) PR A: migrations 134 (business_work_orders verify columns) + 135 (eval/decision history → business_canonical_events backfill; INSERT OR IGNORE idempotent). Rule 4 note: the SQL backfill writes the base table directly (migration-time, same precedent as 075/077); runtime emission goes through spool→ingestor only. -->

<!-- Last reviewed 2026-07-03 — WO-DBA-EVAL-DECISION (fcd8cf14) PR B / T4: Migration 136 (136_drop_eval_decision_tables.sql) drops the four legacy tables backfilled by migration 135 — ds_eval_runs, hook_eval_runs, decision_log, decision_event_link. Every reader was repointed to business_canonical_events (work_order.verified / eval.run.completed / decision.recorded) first: core/work_orders/verify.py, core/eval/runner.py, guardrails/evaluator.py, core/decisions/{emitter,query_engine}.py, projections/api/routes/{evals,intelligence}.py, interfaces/cli/commands/eval.py. Every legacy writer removed in the same change set — no INSERT/UPDATE against the four tables remains in the source tree (test_no_live_table_dropped gate). decision_records, eval_registry, and ds_eval_baselines are untouched. DROP TABLE IF EXISTS ×4; idempotent on both fresh installs (create-then-drop via the 031/104/119→136 chain) and live upgrades. .released_version bumped 135 → 136. -->

<!-- Last reviewed 2026-07-03 — WO-DBA-DROP (authority e0b90310-bf30-4146-b9df-58707a2e5d86): Migration 137 (137_drop_token_usage_records.sql) drops token_usage_records — reverses the migration-129 "NOT DROPPED" call now that PR #450/#451 widened the DuckDB aggregate_metrics.db token_usage_records VIEW to derive model_id, cached_tokens, cache_read_tokens, and estimated_cost (LEFT JOIN token_model_pricing) from events_fact; all 113 live SQLite rows were degenerate (billing_mode/cost_visibility='unknown', estimated_cost=0.0), carrying no information beyond the canonical token.consumed event stream. Every writer removed first: core/telemetry/execution_spine.py::record_token_usage deleted; core/telemetry/emitters.py::emit_token_usage_record's token_usage_records INSERT removed (its execution_events "token.usage_recorded" dual-write is untouched — unrelated telemetry); core/projections/token_projection.py::TokenConsumptionProjection deleted, registration removed from core/projections/runner.py (sync_tick + main()). Every reader repointed to the DuckDB view first (connect_analytics(read_only=True), never raising — a missing/unavailable analytics store degrades to an honest empty state, not a gate failure): projections/core/collectors/authority_sources.py::token_usage_sql() (a not-yet-migrated authority's real SQLite table is still read directly; otherwise DuckDB rows are materialized into a connection-scoped SQLite TEMP TABLE so token_collector.py/model_collector.py/projections/api/routes/{metrics,intelligence}.py keep working via the same subquery-embedding pattern, unchanged), projections/core/cost_analysis.py::api_equivalent_cost(), projections/api/routes/analytics.py::get_trends(), projections/api/routes/insights.py::get_attribution_breakouts(), core/telemetry/read_models.py (token_usage_records removed from the FACT_TABLES required-table gate), core/shared_intelligence/usage_accounting.py::_token_accounting_rows, core/gates/dashboard_truth.py (token_model_null_fraction/token_skill_attributed/priceable_cost_present now run against the DuckDB view; a missing store is pass-with-note and must not block work-order close). Migration-risk note: ai_usage_operational_records carried FOREIGN KEY (token_usage_id) REFERENCES token_usage_records(token_usage_id) (migrations 043/081) — this migration rebuilds it via the same table-reconstruction pattern migration 081 used (CREATE _new + INSERT...SELECT + DROP + RENAME) to drop only that FK clause; every other column/CHECK/FK and its migration-117 indexes are recreated unchanged. decision_records, eval_registry, ds_eval_baselines, raw_sessions, raw_token_usage untouched. studio.db 76 → 75 tables. .released_version bumped 136 → 137. Migration-risk gate: DROP-class change (plus a table-reconstruction on a live-writer table); matrix-watch required before merge. -->

<!-- Last reviewed 2026-07-03 — WO-CI-452-FOLLOWUP (dfddba19): migration 137 amended in place (merged <1h earlier, red full-ci, no downstream consumers) to drop/recreate vw_approach_patterns and vw_guardrail_decisions around the ai_usage_operational_records rebuild — SQLite validates all views at ALTER TABLE RENAME (the migration-118 lesson), and both views can dangle on upgrade-path DBs. Identical recreate DDL to 118. DBs that already applied 137 are unaffected (applied migrations never re-run); aborted upgrades re-run the fixed file. -->

<!-- Last reviewed 2026-07-04 — WO-AI-SPINE (a0cdd612): Migration 139 (139_ai_spine_collapse.sql) drops decision_records, outcome_records, dashboard_attention_items — every writer in core/telemetry/emitters.py already dual-wrote an execution_events row describing the same fact (0/2/0 production rows). No FK, no view referenced any of the three. Readers repointed to derive decisions/outcomes/attention from execution_events filtered by event_type + outcome_status (core/telemetry/read_models.py::_DECISION_EVENTS_VIEW_SQL / _OUTCOME_EVENTS_VIEW_SQL / _ATTENTION_EVENTS_VIEW_SQL). studio.db 65 → 62 tables. .released_version bumped 138 → 139. -->

<!-- Last reviewed 2026-07-04 — WO dff23cb0-950f-4607-bb30-e1a353a6f8ba (operator-approved pre-squash removal): Migration 140 (140_drop_derived_state_survivors.sql) drops findings_current_status (15 rows) and sum_skill_summary (2 rows) — both pure derived state duplicating their source tables (security_events and raw_skill_telemetry respectively), computed by a projection/rollup writer with no independent signal of its own. findings_current_status: FindingsProjection.fold_spine() (core/projections/findings_projection.py, deleted) was the sole writer; every reader repointed to core/findings/current_status.py::FINDINGS_CURRENT_STATUS_SQL, a SQL fragment reproducing the identical current_status derivation (latest finding.status_changed/finding.resolved event body per finding, else 'open') computed fresh on every query — no persisted table, no replacement table. vw_security_summary (migration 112 view) is the one live SQLite VIEW that referenced findings_current_status; rebuilt in this migration to read security_events directly via the same derivation, using the migration-118 drop/recreate-view guard pattern (DROP VIEW before DROP TABLE, CREATE VIEW after). sum_skill_summary: rebuild_summaries() (core/event_store/studio_db.py, deleted) was the sole writer; get_skill_summaries() now runs the identical GROUP BY aggregation live against raw_skill_telemetry via the effective_skill_runs view. No FK references either table from any other table (grepped core/event_store/migrations/ for "REFERENCES findings_current_status(" / "REFERENCES sum_skill_summary(" — no hits) — no table-reconstruction rebuild needed (contrast migration 137's ai_usage_operational_records rebuild for the token_usage_records FK). studio.db 62 → 60 tables. .released_version bumped 139 → 140. -->

<!-- Last reviewed 2026-07-04 — WO 9f47a1a0-11db-4fcb-9d93-d350fd9a5a6f (operator-approved pre-squash removal): Migration 141 (141_drop_orphaned_workflow_raw_tables.sql) drops raw_workflow_runs (2 rows) and raw_workflow_nodes (25 rows), both write-orphaned since 2026-05-18. Root cause: archive_workflow() (core/event_store/studio_db.py, deleted) inserted into these tables inside the same best-effort try/except block that also emitted workflow.completed/workflow.node.completed canonical events; a raw-table INSERT failure silently aborted the whole write, and archive_workflow() always swallowed the exception and returned False. Fix: control/execution/workflow/state.py no longer calls archive_workflow() — its replacement, _emit_canonical_workflow_events(), writes workflow.completed (+ one workflow.node.completed per node) canonical event envelopes straight to the spool (emitters/shared/spool_writer.py), so a legacy raw-table schema failure can never again silently swallow event emission; _emit_workflow_telemetry()'s execution_events dual-write (core/telemetry/emitters.py::emit_workflow_invocation) moved into state.py's new _emit_execution_events_telemetry() helper, unchanged. Both event types were already present in canonical/events/types.py and config/event_type_registry.py (AI-only routing) — no registry changes needed for them; a separate workflow.invocation_recorded raw-only entry was added because this fix makes emit_workflow_invocation's execution_events dual-write (and its automatic canonical-spool mirror via record_execution_event) fire reliably for the first time, surfacing a pre-existing unregistered-event-type warning in the spool ingestor. last_run()/run_count() (studio_db.py) rewritten to read ai_canonical_events filtered by json_extract(payload, '$.workflow'), preserving their return shapes for control/execution/workflow/registry.py::list_workflows(). No FK/view referenced either dropped table — the sole FK is the internal raw_workflow_nodes → raw_workflow_runs relationship (both dropped together, no table-reconstruction rebuild needed). studio.db 60 → 58 tables. .released_version bumped 140 → 141. -->

<!-- Last reviewed 2026-07-03 — WO 468ce225-795d-402f-af91-acf69ec78099 (operator duplication review): Migration 138 (138_drop_raw_token_usage.sql) drops raw_token_usage — a private direct-write telemetry table (session_id, project_id, skill_name, input_tokens, output_tokens, model, recorded_at) fed only by one-time migration/backfill tooling. Only 3 stale rows in the operator's authority, no accounting fields at all (unlike token_usage_records), and — verified by grepping every migration file for CREATE VIEW bodies and REFERENCES clauses — no view or FK references raw_token_usage anywhere in the chain, so this is a plain two-statement drop (DROP INDEX ×3 + DROP TABLE IF EXISTS) with no table-reconstruction rebuild, unlike migration 137's FK-driven ai_usage_operational_records rebuild. Superseded by canonical token.consumed events and the DuckDB aggregate_metrics.db token_usage_records view (same view migration 137 widened). Writers removed in the same change set: core/event_store/studio_db.py::insert_token_usage (get_token_summary, the sole reader, had zero production callers and was deleted alongside it); interfaces/cli/migrate_to_db.py::_ingest_token_log (the call site in main() and the now-unused has_sentinel import removed too); interfaces/cli/backfill_token_sessions.py::backfill_token_usage (backfill_sessions() — raw_sessions backfill — is now this script's sole purpose; the --sessions CLI flag was removed since that path is no longer optional). Two production callers invoked backfill_token_usage() directly and would otherwise have silently no-op'd through their existing try/except wrappers: interfaces/cli/setup.py::step_analytics_bootstrap and interfaces/cli/ds_dashboard.py::harvest_existing_data were both repointed to call backfill_sessions() instead, preserving first-run token-log.md harvesting behavior. schema_tombstones_data.py and schema_keeplist_data.py (raw_token_usage KEEP → DROP) updated in the same change set. studio.db: 75 -> 74 tables. .released_version bumped 137 -> 138. -->
...
<!-- Last reviewed 2026-07-20 — WO-FILESDB-REVET (migration 150): DROP TABLE raw_runtime_state — a ds_config duplicate (identical key/value/updated_at schema). Forward migration after 149; additive-safe (no other table references it). Repointed core/runtime_state.py to store runtime.* singletons in ds_config. .released_version unchanged until activate. -->

<!-- Last reviewed 2026-07-20 — WO-FILESDB-REVET (migration 151): DROP TABLE raw_session_token_accumulators — a derived-noise duplicate of the per-tool token.consumed events. Forward migration after 150; additive-safe. Retired emitter.normalize_stop + token_capture._update_session_accumulator + core/telemetry/session_accumulator.py; session totals derive via the DuckDB token_usage_records view. .released_version unchanged until activate. -->
<!-- Last reviewed 2026-07-20 — WO-FILESDB-C1 (migration 152): rebuilds business_work_order_artifacts (mig 144, unreleased) — adds instance_key (PK becomes work_order_id, kind, instance_key) for multi-instance artifacts (eval stages) and extends the kind CHECK (operator_decision, decision_request, escalation, report, eval) for the C2-C6 disk->table sweep. Create-copy-drop-rename rebuild; rows migrate with instance_key=''; additive, no new table. .released_version unchanged until activate. -->

<!-- Last reviewed 2026-07-20 — WO-FILESDB-RELEASE (ac814dc3): .released_version bumped 143 -> 152, activating the vetted Files-in-Database migration set on live authority DBs (bootstrap auto-applies up to released_version on the next `ds` run). This makes C2's context + review_verdict persist to business_work_order_artifacts (instance_key from 152) on the live DB instead of the transition .planning disk fallback. Vetted set: 144+152 (artifact table + instance_key/kinds), 145+146 (create raw_* that 150+151 immediately drop, net-zero), 147/148/149 (dead-table drops), 150/151 (revet drops). Reconciled clean against a copy of the live authority DB -> schema_version 152, no data loss. Migration-class: 3-platform matrix watch mandatory. -->

<!-- Last reviewed 2026-07-23 — WO-FILESDB-C4B S4 RELEASE: .released_version bumped 152 -> 153 via `ds migrate activate --confirm`, activating migration 153 (raw_operational_snapshots.report_body) on the live authority DB (auto-backup taken first; ADD COLUMN applied; live schema_version 152 -> 153). The full pulse body is now captured in the authority on the live DB, not just fresh-install/CI. This precedes C4B-5 so the disk pulse-<date>.md write can be dropped without a capture gap (the body lands in the DB before disk goes away). Only migration 153 was pending; no other migration activated. Migration-class: 3-platform matrix watch mandatory. -->

<!-- Last reviewed 2026-07-23 — WO-FILESDB-C4B S4 (migration 153): ALTER TABLE raw_operational_snapshots ADD COLUMN report_body TEXT — additive, non-destructive plain ADD COLUMN (no rebuild; no CHECK/PK change), existing rows keep report_body=NULL. Captures the FULL pulse markdown body in the authority (the disk meta/pulse-<date>.md write is dropped in C4B-5); raw_operational_snapshots previously stored only the derived counts. raw_operational_snapshots is a REAL live table (baseline 142). .released_version NOT bumped (stays 152): 153 is unreleased, skipped on the live authority DB (WO-MS live-safety gate) until `ds migrate activate`; fresh installs + CI apply it on next bootstrap. insert_operational_snapshot (core/event_store/event_writer_buffer.py) feature-detects the column via PRAGMA table_info, so the live DB keeps writing snapshots without the body while 153 is dormant. Migration-class: 3-platform matrix watch mandatory. -->


<!-- Last reviewed 2026-08-02 — WO 336b9502 / R5 (migration 154): create-copy-drop-rename rebuild of business_work_order_artifacts extending the kind CHECK with 'impact_affirmation' (the change_impact_affirmed close gate's record). Additive, mirrors 152; existing rows preserved (instance_key kept). Release-guarded. It is the FIRST migration ≥154 under R3's rollback-pairing regime, so it ships a paired reverse at core/event_store/migrations/rollback/154_*.sql (restores the pre-154 kind allowlist; impact_affirmation rows dropped on rollback — advisory ceremony records, not authority). -->

<!-- Last reviewed 2026-08-05 — WO 6a1722d9 (RELEASE, .released_version sentinel bump 153 → 154): activating migration 154 on the live authority DB. R5 (#575) shipped migration 154 AND the calendar-active change_impact_affirmed close gate that requires its impact_affirmation kind, but omitted the sentinel bump — so 154 stayed unreleased (released_version=153), the kind was absent from the live business_work_order_artifacts CHECK, and every WO created on/after the 2026-08-02 cutover failed `ds work-order affirm-impact` with a raw CHECK-constraint error, blocking close. This bump activates the merged, CI-tested, additive+reversible migration via the normal runner path (timestamped auto-backup taken before first live apply; create-copy-drop-rename rebuild; existing rows preserved), making the gate satisfiable as designed. Only migration 154 was pending; no other migration activated. Defense-in-depth: set_wo_artifact (core/work_orders/artifacts.py) now also catches sqlite3.IntegrityError (a stale kind CHECK) → no-op False, so a future gate/migration timing gap degrades gracefully instead of raising. Migration-class: 3-platform matrix watch mandatory. -->

<!-- Last reviewed 2026-08-08 — WO-CLIENT-SCHEMA (migration 155): adds the client layer — CREATE TABLE business_clients + ALTER business_projects ADD COLUMN client_id (nullable FK) + CREATE INDEX + INSERT OR IGNORE three seed clients (SeayInsights/Fulcrum/Hypershift). Purely additive (no create-copy-drop, no DROP), so the forward migration carries no DROP-safety marker; it ships a paired reverse rollback/155_client_layer.sql (drops index, column, table) per the ROLLBACK_ENFORCED_FROM=154 regime. Existing project rows are classified into clients by core/clients/backfill.py at activation (fulcrum/hypershift by name-or-path, else SeayInsights); fresh installs have no projects. .released_version NOT bumped (stays 154) — 155 is unreleased, skipped on the live authority DB until `ds migrate activate`; fresh installs + CI apply it on next bootstrap. Migration-class: 3-platform matrix watch mandatory. -->

<!-- Last reviewed 2026-08-08 — WO-CLIENT-ENGINE: activate_pending_migrations (core/config/sqlite_bootstrap.py) now runs a post-activation, event-sourced client backfill: after applying pending migrations + bumping .released_version, it calls core/clients/backfill.py::backfill_project_clients, which emits a project.client_assigned event per NULL-client project (classified fulcrum/hypershift by name-or-path, else SeayInsights). Idempotent + best-effort (wrapped so a backfill error never blocks an activation); no-op on fresh installs (no projects). This is how activating migration 155 populates business_projects.client_id via the projection path rather than a direct write. No new migration in this WO. -->

<!-- Last reviewed 2026-09-17 — migration 156 (RELEASE): .released_version 155 -> 156 activates 156_ai_events_project_attribution.sql (Attribution Coherence Phase 3). ADDITIVE ONLY: one nullable column (ai_canonical_events.project_id) plus idx_ace_project_id and idx_ace_project_event_type. No table dropped, no row rewritten, no reader shape changed. Rationale: business_canonical_events has always carried project_id and spool/ingestor.py's _extract_ids already resolved one for every envelope, but ai_canonical_events had no column to receive it, so the INSERT discarded it — 0 of 68,433 token.consumed events carried a project, and every cost/model-mix/skill rollup aggregated all clients into one number. spool/ingestor.py now persists it, declares the column in its own CREATE TABLE, and ALTERs it onto a pre-156 table (the ingestor runs against databases of any schema age; without the self-heal the INSERT fails and it drops every event). emitters/claude_code/token_transcript.py resolves the project from the cwd marker instead of get_active_project_id(), with no fall-back to the active project. Historical attribution is backfilled by core/telemetry/project_backfill.py from the Claude Code transcript directory layout, NOT from raw_claude_code_events.project_id, which records the active project and misattributed $19,405 before a spot-check caught it. Paired reverse at core/event_store/migrations/rollback/156_ai_events_project_attribution.sql. Live result: 66,750 events attributed, 99.7% of token events resolvable. Migration-class change — matrix watch required. -->

<!-- Last reviewed 2026-09-18 - lesson terminus: NO MIGRATION ADDED, deliberately. The new 'applied' end state for raw_lessons is a new value in an existing TEXT status column that carries no CHECK constraint, and the arrival ref reuses the existing promoted_to column, so there is no DDL to write. Recorded here because the docs-drift gate flags any core/event_store/studio_db.py change into the sqlite_schema_authority domain, and a reader arriving from that side should see that the schema was examined and found to need nothing, rather than that a migration was forgotten. .released_version stays 155; no sentinel bump, no rollback file, not a matrix-class change. -->

<!-- Last reviewed 2026-09-23 - WORK ORDER PRIORITY (migration 157): adds `business_work_orders.priority` TEXT NOT NULL DEFAULT 'normal' with CHECK (priority IN ('blocker','defect','normal','backlog')) plus idx_work_orders_priority_created. Purely additive (ADD COLUMN + CREATE INDEX), so the forward migration carries no DROP-safety marker; it ships a paired reverse rollback/157_work_order_priority.sql (drops the index, then the column) per the ROLLBACK_ENFORCED_FROM=154 regime. WHY: `ds work-order next` ordered by created_at ASC alone, so the queue was FIFO -- a defect a review lane registers queued behind every older work order, and a red `main` behind a year of backlog. Three agents write to this queue (review lanes pre-push, the watcher post-push, the session building), and the policy that defects come first existed only as a rule an agent had to remember. MEASURED: the projection inserts OR IGNORE so a duplicate created event is a no-op, and SQLite's OR IGNORE also SKIPS a constraint violation -- an undeclared priority produced NO ROW, no error, no trace. So create_work_order refuses an undeclared level (naming the valid set) and WorkOrderProjection coerces an unknown one to the default rather than losing the row. .released_version NOT bumped (stays 156) -- 157 is unreleased, skipped on the live authority DB until `ds migrate activate`; fresh installs + CI apply it on next bootstrap. Migration-class: 3-platform matrix watch mandatory. -->
<!-- Reviewed 2026-09-23 -- fix/home-means-home: every default Dream Studio home in core/ and interfaces/ now resolves through core.config.paths.home_dir() -- DREAM_STUDIO_HOME, else ~/.dream-studio, not created by resolving -- instead of each module spelling Path.home() / ".dream-studio" itself. With DREAM_STUDIO_HOME unset nothing moves: the default is the same directory. With it set (as `ds --home` sets it) these defaults now follow it: `ds migrate activate`'s default database (core/config/sqlite_bootstrap.py). The live-authority check and its backup in sqlite_bootstrap deliberately still recognise the operator's real home. No schema, table or command-surface change. -->
<!-- Last reviewed 2026-09-23 -- fix/collectors-read-the-store-they-were-given: NO MIGRATION, NO SCHEMA CHANGE. core/shared_intelligence/usage_accounting.py::_token_accounting_rows (and every other conn/db_path-scoped token_usage_sql()/fetch_token_usage_records() caller: the three collectors, cost_analysis.py::api_equivalent_cost, the metrics/analytics/intelligence API routes, read_models_tokens.py) previously resolved its DuckDB aggregate_metrics.db read via connect_analytics()'s ambient default regardless of which authority's own studio.db it was scoped to. core/analytics/duckdb_store.py gained analytics_db_path_for(studio_db_path) and analytics_db_path_for_connection(conn) (PRAGMA database_list off the connection) so each of these now resolves the aggregate_metrics.db sibling of its OWN authority; a caller with no explicit authority keeps resolving the ambient default unchanged. connect_analytics(read_only=False) remains restricted to core/projections/runner.py -- this change is entirely on the read_only=True side. -->

<!-- Last reviewed 2026-09-23 - WORK ORDER PHASES RUN IN ORDER (migration 158): adds `business_work_orders.blocked_from_status` TEXT NULL with CHECK (NULL or one of 'created','in_progress','in_review','pushed','ci_issues'). Purely additive (ADD COLUMN), so the forward migration carries no DROP-safety marker; it ships a paired reverse rollback/158_work_order_blocked_from_status.sql (drops the column) per the ROLLBACK_ENFORCED_FROM=154 regime. WHY: work orders now move created -> in_progress -> in_review -> pushed -> ci_issues -> closed, only ci_issues skippable, enforced by one table (core.work_orders.task_status.WORK_ORDER_TRANSITIONS) that start, advance, close, block, unblock and reopen all consult; close refuses any phase but pushed/ci_issues and --force does not waive it. Blocking is allowed from any open phase, and unblock sent every work order to in_progress -- one blocked while pushed came back with its review and push erased, invisible to the watcher that looks for pushed work. Block now records the phase here and on the blocked event as `from_status`; unblock returns to it and says so as `to_status`, which WorkOrderProjection reads first on replay. Writers and the projection check the column exists, so an authority without 158 unblocks to in_progress as before. .released_version NOT bumped (stays 156) -- 158 is unreleased, skipped on the live authority DB until `ds migrate activate`; fresh installs + CI apply it on next bootstrap. Migration-class: 3-platform matrix watch mandatory. -->

<!-- Last reviewed 2026-09-24 - migration-release-158: bump .released_version 156→158. Migrations 157 (work order priority) and 158 (work order phases run in order), both merged to main since the last release, are now released for live application on fresh installs and `ds migrate activate`. No new schema changes in this commit -- release marker only. -->
