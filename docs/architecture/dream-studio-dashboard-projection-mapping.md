...
<!-- Last reviewed 2026-05-21 — Platform profile is a local-state input, not a dashboard projection source. `ds doctor` output may surface platform details (OS, shell, Python version) as part of the installer health display; this is diagnostic text, not a structured projection domain. The dashboard mapping does not need a new domain for platform.json. -->
...
<!-- Last reviewed 2026-05-22 — TA0c retires activity_log read sites in projection routes. Three dashboard-facing route files changed: `projections/api/routes/audits.py` — removed `LEFT JOIN activity_log` from `GET /audits/runs` and `GET /audits/runs/{id}`; `event_timestamp` now derives from `ar.started_at`, severity is literal `'info'`, anomaly fields are literal `0`/`0.0`. `projections/api/routes/hooks.py` — collapsed the conditional `activity_log` join in `GET /hooks/executions` to always use literal fallback values; removed `LEFT JOIN activity_log` from `GET /hooks/executions/{id}`; `GET /hooks/anomalies` now returns empty results (anomaly detection retired — `is_anomaly` was always `False` per studio_db.py). `projections/api/routes/intelligence.py` — already migrated in prior commit. These are projection behavior changes (derived view field sources changed from activity_log to audit_runs/hook_executions own columns or literals); no primary authority change; dashboard consumers of these routes receive the same field names with honest empty-state values where anomaly data was previously stub data. -->
...
<!-- Last reviewed 2026-05-22 — Phase 18.1.5: business_work_orders added as a new L3 projection table populated by WorkOrderProjection from business_canonical_events. Dashboard consumers may read this table as a derived view with projection_state.last_run_at as the freshness signal. The ProjectionEngine owns the write path; dashboard routes must not write to L3 tables directly. No new dashboard projection domain required at this time; existing Work Order Status mapping covers business_work_orders. -->
...
<!-- 2026-06-06: Wave 6 — 13 verified-dead tables dropped (migration 101). dropped tables removed from shared-intelligence surface source_tables declarations; no projection-mapping contract change. No semantic change required. -->
