...
<!-- Reviewed 2026-09-23 — fix/ds-launcher-picks-a-python-that-runs: the Windows launchers (ds.cmd, ds.ps1, and the user-local ones `ds install-command` writes) now use the first Python that actually runs, ordered DS_PYTHON, the installing interpreter (generated launchers only), python, py -3, python3 -- they previously asked `py` first and trusted a name without running it. Operator guidance is in troubleshooting.md. No change to the adapter runtime, projection model, command surface or state tiers. -->
<!-- Reviewed 2026-09-23 -- fix/home-means-home: every default Dream Studio home in core/ and interfaces/ now resolves through core.config.paths.home_dir() -- DREAM_STUDIO_HOME, else ~/.dream-studio, not created by resolving -- instead of each module spelling Path.home() / ".dream-studio" itself. With DREAM_STUDIO_HOME unset nothing moves: the default is the same directory. With it set (as `ds --home` sets it) these defaults now follow it: the installed runtime's default home (core/installed_runtime.py). The live-authority check and its backup in sqlite_bootstrap deliberately still recognise the operator's real home. No schema, table or command-surface change. -->

<!-- Reviewed 2026-09-24 -- feat/mcp-server: the `mcp` adapter slot adapter-projections/mcp/server-policy.json already declared (adapter_role: projection, authority.source: dream_studio_sqlite) is now backed by a real runtime -- `ds mcp serve` (integrations/mcp/, docs/contracts/mcp-server-contract.md). It follows the router-preference rule stated above: every tool call is a read against the same core.* query functions the CLI itself calls, never a second path into SQLite, and the registry is curated to read-only paths -- no tool in it mutates authority state. Network exposure (binding beyond 127.0.0.1) is opt-in and does not relax the bearer-token auth every /mcp request requires. No new authority table, no change to which store owns source of truth. -->
