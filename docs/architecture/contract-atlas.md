...
<!-- Last reviewed 2026-06-27 — Wave 1 substrate drop (migration 130): removed artifact_records from telemetry module owned_tables in core/module_contracts.py (table dropped; 0 rows, no production writer). Removed artifact_records from expert_workflows.py AUTHORITY_WRITE_TARGETS and from platform_hardening.py normalization_targets. Removed artifact_records/authority_projection_records from scoped_agents.py result_normalization_targets. No claim-boundary change — artifact_records was aspirational telemetry never populated in production. -->
...
<!-- Last reviewed 2026-06-27 — Wave 2 substrate realignment (migration 131, worktree-agent-a910d590fedb5c672): migration 131 retires 24 dormant feature tables (writers existed but no live entry point ever invoked them). core/shared_intelligence/contract_atlas.py: the adapter_usage_accounting block no longer emits a task_attribution key (task_attribution_records dropped); the task_attribution_model / github_repo_intake / platform-hardening atlas sections now reflect retired source tables. projections/api/routes/shared_intelligence.py: the dormant shared-intelligence write routes (capability/policy still live) no longer back adapter_result_records, shared_context_packets, learning_event_records, hardening_candidate_records, model_provider_profiles, artifact_authority_records, skill_evaluation_runs, connector_ingestion_runs, github_repo_evaluations/adoption_decisions, task_attribution_records — all dropped. No claim/authority-boundary change: these tables were aspirational, never populated in production. -->

<!-- Last reviewed 2026-06-28 — Batch 1 canonical-first migration (migration 133, worktree-agent-a5a922af244c65bd7): drops compliance_review_flags, release_readiness_records, policy_decision_records, guard_events (all DEAD — no live production writer reachable). contract_atlas.py: commented out release_readiness_records and policy_decision_records from _source_tables() — they no longer exist post-migration 133. Atlas sections for security/production-readiness now reflect these tables as retired. -->

<!-- Last reviewed 2026-07-04 — migration 139 (WO-AI-SPINE, AD-5): core/module_contracts.py telemetry module owned_tables list updated — outcome_records removed (dropped migration 139; 0/2/0 production rows across decision_records/outcome_records/dashboard_attention_items; their writers in core/telemetry/emitters.py already dual-wrote execution_events, so the per-type tables were pure duplication). No contract-atlas claim, authority boundary, or public-export shape change: this is a source-table-list correction, not a new capability. -->

<!-- Last reviewed 2026-07-04 — migration 140 (WO dff23cb0-950f-4607-bb30-e1a353a6f8ba): contract_atlas.py's _source_tables() no longer adds findings_current_status (dropped migration 140 — pure derived state, FindingsProjection.fold_spine() upsert over security_events, 15 rows); security_events remains. DASHBOARD_MODULES' security_analytics entry (core/telemetry/execution_spine.py) drops findings_current_status from owns_tables/source_tables, keeping security_events — the "table:security_analytics reads_source_table table:security_events" confirmed-dependency-graph edge is unchanged, the "table:findings_current_status" edge is gone. No contract-atlas claim, authority boundary, or public-export shape change: security findings are still fully queryable, now derived from security_events at read time (core/findings/current_status.py) instead of a stored projection. -->
...
<!-- Last reviewed 2026-09-22 — three skill-surface domains widened, no new domains and no boundary change (same class as the 2026-05-29 sqlite_bootstrap widening): projects_engine_skill_surface, milestones_engine_skill_surface and work_orders_engine_skill_surface previously matched only `core/<x>/**`, so the CLI half of each surface was uncovered. An agent calls the CLI, not the engine, so a new command could ship with its skill never mentioning it and this gate stay silent — which is how `ds project onboard` nearly merged undocumented. Added `interfaces/cli/commands/project.py`, `interfaces/cli/commands/milestone.py` and the glob `interfaces/cli/commands/work_order*.py` (a glob because that CLI is five sibling modules; same precedent as `shared_intelligence_adapters`' `adapter_*.py`). Measured over 200 commits before widening: those modules changed 23 times, 7 without touching the matching skill, so this fires on roughly 3.5% of commits. Domain count unchanged; contract shapes, release-blocking status and required_doc_refs unchanged. -->

<!-- Last reviewed 2026-09-22 - milestones_engine_skill_surface repointed, no domain added or removed and no boundary change: its contract_refs and required_doc_refs moved from canonical/skills/ds-milestone/SKILL.md to canonical/rules.yml because that pack was deleted. The pack was narration, not a capability - all four of its operations were already `ds milestone create|list|status|close`, and its own SKILL.md opened with "Not a CLI command" while telling the model to call core.milestones.* directly. It had drifted from the engine it described: it documents failure keys `design_audit_required` and `security_audit_required`, and close.py has never emitted either - every failure it produces is an English sentence. The coupling this domain guards is unchanged and still real (an engine change must land against the agent-facing statement of what a milestone close refuses); only the statement's location moved, from a SKILL.md to seven entries in the rule registry, four carrying enforced_by and three declared unenforced with reasons. source_patterns, release_blocking status and domain count are all unchanged. The sibling domains projects_engine_skill_surface and work_orders_engine_skill_surface still point at their packs and are untouched here. -->

<!-- Last reviewed 2026-09-22 - projects_engine_skill_surface repointed, no domain added or removed and no boundary change: its contract_refs and required_doc_refs moved from canonical/skills/ds-project/SKILL.md to canonical/rules.yml because that pack was dissolved. Every operation it narrated was already a command - `ds project list|set-active|deactivate|delete|register|state`, `ds design-brief show|create|lock|update`, and the three brownfield doors added in #785 - so what remained was an interview: how to ask, what not to write down, when to stop. That moved to the registry as eight rules (one enforced: a cascade delete previews its dependents; seven guidance). The coupling this domain guards is unchanged and still real - a projects-engine change must land against the agent-facing statement of how projects behave - and that statement is now the rules. source_patterns, release_blocking status and domain count are all unchanged. The sibling domain milestones_engine_skill_surface was repointed the same way when ds-milestone went; work_orders_engine_skill_surface still points at its pack. -->
<!-- Last reviewed 2026-09-22 - work_orders_engine_skill_surface repointed, no domain added or removed and no boundary change: its contract_refs (six skill files) and required_doc_refs moved to canonical/rules.yml because the ds-workorder pack was dissolved, the last of the three lifecycle packs. Every one of its five modes was narration over a command that already existed - `ds work-order start|close|block|task-done|tasks` - and TEN of its twenty numbered rules were already registry entries under other names: "read functions before you write", "present returned dicts" and "errors are operator-visible" are surfaced-state-is-what-the-call-returned; "mutations require explicit user confirmation" is a-destructive-action-needs-an-unambiguous-yes; the attribution guard is an-ambiguous-reference-stops-and-asks plus work-order-declares-module-boundary; the separate-runner instruction is tests-run-by-a-different-agent; the verdict-scope rule is a-verdict-declares-what-it-did-not-execute; carried work is carried-work-is-exempt-by-record-never-by-status. Each was a second copy free to drift, which is the argument for the dissolution stated by the pack itself. Six more statements moved in as rules carrying the tests that enforce them: an unrecognised gate name fails closed, a work order closes only over finished tasks, a task nobody can check cannot be filed, a failed completion is not a completion, a work-order type is refused at the door, and (guidance) a context artifact leads with the prompt chain. Two guarantees moved DOWN rather than sideways: `ds work-order close` now prints the main-CI advisory to stderr itself instead of instructing a model to print it verbatim, and the brief-currency explanation is in the gate's own verdict text, naming the UI types from the set the gate tests. source_patterns, release_blocking status and domain count are all unchanged. All three lifecycle domains - milestones, projects and work orders - now point at the registry; no skill-surface domain names a lifecycle pack, because none exists. -->
<!-- Reviewed 2026-09-23 -- fix/home-means-home: every default Dream Studio home in core/ and interfaces/ now resolves through core.config.paths.home_dir() -- DREAM_STUDIO_HOME, else ~/.dream-studio, not created by resolving -- instead of each module spelling Path.home() / ".dream-studio" itself. With DREAM_STUDIO_HOME unset nothing moves: the default is the same directory. With it set (as `ds --home` sets it) these defaults now follow it: the installed-runtime model the Contract Atlas describes is built from the home in force (core/shared_intelligence/contract_atlas_main.py). The live-authority check and its backup in sqlite_bootstrap deliberately still recognise the operator's real home. No schema, table or command-surface change. -->
<!-- Reviewed 2026-09-23 — chore/run-docs-drift-before-push: core/shared_intelligence/contract_registry_report.py's `_matches` (used by the contract_atlas domain's own source/doc-hit matching, PRIVATE_ARTIFACT_PATTERNS and PUBLICATION_RISK_PATTERNS) now also tries a pattern with its leading "**/" stripped, so a match against a root-level file (fnmatch has no real globstar: "**/*.db" never matched a bare "secrets.db") is no longer missed. Matching-mechanism bug fix only: no domain added or removed, no source_patterns/required_doc_refs/release_blocking change, and no existing claim in this doc described the old (buggy) behavior for this to contradict. -->
