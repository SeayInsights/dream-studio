...
<!-- Reviewed 2026-09-26 - pack-split (eighth slice, first half): quality's `database-compliance`
mode merged into the EXISTING security pack's `comply` mode as a new `privacy` sub-mode
(canonical/skills/security/modes/comply/privacy/{SKILL.md,rules.yml,config.yml}), rather than a
directory move -- this is a content merge, not a relocation, since comply already had independent
content (map/gaps/evidence: mapping a CLIENT's external SARIF scan findings to compliance
framework controls) that database-compliance's in-repo GDPR/HIPAA/CCPA detector does not overlap
with; the two are pipeline-adjacent peers, not duplicates. `core/skills/audit/rules_scanner_shared.py`'s
`_skill_dir()` needed a new `_NESTED_SKILL_DIRS` alias map for this move -- its existing
one-level-deep glob (`canonical/skills/*/modes/<skill_id>`) cannot find rules.yml nested two
levels down under an existing sibling mode, and would have silently returned 0 findings with only
a log warning (a new regression-guard test added: test_audit_dbc_rules_yml_actually_resolves,
mutation-verified to fail without the alias). No directory-tree layout change -- security/ already
existed. This is the first of two content-merge slices (quality:security -> security:review is
