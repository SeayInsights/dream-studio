...
def validate_test_file(file_path: Path) -> dict:
    ...
    issues = []
    stats = {"test_classes": 0, "test_functions": 0, "fixtures": 0, "has_docstring": False}
...
def main():
    ...
    test_files = ["test_reports.py", "test_exporters.py", "test_email.py", "test_scheduler.py"]
    ...
    all_issues = []
    total_stats = {"files": 0, "test_classes": 0, "test_functions": 0, "fixtures": 0}
    ...
    if all_issues:
        ...
        if total_stats["test_functions"] >= expected_min_tests:
            ...
            print(
                f"WARNING: Test count ({total_stats['test_functions']}) may be below coverage target ({expected_min_tests}+)"
            )
