...
from projections.api.websocket.connection_manager import ConnectionManager
from projections.core.streaming.metric_streamer import MetricStreamer
...
def test_websocket_subscribe_message():
    ...
    with TestClient(app) as client:
        with client.websocket_connect("/api/v1/stream/metrics") as websocket:
            ...
            websocket.send_json({"type": "subscribe", "metrics": ["sessions", "tokens"]})
...
def test_websocket_unsubscribe_message():
    ...
    with TestClient(app) as client:
        with client.websocket_connect("/api/v1/stream/metrics") as websocket:
            ...
            # Unsubscribe
            websocket.send_json({"type": "unsubscribe", "metrics": ["sessions"]})
...
def test_websocket_invalid_subscribe_message():
    ...
    with TestClient(app) as client:
        with client.websocket_connect("/api/v1/stream/metrics") as websocket:
            ...
            websocket.send_json({"type": "subscribe", "metrics": []})
...
def test_websocket_invalid_unsubscribe_message():
    ...
    with TestClient(app) as client:
        with client.websocket_connect("/api/v1/stream/metrics") as websocket:
            ...
            websocket.send_json({"type": "unsubscribe", "metrics": "not-a-list"})
...
async def test_metric_streamer_init():
    ...
    manager = ConnectionManager()
    streamer = MetricStreamer(connection_manager=manager, poll_interval=30, db_path=":memory:")
...
async def test_metric_streamer_start():
    ...
    manager = ConnectionManager()
    streamer = MetricStreamer(connection_manager=manager, poll_interval=1, db_path=":memory:")
...
async def test_metric_streamer_poll_loop():
    ...
    streamer = MetricStreamer(
        connection_manager=manager, poll_interval=0.1, db_path=":memory:"  # Fast poll for testing
    )
...
def test_multiple_clients_different_subscriptions():
    ...
    with TestClient(app) as client:
        ...
        with client.websocket_connect("/api/v1/stream/metrics") as ws1:
            ...
            ws1.send_json({"type": "subscribe", "metrics": ["sessions"]})
            ...
            with client.websocket_connect("/api/v1/stream/metrics") as ws2:
                ...
                ws2.send_json({"type": "subscribe", "metrics": ["tokens", "skills"]})
