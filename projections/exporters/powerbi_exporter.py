...
class PowerBIExporter:
    ...
    def export_dataset(
        self, report_data: Dict[str, Any], output_path: Union[str, Path]
    ) -> Tuple[bool, str]:
        """
    ...
    def create_data_model(self, report_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Tokens table
        tokens_data = self._extract_section_data(
            section_data, ["Tokens", "Token Usage", "Cost Analysis"]
        )
        if tokens_data:
        ...
        # Workflows table
        workflows_data = self._extract_section_data(
            section_data, ["Workflows", "Workflow Performance"]
        )
        if workflows_data:
    ...
    def _extract_section_data(
        self, section_data: Dict[str, Dict[str, Any]], possible_titles: List[str]
    ...
    def _build_models_table(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(model_list, list) and model_list:
            ...
            # Fallback
            rows.append(
                {"model_name": "all", "usage_count": data.get("total_usage", 0), "total_tokens": 0}
            )
    ...
    def _export_csv_files(self, tables: List[Dict[str, Any]], data_dir: Path) -> List[str]:
        """
        ...
        for table in tables:
            ...
            with open(file_path, "w", newline="", encoding="utf-8-sig") as f:
                writer = csv.DictWriter(
                    f, fieldnames=[col["name"] for col in columns], quoting=csv.QUOTE_MINIMAL
                )
    ...
    def _generate_pbids_file(self, data_dir: Path) -> str:
        ...
        pbids = {
            ...
            "connections": [
                {
                    "details": {"protocol": "file", "address": {"path": abs_data_path}},
    ...
    def _generate_readme(self, csv_files: List[str], data_model: Dict[str, Any]) -> str:
