...
class PPTXTemplateLoader:
    ...
    def apply_template(self, prs: Presentation, template_name: str) -> Presentation:
        """
    ...
    def create_executive_template(self) -> Presentation:
        ...
        self._add_title_slide(slide, "Dream-Studio Analytics Report", "[Report Period]")
    ...
    def create_technical_template(self) -> Presentation:
        ...
        slides_content = [
            ...
            ("Lessons Learned", "content", ["Key insight 1", "Key insight 2", "Key insight 3"]),
        ...
        for idx, (title, slide_type, content) in enumerate(slides_content, 1):
            ...
            if slide_type == "title":
                self._add_title_slide(
                    slide, "Dream-Studio Technical Analytics", "[Detailed Analysis Report]"
                )
            elif slide_type == "toc":
    ...
    def _add_title_slide(self, slide, title: str, subtitle: str):
        ...
        line = slide.shapes.add_shape(
            MSO_SHAPE.RECTANGLE, Inches(1), Inches(3.6), Inches(3), Inches(0.05)
        )
        line.fill.solid()
    ...
    def _add_content_slide(self, slide, title: str, bullets: List[str]):
        ...
        line = slide.shapes.add_shape(
            MSO_SHAPE.RECTANGLE, Inches(0.5), Inches(0.95), Inches(2), Inches(0.03)
        )
        line.fill.solid()
    ...
    def _add_chart_slide(self, slide, title: str):
        ...
        chart_placeholder = slide.shapes.add_shape(
            MSO_SHAPE.RECTANGLE, Inches(0.8), Inches(1.3), Inches(5.5), Inches(3.8)
        )
        ...
        # Insights box
        insights_box = slide.shapes.add_textbox(Inches(6.5), Inches(1.3), Inches(3), Inches(3.8))
    ...
    def _add_metrics_slide(self, slide, title: str):
        ...
        positions = [(0.8, 1.5), (5.3, 1.5), (0.8, 3.5), (5.3, 3.5)]

        for (metric_title, value, color), (x, y) in zip(metrics, positions):
            self._add_metric_card(slide, x, y, metric_title, value, color)

    def _add_metric_card(self, slide, x: float, y: float, title: str, value: str, color: RGBColor):
        """Add a single metric card."""
        # Card background
        card = slide.shapes.add_shape(
            MSO_SHAPE.ROUNDED_RECTANGLE, Inches(x), Inches(y), Inches(4), Inches(1.5)
        )
        card.fill.solid()
    ...
    def _add_toc_slide(self, slide):
        ...
        # Two column layout
        left_box = slide.shapes.add_textbox(Inches(0.8), Inches(1.3), Inches(4), Inches(3.8))
        right_box = slide.shapes.add_textbox(Inches(5.2), Inches(1.3), Inches(4), Inches(3.8))
        ...
        for i, section in enumerate(sections[mid:], mid):
            p = (
                right_box.text_frame.add_paragraph()
                if i > mid
                else right_box.text_frame.paragraphs[0]
            )
            p.text = f"{i+1}. {section}"
            p.font.size = Pt(16)
            p.font.color.rgb = self.colors["dark"]
    ...
    def _add_qa_slide(self, slide):
        ...
        # Q&A text
        qa_box = slide.shapes.add_textbox(Inches(2), Inches(1.5), Inches(6), Inches(1.5))
    ...
    def _add_footer(self, slide, slide_num: int, total_slides: int):
        ...
        footer_box = slide.shapes.add_textbox(Inches(9), Inches(5.2), Inches(0.8), Inches(0.3))
