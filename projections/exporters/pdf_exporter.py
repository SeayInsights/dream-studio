...
class PDFExporter:
    ...
    def export_to_pdf(self, report_data: Dict[str, Any], output_path: str) -> Tuple[bool, str]:
        """
    ...
    def _build_section(self, section: Dict[str, Any], styles: Any, heading_style: Any) -> List[Any]:
    ...
    def add_charts(self, chart_data: List[Dict[str, Any]]) -> List[Any]:
        ...
        for chart in chart_data:
            ...
            # Add chart title
            elements.append(Paragraph(f"<b>{chart_title}</b> ({chart_type})", styles["Heading3"]))
            ...
            if image_path and os.path.exists(image_path):
                try:
                    ...
                    elements.append(
                        Paragraph(f"[Chart placeholder - {chart_type}]", styles["Italic"])
                    )
            else:
                # Placeholder for chart
                elements.append(
                    Paragraph(f"[Chart: {chart_title} - Image not available]", styles["Italic"])
                )
        ...
        try:
            ...
            for section in report_data.get("sections", []):
                ...
                if charts:
                    ...
                    for chart in charts:
                        text_content.append(
                            f"  - {chart.get('title', 'Chart')} ({chart.get('type', 'unknown')})"
                        )
