...
class ProductionDashboard:
    ...
    def __init__(self, db_path: str | None = None):
        ...
        Args:
            db_path: Explicit local dashboard database path. If omitted, this
                deprecated local generator uses the operator's native runtime
                path for local dashboard output only. It is not an enterprise
                aggregation input surface.
        """
        import os
    ...
    def _collect_metrics(self, days: int) -> Dict[str, Any]:
        ...
        return {name: collector.collect(days=days) for name, collector in collectors.items()}
    ...
    def _generate_ml_insights(self, metrics: Dict[str, Any], days: int) -> Dict[str, Any]:
        """
        Generate ML-powered insights (Enterprise feature)
        ...
        try:
            ...
            if "invocations" in skill_data and len(skill_data["invocations"]) >= 3:
                ...
                if all(
                    col in telemetry.columns for col in ["session_id", "skill_name", "invoked_at"]
                ):
        ...
        try:
            ...
            if (
                "invocations" in skill_data
                and len(skill_data["invocations"]) >= 20
                and "sessions" in session_data
                and len(session_data["sessions"]) >= 20
            ):
        ...
        try:
            ...
            if timeline and len(timeline) >= 14:
                ...
                if token_timeline and len(token_timeline) >= 14:
                    ...
                    token_df = token_df.set_index("date")[["total"]].rename(
    ...
    def _build_html(
        ...
        has_ml_data = any(
            [ml_insights.get("forecasts"), ml_patterns, ml_recommendations, ml_benchmarks]
        )
...
"""
                for benchmark in ml_benchmarks:
                    ...
                    status_emoji = (
                        "✅"
                        if status == "above_baseline"
                        else "[WARNING]" if status == "below_baseline" else "➡️"
                    )
