...
Example usage:
    >>> from ml.recommendations import RecommendationEngine, generate_recommendations
    >>> import pandas as pd
    >>>
    >>> # Generate recommendations from an explicit aggregate input database
    >>> recommendations = generate_recommendations('exports/aggregate_metrics.db')
...
class RecommendationEngine(BaseModel):
    ...
    def fit(self, skill_telemetry: "pd.DataFrame", session_data: "pd.DataFrame") -> None:
        ...
        self.recommendations = sorted(
            self.recommendations, key=lambda x: x["impact_score"], reverse=True
        )
    ...
    def _generate_skill_optimization(self) -> List[Dict]:
        ...
        sequence_patterns = self.pattern_detector.get_patterns(
            pattern_type="sequence", min_confidence=0.6, min_lift=1.5
        )
        ...
        cooccurrence_patterns = self.pattern_detector.get_patterns(
            pattern_type="cooccurrence", min_lift=1.8
        )
        ...
        for pattern in cooccurrence_patterns[:5]:  # Top 5
            ...
            impact = min(
                100, int((confidence * 25) + (min(lift / 4, 1) * 35) + (min(support / 15, 1) * 40))
            )
    ...
    def _generate_workflow_improvements(self) -> List[Dict]:
                ...
                for skill, success_count in skill_success_counts.most_common(10):
                    ...
                    if total_count >= self.min_support:
                        ...
                        if success_rate > overall_success_rate * 1.3:  # 30% better than average
                            impact = min(
                                100, int((success_rate * 50) + (min(success_count / 20, 1) * 50))
                            )
        ...
        temporal_patterns = self.pattern_detector.get_patterns(
            pattern_type="temporal_hour", min_lift=1.5
        )
                ...
                recommendations.append(
                    {
                        ...
                        "impact_score": impact,
                        "actionable": True,
                        "data": {"pattern": pattern_str, "lift": float(lift)},
                    }
                )
    ...
    def _generate_inefficiency_detection(self) -> List[Dict]:
            ...
            if total_failed >= 3:
                ...
                for skill, failure_count in skill_failure_counts.items():
                    ...
                    if total_count >= self.min_support:
                        ...
                        if failure_rate > 0.4:
                            impact = min(
                                100, int((failure_rate * 60) + (min(failure_count / 10, 1) * 40))
                            )
        ...
        if "duration" in self._session_data.columns and "tokens" in self._session_data.columns:
            ...
            session_efficiency["token_efficiency"] = session_efficiency["tokens"] / (
                session_efficiency["duration_minutes"] + 1e-6
            )
    ...
    def _generate_performance_boosters(self) -> List[Dict]:
        ...
        try:
            ...
            best_cluster = (
                max(cluster_info, key=lambda c: c["characteristics"]["avg_success_rate"])
                if cluster_info
                else None
            )
                ...
                recommendations.append(
                    {
                        ...
                        "impact_score": impact,
                        "actionable": True,
                        "data": {"cluster_label": best_label, "characteristics": best_chars},
                    }
                )

            # Compare user's current performance to benchmarks
            if "outcome" in self._session_data.columns:
                overall_success = (self._session_data["outcome"] == "success").mean()
...
def generate_recommendations(
    db_path: str, min_support: int = 3, min_impact: Optional[int] = None
) -> List[Dict]:
    """
