...
Example usage:
    >>> from ml.patterns import PatternDetector, detect_skill_patterns
    >>> import pandas as pd
    >>>
    >>> # Detect patterns from an explicit aggregate input database
    >>> patterns = detect_skill_patterns('exports/aggregate_metrics.db', min_support=3)
    >>> for p in patterns[:5]:
...
class PatternDetector(BaseModel):
    ...
    def _build_temporal_db(self) -> None:
        """Build temporal database (skill, timestamp) pairs."""
        self._temporal_db = [
            (row["skill_name"], row["invoked_at"]) for _, row in self.data.iterrows()
        ]
    ...
    def _detect_sequence_patterns(self) -> List[Dict]:
        ...
        for (skill_a, skill_b, skill_c), count in trigram_counts.items():
            if count >= self.min_support:
                ...
                confidence = (
                    count / bigram_counts[bigram_key] if bigram_counts[bigram_key] > 0 else 0.0
                )
        ...
        for hour, skill_counter in hour_skill_counts.items():
            for skill, count in skill_counter.items():
                if count >= self.min_support:
                    ...
                    prob_skill = (
                        skill_counts[skill] / total_invocations if total_invocations > 0 else 0.0
                    )
    ...
    def _detect_cooccurrence_patterns(self) -> List[Dict]:
        ...
        for (skill_a, skill_b), count in cooccurrence_counts.items():
            if count >= self.min_support:
                ...
                confidence = (
                    count / skill_session_counts[skill_a]
                    if skill_session_counts[skill_a] > 0
                    else 0.0
                )

                # Lift: confidence / P(B in any session)
                prob_b = (
                    skill_session_counts[skill_b] / total_sessions if total_sessions > 0 else 0.0
                )
                lift = confidence / prob_b if prob_b > 0 else 0.0
    ...
    def evaluate(self, data: "pd.DataFrame", target: "np.ndarray") -> dict:
        ...
        return {
            ...
            "avg_support": (
                float(np.mean([p["support"] for p in self.patterns])) if self.patterns else 0.0
            ),
            "avg_confidence": (
                float(np.mean([p["confidence"] for p in self.patterns])) if self.patterns else 0.0
            ),
            "avg_lift": (
                float(np.mean([p["lift"] for p in self.patterns])) if self.patterns else 0.0
            ),
        }
...
def detect_skill_patterns(
    db_path: str, min_support: int = 3, pattern_type: Optional[str] = None
) -> List[Dict]:
    """
...
def detect_workflow_patterns(db_path: str, min_support: int = 2) -> List[Dict]:
    """
