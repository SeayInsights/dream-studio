...
class TimeSeriesForecaster(BaseModel):
    ...
    def _fit_exponential(self) -> None:
        ...
        try:
            self.model = ExponentialSmoothing(values, trend=None, seasonal=None)
            self.model = self.model.fit()
    ...
    def evaluate(self, data: "pd.DataFrame", target: "np.ndarray") -> dict:
        ...
        return {"mae": float(mae), "rmse": float(rmse), "method": self.method}

    def forecast(
        self, periods: int, confidence_levels: List[float] = [0.80, 0.95]
    ) -> Dict[str, List[float]]:
        """
        ...
        forecast_dates = [(last_date + timedelta(days=i + 1)).isoformat() for i in range(periods)]
    ...
    def _forecast_exponential(self, periods: int) -> Dict[str, List[float]]:
        ...
        pred_int = self.model.get_prediction(start=len(self.data), end=len(self.data) + periods - 1)
...
def forecast_token_usage(
    db_path: str, periods: int = 7, method: str = "auto"
) -> Dict[str, List[float]]:
    """
...
def generate_forecast_report(
    db_path: str, periods_short: int = 7, periods_long: int = 30
