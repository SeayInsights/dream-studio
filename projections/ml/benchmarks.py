...
Example usage:
    >>> from ml.benchmarks import BenchmarkEngine, run_benchmark_suite
    >>> import pandas as pd
    >>>
    >>> # Run full benchmark suite from an explicit aggregate input database
    >>> results = run_benchmark_suite('exports/aggregate_metrics.db')
...
class BenchmarkEngine(BaseModel):
    ...
    def _calculate_trend(
        self, metric_name: str, current_value: float, current_period_days: int
    ) -> TrendType:
        """
    ...
    def _get_historical_avg(
        self, metric_name: str, end_date: datetime, days: int
    ) -> Optional[float]:
        """
        ...
        window_data = self.data.loc[
            (self.data.index >= start_date) & (self.data.index < end_date), metric_name
        ].dropna()
...
def run_benchmark_suite(
    db_path: str, current_period_days: int = 7, baselines: Optional[Dict[str, float]] = None
) -> List[Dict]:
    """
    ...
    try:
        ...
        for metric_name, current_value in current_metrics.items():
            if metric_name in historical_df.columns:
                benchmark = engine.benchmark_metric(metric_name, current_value, current_period_days)
