...
class BehaviorClusterer(BaseModel):
    ...
    def _extract_features(self, data: "pd.DataFrame") -> "pd.DataFrame":
        ...
        if "skills_used" in data.columns:
            features["skill_diversity"] = data["skills_used"].apply(
                lambda x: (
                    len(set(x.split(","))) / len(x.split(",")) if isinstance(x, str) and x else 0
                )
            )
        ...
        if "outcome" in data.columns:
            features["success"] = data["outcome"].apply(lambda x: 1 if x == "success" else 0)
        else:
            features["success"] = 0
    ...
    def fit(self, data: "pd.DataFrame") -> None:
        ...
        if SKLEARN_AVAILABLE:
            ...
            self.cluster_labels, self.cluster_centers = self._simple_kmeans(
                scaled_features, self.n_clusters
            )
        ...
        if SKLEARN_AVAILABLE:
            try:
                ...
                if len(unique) > 1 and len(feature_matrix) > 1:
                    scaled_features = (
                        self.scaler.transform(feature_matrix) if self.scaler else feature_matrix
                    )
    ...
    def _simple_kmeans(
        self, features: "np.ndarray", n_clusters: int, max_iters: int = 100
    ...
    def _assign_to_nearest(self, features: "np.ndarray", centers: "np.ndarray") -> "np.ndarray":
        """
