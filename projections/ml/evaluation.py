...
Example usage:
    >>> from ml.evaluation import evaluate_forecast_accuracy, export_evaluation_report
    >>> import numpy as np
    >>>
...
def evaluate_forecast_accuracy(actual: "np.ndarray", predicted: "np.ndarray") -> Dict[str, float]:
    """
    ...
    return {"mae": mae, "rmse": rmse, "mape": mape, "n_samples": int(len(actual))}


def evaluate_pattern_quality(
    detected_patterns: List[Dict], labeled_patterns: List[Dict]
) -> Dict[str, float]:
    """
...
def evaluate_recommendation_impact(
    recommendations: List[Dict], outcomes: List[Dict]
...
def export_evaluation_report(metrics: Dict, filename: Union[str, Path]) -> None:
    """
    ...
    if recommendation_results is not None:
        try:
            ...
            if recommendations is not None and outcomes is not None:
                report["recommendations"] = evaluate_recommendation_impact(
