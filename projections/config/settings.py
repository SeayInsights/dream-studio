...
Provides type-safe configuration loading from projections.yaml with defaults.
    ...
    def validate(self) -> None:
        ...
        for channel in self.notification_channels:
            if channel not in valid_channels:
                raise ValueError(
                    f"Invalid notification channel: {channel}. " f"Must be one of {valid_channels}"
                )
...
class ExportSettings:
    ...
    def __init__(self, config: dict | None = None):
        ...
        self.pdf: Dict[str, str] = config.get(
            "pdf", {"page_size": "letter", "orientation": "portrait"}
        )
        self.excel: Dict[str, Any] = config.get(
            "excel", {"max_rows_per_sheet": 1000000, "include_charts": True}
        )
        self.pptx: Dict[str, str] = config.get(
            "pptx", {"slide_size": "16:9", "template": "default"}
        )
...
class EmailSettings:
    ...
    def __init__(self, config: dict | None = None):
        ...
        self.from_address: str = config.get(
            "from_address", "Dream Studio Analytics <noreply@example.com>"
        )
...
class SchedulerSettings:
    ...
    def __init__(self, config: dict | None = None):
        ...
        self.job_defaults: Dict[str, Any] = config.get(
            "job_defaults",
            {"misfire_grace_time": 900, "coalesce": True, "max_instances": 1},  # 15 minutes
        )
