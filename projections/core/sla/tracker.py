...
class SLATracker:
    ...
    def define_sla(
        ...
        if sla_type not in valid_types:
            raise ValueError(f"Invalid sla_type '{sla_type}'. Must be one of: {valid_types}")
    ...
    def _infer_sla_type(self, metric: str) -> str:
        ...
        if any(keyword in metric_lower for keyword in ["availability", "uptime", "downtime"]):
            return "availability"

        # Success rate indicators (higher is better)
        if any(keyword in metric_lower for keyword in ["success_rate", "success", "completion"]):
            return "success_rate"

        # Error/failure indicators (lower is better)
        if any(keyword in metric_lower for keyword in ["error", "failure", "failed"]):
            return "error_rate"

        # Response time indicators (check last to avoid false positives)
        if any(keyword in metric_lower for keyword in ["duration", "time", "latency", "response"]):
    ...
    def check_compliance(self) -> dict[str, Any]:
        ...
        try:
            ...
            for sla in slas:
                ...
                current_value = self._calculate_metric_value(cursor, metric, window, sla_type)

                # Determine compliance
                compliant = self._is_compliant(current_value, target, sla_type)

                # Calculate breach percentage
                breach_pct = self._calculate_breach_percentage(current_value, target, sla_type)
    ...
    def _calculate_metric_value(
        self, cursor: sqlite3.Cursor, metric: str, window_hours: int, sla_type: str
    ) -> float:
        """
    ...
    def _is_compliant(self, current_value: float, target: float, sla_type: str) -> bool:
        """
