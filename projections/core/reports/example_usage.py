...
def example_summary_report():
    ...
    report = generator.generate_report(
        report_type="summary", config={"date_range": ("2026-04-01", "2026-04-30")}
    )

    print("=== Summary Report ===")
    print(f"Generated: {report['metadata']['generated_at']}")
    print(
        f"Date range: {report['metadata']['date_range']['start']} to {report['metadata']['date_range']['end']}"
    )
...
def example_custom_report():
        ...
        "sections": [
            {
                ...
                "charts": [{"type": "line", "metric": "tokens.daily_average"}],
            },
            {
                "title": "Top Skills Performance",
                "metrics": ["skills.top_skills", "skills.success_rate_overall"],
                "charts": [],
            },
        ]
    }
    ...
    report = generator.generate_report(
        report_type="custom",
        config={"date_range": ("2026-04-01", "2026-04-30"), "template": template},
    )
