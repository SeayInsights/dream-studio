...
SECURITY NOTE: Never hardcode credentials. Use environment variables.
Set these before running:
    export EMAIL_USERNAME="your-email@gmail.com"
    export EMAIL_PASSWORD="<pw>"
"""

import os
from projections.core.email import EmailSender, TemplateRenderer
...
def example_custom_template():
    ...
    html = renderer.render(
        "report_notification.html",
        {
            ...
            "key_metrics": {"Metric 1": "100", "Metric 2": "200", "Metric 3": "300"},
        },
    )

    # Render from string
    custom_html = renderer.render_string(
        "<h1>{{title}}</h1><p>{{message}}</p>", {"title": "Hello", "message": "World"}
    )
...
def example_email_with_attachments():
    ...
    success = sender.send_email(
        ...
        attachments=["report_summary.pdf", "detailed_metrics.xlsx", "charts.png"],
        html=True,
    )
...
def example_email_with_inline_images():
    ...
    success = sender.send_email(
        ...
        body=html_body,
        inline_images={"dashboard_screenshot": "dashboard.png"},
        html=True,
    )
...
if __name__ == "__main__":
    ...
    print("-" * 50)
    print("\nSECURITY: Set EMAIL_USERNAME and EMAIL_PASSWORD environment variables!")
    print("Example: export EMAIL_USERNAME='user@gmail.com' EMAIL_PASSWORD='<pw>'")
    print("\nAvailable examples:")
