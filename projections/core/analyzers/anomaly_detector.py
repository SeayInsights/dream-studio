...
class AnomalyDetector:
    ...
    def __init__(self, sensitivity: str = "medium"):
        ...
        """
        self.thresholds = {"low": 3.0, "medium": 2.0, "high": 1.5}
    ...
    def comprehensive_anomaly_scan(
        ...
        if severe_outliers:
            recommendations.append(
                f"Found {len(severe_outliers)} severe outliers - immediate investigation needed"
            )

        if len(trend_breaks) > 0:
            recommendations.append(
                f"Detected {len(trend_breaks)} trend breaks - review for root causes"
            )
        ...
        return {
            ...
            "anomaly_rate_pct": round(
                (anomaly_count / total_points * 100) if total_points > 0 else 0, 2
            ),
