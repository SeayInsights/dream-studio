...
class PerformanceAnalyzer:
        ...
        if roi_scores:
            ...
            if worst_roi[1] < best_roi[1] * 0.5:  # If worst is <50% of best
                recommendations.append(
                    f"Consider reducing {worst_roi[0]} usage - ROI is {worst_roi[1]} vs {best_roi[1]} for {best_roi[0]}"
                )
    ...
    def analyze_session_health(self, session_metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if avg_duration < 5:
            ...
            recommendations.append(
                "Long sessions detected - consider breaking work into smaller chunks"
            )
        if sessions_per_day < 1:
            recommendations.append(
                "Low session frequency - increase regular usage for better insights"
            )

        # Project concentration analysis
        if by_project:
            top_project_pct = (
                max(by_project.values()) / total_sessions * 100 if total_sessions > 0 else 0
            )
            if top_project_pct > 80:
                recommendations.append(
                    f"Highly concentrated on one project ({top_project_pct:.0f}%) - consider diversifying or this may indicate focused sprint"
                )
