...
def create_parser():
    ...
    create_parser.add_argument(
        "--type", required=True, choices=["summary", "detailed", "custom"], help="Report type"
    )
    create_parser.add_argument(
        "--schedule", required=True, help="Cron expression (e.g., '0 9 * * *')"
    )
    create_parser.add_argument(
        "--recipients", required=True, help="Comma-separated email addresses"
    )
    create_parser.add_argument(
        "--format", default="pdf", choices=["pdf", "excel"], help="Export format"
    )
