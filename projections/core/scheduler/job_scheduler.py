...
Example:
    >>> from projections.core.scheduler import ReportScheduler, ScheduleStorage
...
class ReportScheduler:
    ...
    def schedule_report(self, schedule_config: Dict[str, Any]) -> str:
        ...
        next_run = self._calculate_next_run(
            schedule_config["schedule"], schedule_config.get("timezone", "UTC")
        )
    ...
    def run_scheduled_job(self, job_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not config.get("enabled", True):
                ...
                return {"success": False, "job_id": job_id, "message": "Job is disabled"}
            ...
            report_data = generator.generate_report(config["report_type"], config.get("config", {}))
            ...
            next_run = self._calculate_next_run(config["schedule"], config.get("timezone", "UTC"))
            self.storage.update_last_run(
                job_id, datetime.now(), next_run.isoformat() if next_run else None
            )
    ...
    def start(self) -> None:
        ...
        if self.use_apscheduler and self.scheduler:
            ...
            self._simple_scheduler_thread = threading.Thread(
                target=self._simple_scheduler_loop, daemon=True
            )
    ...
    def _simple_scheduler_loop(self) -> None:
        ...
        # Track last run time for interval jobs
        if not hasattr(self, "_simple_jobs"):
            self._simple_jobs = {}
        if not hasattr(self, "_job_last_run"):
            self._job_last_run = {}

        while self.running:
            try:
                # Execute simple jobs added via add_job (for testing)
                now = datetime.now()
                for job_id, job_info in list(self._simple_jobs.items()):
                    ...
                    if trigger == "interval":
                        ...
                        last_run = self._job_last_run.get(job_id)

                        if last_run is None or (now - last_run).total_seconds() >= seconds:
                            try:
                                func()
                                self._job_last_run[job_id] = now
                            except Exception as e:
                                logger.error(f"Simple job {job_id} error: {e}")

                # Check all enabled schedules from storage
                schedules = self.storage.load_schedules(enabled_only=True)
                ...
                # Sleep for 1 second (fast enough for tests)
                time.sleep(1)
    ...
    def _validate_cron_expression(self, cron_expr: str) -> None:
        ...
        if len(parts) != 5:
            raise ValueError(
                f"Cron expression must have 5 parts (minute hour day month day_of_week), got {len(parts)}"
            )
    ...
    def _export_report(
        self, report_data: Dict[str, Any], config: Dict[str, Any], job_id: str
    ) -> Path:
        ...
        if format_type == "pdf":
            from projections.exporters.pdf_exporter import PDFExporter
    ...
    def _send_report_email(self, report_path: Path, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            from projections.core.email import EmailSender
            ...
            return {"success": False, "message": "Email delivery not yet implemented"}
    ...
    def get_schedule(self, job_id: str) -> Optional[Dict[str, Any]]:
        """
        Get schedule configuration by job ID
        ...
        """
        return self.storage.get_schedule(job_id)

    def list_schedules(self) -> List[Dict[str, Any]]:
        """
        List all schedules (alias for list_jobs for test compatibility)
    ...
    def delete_schedule(self, job_id: str) -> bool:
        """
        Delete a schedule (alias for delete_job for test compatibility)
    ...
    def update_schedule(self, job_id: str, updated_config: Dict[str, Any]) -> bool:
        """
        Update an existing schedule

        Args:
            job_id: Job identifier
            updated_config: Dictionary of fields to update

        Returns:
            bool: True if updated successfully
        """
        # Use storage's update_schedule method
        success = self.storage.update_schedule(job_id, updated_config)

        if not success:
            logger.error(f"Cannot update non-existent schedule: {job_id}")
            return False

        # Update in scheduler if running
        if self.running:
            merged_config = self.storage.get_schedule(job_id)
            if self.use_apscheduler and self.scheduler:
                try:
                    # Remove old job
                    self.scheduler.remove_job(job_id)
                    ...
                    pass
                # Add updated job
                self._add_job_to_scheduler(job_id, merged_config)
    ...
    def add_job(self, func, trigger, **kwargs):
        """
        Add a job directly to the scheduler (for testing)
        ...
        if self.use_apscheduler and self.scheduler:
            ...
            # Fallback: store job in a simple dict for simple scheduler
            if not hasattr(self, "_simple_jobs"):
                self._simple_jobs = {}

            job_id = kwargs.get("id", str(uuid.uuid4()))
            self._simple_jobs[job_id] = {"func": func, "trigger": trigger, "kwargs": kwargs}
            return job_id
    ...
    def get_next_run_time(self, job_id: str) -> Optional[datetime]:
        ...
        Returns:
            datetime: Next run time or None if not found/scheduled
        """
        config = self.storage.get_schedule(job_id)
        if not config:
            return None

        # Check if we have next_run in storage
        if "next_run" in config and config["next_run"]:
            try:
                from dateutil import parser

                return parser.parse(config["next_run"])
            except:
                pass

        # Calculate from schedule if available
        if "schedule" in config:
            return self._calculate_next_run(config["schedule"], config.get("timezone", "UTC"))
