    ...
    def send(self, notification: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if HAS_REQUESTS:
                ...
                if success:
                    logger.debug(
                        f"WebhookChannel: Notification sent to {self.webhook_url} (status: {response.status_code})"
                    )
                else:
                    logger.error(
                        f"WebhookChannel: Request failed with status {response.status_code}: {response.text}"
                    )
                ...
                with urllib.request.urlopen(req, timeout=self.timeout) as response:
                    ...
                    if success:
                        logger.debug(
                            f"WebhookChannel: Notification sent to {self.webhook_url} (status: {status})"
                        )
                    else:
    ...
    def send(self, notification: Dict[str, Any]) -> bool:
        ...
        for channel_name, channel in self.channels.items():
            try:
                ...
                if not success:
                    logger.warning(
                        f"NotificationDispatcher: Channel '{channel_name}' failed to send notification"
                    )
        ...
        if overall_success:
            logger.info(
                f"NotificationDispatcher: Notification sent ({sum(results)}/{len(results)} channels succeeded)"
            )
        else:
