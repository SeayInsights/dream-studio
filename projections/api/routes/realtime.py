        ...
        while True:
            try:
                ...
                if message_type == "subscribe":
                    ...
                    # Send acknowledgment
                    await websocket.send_json(
                        {"type": "ack", "action": "subscribe", "metrics": metrics}
                    )
                    ...
                    # Send acknowledgment
                    await websocket.send_json(
                        {"type": "ack", "action": "unsubscribe", "metrics": metrics}
                    )
...
@router.post("/broadcast/hook-execution")
async def broadcast_hook_execution(payload: Dict[str, Any]) -> Dict[str, str]:
    """
    Broadcast hook execution event to WebSocket clients subscribed to 'hooks'.
    ...
    required_fields = ["hook_name", "status", "timestamp"]
    for field in required_fields:
        if field not in payload:
            return {"status": "error", "message": f"Missing required field: {field}"}

    # Broadcast to clients subscribed to 'hooks'
    event = {"type": "hook_execution", "data": payload}

    await connection_manager.send_to_subscribers("hooks", event)

    logger.info(f"Broadcast hook execution: {payload['hook_name']} - {payload['status']}")

    return {"status": "success", "message": "Hook execution event broadcast to subscribers"}
