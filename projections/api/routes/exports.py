...
async def create_export(request: ExportRequest):
    ...
    try:
        privacy = classify_export_privacy(
            include_raw_data=request.include_raw_data,
            sources=request.sources,
            redaction_classification=request.redaction_classification,
        )
    except ExportPrivacyError as exc:
        raise HTTPException(
            status_code=400,
            detail={
                "error": "Export privacy classification required",
                "status_code": 400,
...
async def get_export_status(export_id: str):
    ...
    return ExportResponse(
        ...
        privacy_classification=export_data.get("privacy", {}).get("classification"),
    )
...
async def download_export(export_id: str):
    ...
    if format_type == ReportFormat.JSON:
        ...
        from projections.core.insights import InsightEngine
        ...
        export_content = {
            ...
            "privacy": export_data.get("privacy", {}),
            "data": {"metrics": metrics, "analysis": analysis, "insights": insights},
        }
        ...
        # Generate Excel file
        from projections.exporters import ExcelExporter
        from .insights import collect_metrics, analyze_metrics
        from projections.core.insights import InsightEngine, RecommendationEngine
        import tempfile
        ...
        # Generate CSV file
        from projections.exporters import CSVExporter
        ...
        # Return file
        return FileResponse(
            path=result, filename=f"analytics_report_{export_id}.csv", media_type="text/csv"
        )
        ...
        # Return file
        return FileResponse(
            path=result, filename=f"analytics_report_{export_id}.zip", media_type="application/zip"
        )

    elif format_type == ReportFormat.PPTX:
        # Generate PowerPoint file
        from projections.exporters import PPTXExporter
        from .insights import collect_metrics, analyze_metrics
        from projections.core.insights import InsightEngine, RecommendationEngine
        ...
        if not result["success"]:
            raise HTTPException(
                status_code=500, detail=f"PowerPoint export failed: {result['error']}"
            )
        ...
        from projections.exporters import PowerBIExporter
        from .insights import collect_metrics, analyze_metrics
        from projections.core.insights import InsightEngine
        ...
        if not result["success"]:
            raise HTTPException(
                status_code=500, detail=f"Power BI export failed: {result['error']}"
            )
...
async def export_pdf(
    ...
    try:
        from projections.exporters import PDFExporter
        ...
        return FileResponse(
            path=result["path"], media_type="application/pdf", filename=f"report_{report_id}.pdf"
        )
...
async def export_excel(
    ...
    """
    try:
        classify_export_privacy(include_raw_data=include_raw_data)
    except ExportPrivacyError as exc:
        raise HTTPException(
            status_code=400,
            detail={
...
async def export_pptx(
    ...
    try:
        ...
        result = exporter.export_to_pptx(report["content"], temp_path, template=template)
...
async def export_csv(
    date_range: Optional[str] = Query(
        None, description="Date range in format 'YYYY-MM-DD,YYYY-MM-DD'"
    ),
    ...
    try:
        ...
        report = generator.generate_report(
            "detailed", config={"date_range": date_range_tuple, "filters": filters_dict}
        )
