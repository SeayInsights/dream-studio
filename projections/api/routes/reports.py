...
from ..models.reports import ExportPrivacyError, classify_export_privacy
...
class ReportGenerateRequest(BaseModel):
    ...
    date_range: Optional[Tuple[str, str]] = Field(
        None, description="Optional date range tuple (start_date, end_date) in ISO format"
    )
    template: Optional[str] = Field(
        None, description="Optional template name to use for report generation"
    )
    filters: Optional[dict] = Field(None, description="Optional filters to apply to data")
...
async def generate_report(request: ReportGenerateRequest):
    ...
    if request.report_type not in valid_types:
        raise HTTPException(
            ...
            detail={
                ...
                "status_code": 400,
                "details": {"provided": request.report_type, "valid_types": valid_types},
            },
        )
    ...
    try:
        privacy = classify_export_privacy(sources=sections)
    except ExportPrivacyError as exc:
        raise HTTPException(
            status_code=400,
            detail={
                "error": "Report privacy classification required",
                "status_code": 400,
    ...
    if request.date_range:
        ...
        date_range_dict = {"start": (now.replace(day=1)).isoformat(), "end": now.isoformat()}
    ...
    try:
        ...
        if ReportGenerator:
            ...
            # Generate report
            report_data = generator.generate_report(request.report_type, config=report_config)
            ...
            _reports_store[report_id] = {
                ...
                "date_range": date_range_dict,
                "status": "completed",
                "content": {"message": "Report generator not available - placeholder data"},
                "config": {"sections": sections, "privacy": privacy},
                "privacy": privacy,
                "error": None,
            }
            status = "completed"
...
async def list_reports(
    ...
    all_reports = sorted(_reports_store.values(), key=lambda r: r["generated_at"], reverse=True)
    ...
    return ReportListResponse(reports=report_items, total=total, page=page, page_size=page_size)
