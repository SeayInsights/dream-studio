...
def _ensure_imports():
    ...
    from core.utils import deprecation as _dep  # noqa: PLC0415
    from core.storage.document_store import DocumentStore as _DS  # noqa: PLC0415
...
def _filter_high_gotchas(entries: list[dict]) -> list[dict]:
    ...
    return sorted(
        filtered, key=lambda e: (severity_order.get(e["severity"].lower(), 99), e.get("id", ""))
    )
...
def compile_context(
    ...
    if skill_text is None:
        ...
        if _deprecation:
            _deprecation.warn_file_read(
                skill_md_path, f"DocumentStore.get_skill('{pack}', '{skill}')"
            )
    ...
    if orch_path.exists():
        orch_sections = _extract_orchestration_sections(orch_path.read_text(encoding="utf-8"))
    ...
    try:
        ...
        try:
            ...
            if team_gotchas:
                if _deprecation:
                    _deprecation.warn_file_read(
                        ".dream-studio/team/gotchas.yml", "DocumentStore.get_team_gotchas()"
                    )
    ...
    if not skill_gotchas_loaded and gotchas_path.exists():
        if _deprecation:
            _deprecation.warn_file_read(
                gotchas_path, f"DocumentStore.get_skill_gotchas('{pack}', '{skill}')"
            )
    ...
    if repo_context_path is not None:
        ...
        if rc_path.exists():
            ...
            repo_context_block = json.dumps(json.loads(raw_json), sort_keys=True, indent=2)
