...
def parse_handoff(path: Path) -> dict:
    ...
    # DUAL-WRITE: Emit canonical event after successful parse
    if "_parse_error" not in result:
        try:
            bridge = _get_bridge()
            if bridge:
                bridge.emit_from_legacy(
                    activity_type="ingestion.session.created",
                    stream_id=f"handoff-{result['date']}-{path.stem}",
                    stream_type="session",
                    event_data={
    ...
    if "_parse_error" not in result:
        try:
            ...
            if bridge:
                bridge.emit_from_legacy(
                    activity_type="ingestion.session.created",
                    stream_id=f"recap-{result['date']}-{path.stem}",
                    stream_type="session",
                    event_data={
                        "session_id": path.stem,
                        "date": result["date"],
                        "topic": result["topic"],
                        "what_built_count": len(result["what_built"]),
                        "risk_flags_count": len(result["risk_flags"]),
                        "corrections_count": len(result["corrections"]),
                        "source_file": str(path),
                        "operation": "parse_recap",
                    },
                    status="completed",
                )
        except Exception:
