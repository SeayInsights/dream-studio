...
import sys
CLI:  py control/execution/models/selector.py --skill=<name> [--default=sonnet]
API:  from control.execution.models.selector import recommend_model, get_model_for_skill
...
def _db_path() -> Path:
    try:
        ...
        from core.config import paths  # type: ignore[import]
...
def main() -> None:
    ...
    ap.add_argument(
        "--default", default="sonnet", choices=TIERS, help="Fallback tier when no history exists"
    )
    ap.add_argument(
        "--frontmatter",
        action="store_true",
