...
try:
    from core.event_store.studio_db import last_run as _db_last_run, run_count as _db_run_count
except ImportError:
    _db_last_run = _db_run_count = None  # type: ignore[assignment]

try:
    from control.execution.workflow.cost import estimate_workflow_cost as _estimate_cost
except ImportError:
...
def format_registry_table(workflows: list[dict]) -> str:
    ...
    bar = (
        lambda c1, c2, c3: f"{c1}{'─'*(wn+2)}{c2}{'─'*(wd+2)}{c2}{'─'*(wt+2)}{c2}{'─'*(wl+2)}{c2}{'─'*(wr+2)}{c3}"
    )
    lines = [
        bar("┌", "┬", "┐"),
