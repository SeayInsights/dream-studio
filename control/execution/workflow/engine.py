...
def _resolve_session_ref(filename: str, session_dir: str | None) -> str | None:
    ...
    if not session_dir:
        return None
    try:
        from control.session.cache import read_session_file, read_all_session_files
...
def _coerce(val: str) -> int | float | str:
    ...
    if val == "quality-score":
        ...
        if score_path.is_file():
            try:
                return float(
                    json.loads(score_path.read_text(encoding="utf-8")).get("overall_score", 0)
                )
...
def compress_node_output(raw_output: str, compress_type: str) -> str | None:
    ...
    if compress_type != "findings":
        return None
    try:
        from control.analysis.findings_summarizer import summarize_findings
