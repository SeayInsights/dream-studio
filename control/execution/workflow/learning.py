...
def track_workflow_success(workflow_chain: list[str], outcome: str) -> None:
    ...
    Args:
        workflow_chain: List of skill calls, e.g., ['ds-core think',
                       'ds-core plan', 'ds-core build']
        ...
        if row:
            ...
            conn.execute(
                ...
                (
                    workflow_id,
                    "",
                    f"Auto-tracked workflow: {' → '.join(workflow_chain[:3])}",
                    chain_json,
                    success_count,
                    total_count,
                    category,
...
def get_recommended_workflows(
    context: dict[str, Any] | None = None, limit: int = 5
) -> list[dict[str, Any]]:
    """
    ...
    with get_connection() as conn:
        ...
        query += (
            " ORDER BY (CAST(success_count AS REAL) / total_count) DESC, total_count DESC LIMIT ?"
        )
        params.append(limit)
