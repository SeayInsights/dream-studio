...
def _get_model_recommendation(skill_name: str) -> str | None:
    ...
    try:
        from control.skills.model_selector import recommend_model
...
def pre_run_cost_gate(
    ...
    for cn in cost_nodes:
        ...
        if tokens is not None:
            total += tokens
            lines.append(f"│  {nid:<{col_width}}{tokens:>10,} tok  {model_str:<{model_col}}")
        else:
            lines.append(f"│  {nid:<{col_width}}{'(unest.)':>10}      {model_str:<{model_col}}")
