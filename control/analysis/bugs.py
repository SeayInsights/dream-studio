    ...
    for file_path in path.rglob("*.py"):
        ...
        try:
            ...
            for module, line_num in imports.items():
                ...
                usage_count = (
                    sum(1 for line in lines if module in line) - 1
                )  # Subtract the import itself
                if usage_count == 0:
                    bugs.append(
                        {
                            "type": "resource_leak",
...
def _emit_security_bug_telemetry(bug: dict[str, Any], bug_id: str) -> None:
    ...
    try:
        ...
        emit_security_finding(
            ...
            category=str(bug.get("category") or "security"),
            rule_id=str(bug.get("pattern") or bug.get("type") or bug_id),
