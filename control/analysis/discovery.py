...
try:
    ...
    from control.context.context7_manager import Context7Manager
...
def discover_project(path: Path, use_context7: bool = False, query: str = "") -> Dict[str, Any]:
    ...
    if use_context7 and _CONTEXT7_AVAILABLE and total_files >= 10000:
        print(
            f"📊 Large project detected ({total_files:,} files) - using Context7 progressive loading"
        )
        ...
        print(
            f"   ✅ Loaded {len(context_result.get('details', []))} most relevant files ({context_result.get('coverage', 0)*100:.1f}% coverage)"
        )
...
def _get_git_metadata(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        first_commit_date = (
            first_commit_result.stdout.strip() if first_commit_result.returncode == 0 else None
        )
        ...
        last_commit_date = (
            last_commit_result.stdout.strip() if last_commit_result.returncode == 0 else None
        )
