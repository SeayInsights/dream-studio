        ...
        if "api" in file.name.lower() or "client" in file.name.lower():
            try:
                ...
                if "axios" in content or "fetch" in content:
                    # Extract API setup
                    match = re.search(
                        r"(export\s+(?:const|class)\s+\w*(?:API|Client)\w*[\s\S]{0,400})", content
                    )
                    if match:
                        patterns.append(
                            {
        ...
        if "store" in file.name.lower() or "state" in file.name.lower():
            try:
                ...
                if "zustand" in content or "redux" in content or "createContext" in content:
                    match = re.search(
                        r"(export\s+(?:const|function)\s+\w*[Ss]tore\w*[\s\S]{0,400})", content
                    )
                    if match:
                        patterns.append(
                            {
    ...
    for file in repo_path.rglob("*.py"):
        try:
            ...
            if "class" in content and "Exception" in content:
                match = re.search(r"(class \w+Exception[\s\S]{0,300})", content)
                if match:
                    patterns.append(
                        {
        ...
        if "route" in file.name.lower() or "api" in file.name.lower():
            try:
                ...
                if "@app.route" in content or "@router." in content:
                    match = re.search(r"(@(?:app\.route|router\.\w+)[\s\S]{0,400})", content)
                    if match:
                        patterns.append(
                            {
...
def analyze_repo(repo_url: str, shallow: bool = True) -> dict:
    ...
    try:
        ...
        with transaction() as c:
            ...
            existing = c.execute(
                "SELECT repo_id FROM reg_analyzed_repos WHERE repo_url = ?", (repo_url,)
            ).fetchone()
