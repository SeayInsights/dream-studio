...
def check_line_counts(skill_files: List[Path]) -> ValidationResult:
    ...
    for skill_file in skill_files:
        ...
        if line_count > limit:
            result.add_error(
                str(skill_file), f"{file_type} SKILL.md has {line_count} lines (limit: {limit})"
            )
    ...
    for skill_file in skill_files:
        ...
        if match:
            ...
            try:
                ...
                result.add_error(str(skill_file), f"Invalid YAML frontmatter: {str(e)}")
...
def check_banned_phrases(skill_files: List[Path]) -> ValidationResult:
    ...
    for skill_file in skill_files:
        ...
        if found_phrases:
            result.add_warning(
                str(skill_file), f"Found scaffolding phrases: {', '.join(found_phrases)}"
            )
...
def check_reference_links(skill_files: List[Path]) -> ValidationResult:
    ...
    for skill_file in skill_files:
        ...
        if broken_links:
            result.add_warning(
                str(skill_file), f"Broken reference links: {', '.join(sorted(broken_links))}"
            )
...
def find_skill_files(root_dir: Path, changed_files: Optional[List[str]] = None) -> List[Path]:
    ...
    if changed_files:
        ...
        skill_files = [
            Path(f) for f in changed_files if f.startswith("skills/") and f.endswith("SKILL.md")
        ]
    else:
