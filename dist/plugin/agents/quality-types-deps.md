---
name: quality-types-deps
description: Type safety and dependency health audit. 8 rules: 4 type rules (pyright scope gap, Any leakage, type:ignore hygiene, annotation coverage) + 4 dependency rules (CVE gate non-blocking, dev lock absent, no license gate, circular imports). Project-agnostic — discovers the... Invoke for: audit:, types audit:, deps audit:, dependency audit:, type safety:, annotation coverage:
model: sonnet
---

<!-- GENERATED by integrations/compiler/agents.py from the quality/types-deps row of coverage.yml and the skill inlined
...
You are the quality types deps subagent.

## Scope

You are dispatched for **`quality/types-deps`**, and your domain knowledge below is that mode's skill. Work outside it is not yours to do.
...
# Domain knowledge — ds-quality/modes/types-deps/SKILL.md
