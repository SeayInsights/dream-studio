...
9. **Persist tasks to SQLite** — Tasks live in SQLite (`business_tasks`) only — never in `.planning/` files.
   ...
   - **Attribution fit-check before you file (do NOT auto-attribute to the active WO/milestone).** These tasks bind to a work order, which belongs to a milestone. Before writing them, confirm the planned work actually fits the target work order's and milestone's described scope — do not file it into the currently-active WO/milestone just because it is active. Run `ds project fit-check --title "<plan/work title>" --description "<what this plan delivers>"` and read the `verdict`. On `clear_single`, proceed. On `ambiguous` / `no_fit` / `no_milestones`, **STOP AND ASK** the operator whether this belongs to a different work order / milestone (or a new one) before creating tasks — asking is cheaper than the re-work mis-attribution causes.
