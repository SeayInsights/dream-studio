...
1. **Gotcha check** — Use the maintained Dream Studio gotcha registry if present; otherwise read nearby `gotchas.yml`. Show top 3 recent gotchas.
2. **Approach history** — Use the maintained Dream Studio approach-history interface if present. If unavailable, continue with the standard debug trace.
