...
- 🟠 [L. Application security standards](../../../references/regulatory-anchors.md#l-application-security-standards): OWASP Top 10 and OWASP ASVS v5.0.0 define the vulnerability categories checked; OWASP ASVS Levels 1–3 map to the HIGH/MEDIUM/LOW severity thresholds.
...
**NOT for:**
- Full codebase compliance scans against the 22-rule baseline (use `security:review audit --full-repo`, the sibling `audit` sub-mode — a rule-based scan, not a diff review)
- Client CI/Semgrep orchestration across a GitHub org (use `security:scan`)
