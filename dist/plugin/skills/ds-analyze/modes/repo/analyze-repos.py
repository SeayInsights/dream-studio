...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Analyze single local repo (general SKILL.md patterns)
  python analyze-repos.py /path/to/repo

  # Analyze single GitHub repo
  python analyze-repos.py https://github.com/user/repo

  # Domain-specific analysis (design, career, finance, real_estate)
  python analyze-repos.py /path/to/design-repo --domain design
  python analyze-repos.py https://github.com/user/career-repo --domain career

  # Auto-detect domain
  python analyze-repos.py /path/to/repo --auto-detect --verbose

  # Compare multiple design repos
  python analyze-repos.py repo1/ repo2/ repo3/ --domain design --compare
  ...
  python analyze-repos.py https://github.com/user/repo --verbose --format json
    ...
    parser.add_argument("repos", nargs="+", help="GitHub URLs or local repository paths")
    parser.add_argument(
        "--compare", action="store_true", help="Generate comparison report (for 2+ repos)"
    )
    ...
    parser.add_argument(
        "--domain",
        type=str,
        choices=["general", "design", "career", "finance", "real_estate"],
        help="Domain for analysis (general, design, career, finance, real_estate)",
    )
    parser.add_argument(
    ...
    try:
        ...
        if args.auto_detect and len(repo_paths) == 1:
            ...
            detected = DomainAnalyzerRegistry.auto_detect_domain(
                Path(repo_paths[0]), verbose=args.verbose
            )
            if args.verbose:
                print(f"[DETECT] Auto-detected domain: {detected}")
            domain = detected

        # Run analysis
        result = analyze_repositories(
            repo_paths=repo_paths, output_format=args.format, domain=domain, verbose=args.verbose
        )
