...
def _make_violation(
    rule_id: str, severity: str, line_no: int, col: int, message: str, context_line: str, fix: str
) -> Violation:
    return Violation(
        rule=rule_id,
        severity=severity,
...
def print_human_summary(result: dict) -> None:
    ...
    print(
        header.encode(sys.stdout.encoding or "utf-8", errors="replace").decode(
...
def main() -> None:
    ...
    if args.rules:
        ...
        for rid in args.rules:
            if rid not in known:
                print(
                    f"Error: unknown rule '{rid}'. Known rules: {', '.join(sorted(known))}",
                    file=sys.stderr,
                )
                sys.exit(2)
