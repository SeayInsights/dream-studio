...
def build_primitives(brand: dict) -> tuple[dict, dict]:
    ...
    spacing_primitives = {k: {"$value": v, "$type": "dimension"} for k, v in spacing.items()}
...
def build_components() -> dict:
    ...
    return {
        ...
        "input": {
            ...
            "radius": {"$value": "6px", "$type": "dimension"},
            "padding": {
                "$value": "{primitive.spacing.1} {primitive.spacing.2}",
                "$type": "dimension",
            },
        },
    }
...
def resolve_reference(
    ...
    if len(parts) > 1:
        ...
        for part in parts:
            resolved.append(
                resolve_reference(
                    part, prim_colors, prim_fonts, prim_spacing, sem_colors, sem_fonts
                )
            )
...
def build_css(primitives: dict, semantic: dict, components: dict) -> str:
    ...
    for name, token in sem_colors.items():
        css_val = resolve_reference(
            token["$value"], prim_colors, prim_fonts, prim_spacing, sem_colors, sem_fonts
        )
    ...
    for name, token in sem_fonts.items():
        raw = token["$value"]
        css_val = resolve_reference(
            raw, prim_colors, prim_fonts, prim_spacing, sem_colors, sem_fonts
        )
...
def main() -> None:
    ...
    output_dir = (
        os.path.abspath(args.output_dir) if args.output_dir else os.path.dirname(input_path)
    )
