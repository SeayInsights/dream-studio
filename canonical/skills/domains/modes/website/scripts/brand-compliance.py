...
def parse_rgb(color_str: str):
    ...
    m = re.match(r"rgba?\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)", color_str.strip(), re.IGNORECASE)
    if m:
...
_HSL_RE = re.compile(
    r"hsla?\(\s*[\d.]+\s*,\s*[\d.]+%\s*,\s*[\d.]+%(?:\s*,\s*[\d.]+)?\s*\)", re.IGNORECASE
)
...
def extract_fonts(html: str) -> list:
    ...
    for css_text, base_offset in _extract_style_regions(html):
        for m in _FONT_FAMILY_RE.finditer(css_text):
            ...
            families = [f.strip().strip("\"'").strip() for f in value.split(",") if f.strip()]
            char_pos = base_offset + m.start()
            line = _char_to_line(char_pos, line_idx)
...
def load_brand_tokens(path: str) -> dict:
    ...
    def _collect_colors(obj):
        if isinstance(obj, str):
            ...
            for k, v in obj.items():
                if k.lower() in ("value", "color", "hex", "default") or isinstance(v, (str, dict, list)):
                    _collect_colors(v)
    ...
    def _collect_fonts(obj):
        if isinstance(obj, str):
            ...
            for k, v in obj.items():
                if k.lower() in ("family", "font-family", "display", "body", "heading", "value") or isinstance(v, (str, dict, list)):
                    _collect_fonts(v)
    ...
    # --- Spacing base ---
    spacing_data = (
        raw.get("spacing") or raw.get("space") or raw.get("brand", {}).get("spacing") or {}
    )
...
def main():
    parser = argparse.ArgumentParser(description="Score an HTML file against brand-tokens.json.")
    ...
    if all_violations:
        ...
        for v in all_violations[:10]:
            ...
            if vtype == "color":
                ...
                print(f"  L{line}: Non-brand font '{v['found']}'" f" (expected: '{v['expected']}')")
            elif vtype == "spacing":
                print(
                    f"  L{line}: Off-grid spacing {v['found']}" f" (nearest: {v['nearest_grid']})"
                )
    else:
        print()
