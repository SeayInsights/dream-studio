...
1. **Gotcha check** — Use the maintained Dream Studio gotcha registry if present; otherwise fall back to reading nearby `gotchas.yml` files. Show top 3 recent gotchas as "Pre-flight: 3 recent gotchas for [skill]" — informational only, doesn't block.
...
1. Use the maintained gotcha scanner or read nearby `gotchas.yml` files for the skills involved in this plan.
...
1. **Generate static context:** use the maintained context compiler if available.
2. **Assemble prompt:** use the maintained prompt assembler if available.
