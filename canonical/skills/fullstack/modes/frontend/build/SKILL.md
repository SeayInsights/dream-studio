...
| User gives a specific website sub-mode | Invoke `website:` with that sub-mode |
| No sub-mode specified | Invoke `website:` with auto-detect |
| Stack is React SPA | Route to `apps:saas-build` instead of `website:` |
| Stack is Astro or vanilla HTML | Route to `website:` page mode |
...
2. **Invoke `website:`**
   - Delegate all design and build work to the `website:` pipeline
...
DO delegate all design and build work to `website:`.
