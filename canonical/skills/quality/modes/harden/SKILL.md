...
- UTC enforcement: create a project-local time utility stub when the target project needs one, then replace bare `datetime.now()` manually
- Pydantic models: create a project-local models module when the target project needs one
- Audit log: create a project-local audit module when the target project needs one
- Sentry telemetry: create a project-local telemetry module when the target project needs one
- BOM script: create `scripts/bom.py`

Copy from project-standards templates where appropriate. These templates are for target projects and must not recreate Dream Studio's retired root hook library.
