...
**Status:** Active reference for ds-code-health:testing skill (18.5.1)
...
- **Code testability** — covered by ds-code-health:code-quality (18.4.3). CQ cares whether code is structured to be testable (pure functions, CQS, no side effects from getters). Testing cares whether the tests that exist are good.
- **CI tooling & gate configuration** — deferred to ds-release:ops (18.6.3). Testing verifies "tests run in CI" exists; ops owns the CI gate structure.
- **Coverage tooling setup** — testing checks coverage adequacy; ops owns the tooling configuration.
- **Type-checking** — ds-code-health:code-quality (cq-M-partial).
...
"Consider ds-code-health:testing after refactoring to assess whether new tests are well-structured."
...
This item from Section I is assigned to ds-code-health:testing because its manifestation is in test code. The production-code variant (no sleep() in production) is owned by ds-code-health:code-quality (cq-019). Both rules may fire on the same commit; reports note the sibling.
