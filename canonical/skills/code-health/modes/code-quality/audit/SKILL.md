...
## Trigger
`ds-code-health:code-quality:audit`, `audit:`, `code-quality audit:`, `cq audit:`
...
Baseline overlap detection is implemented in `canonical/skills/code-health/shared/flake8_baseline.py`.
`code-health` is a hyphenated pack name, so import it with `importlib` rather than a normal
dotted `from ... import` statement:
