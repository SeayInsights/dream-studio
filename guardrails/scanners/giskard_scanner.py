...
class GiskardScanner:
    ...
    INJECTION_PATTERNS = [
        (
            r"ignore\s+(?:previous|all|prior)\s+(?:instructions|prompts|directions)",
            Severity.CRITICAL,
        ),
    ...
    PII_PATTERNS = [
        ...
        (
            r"\b(?:passport|license)\s*(?:number|#|no\.?)?\s*:?\s*[A-Z0-9]{6,9}\b",
            "id_number",
            Severity.HIGH,
        ),
    ]
    ...
    HALLUCINATION_PATTERNS = [
        (
            r"\b(?:definitely|certainly|absolutely|guaranteed)\b(?!\s+(?:not|no|never))",
            Severity.MEDIUM,
        ),
    ...
    BIAS_PATTERNS = [
        (
            r"\b(?:all|every|no)\s+(?:women|men|blacks|whites|asians|hispanics|jews|muslims|christians)\s+(?:are|do|have)\b",
            Severity.HIGH,
        ),
        (
            r"\b(?:typical|stereotypical)\s+(?:woman|man|black|white|asian|hispanic|jew|muslim|christian)\b",
            Severity.MEDIUM,
        ),
