...
class RebuffValidator:
    ...
    EXACT_PATTERNS = [
        ...
        InjectionPattern(
            pattern=r"\[INST\]|\[/INST\]", risk_weight=0.9, description="Instruction tag injection"
        ),
        InjectionPattern(
    ...
    STRUCTURAL_PATTERNS = [
        InjectionPattern(
            pattern=r"[-=]{10,}", risk_weight=0.3, description="Separator line (context boundary)"
        ),
        InjectionPattern(
    ...
    def validate_user_input(self, input_text: str, context: dict = None) -> dict:
        ...
        exact_matches = self._check_patterns(input_text, self.EXACT_PATTERNS, layer="exact")
        detected.extend(exact_matches)

        # Layer 2: Fuzzy patterns
        fuzzy_matches = self._check_patterns(input_text, self.FUZZY_PATTERNS, layer="fuzzy")
        detected.extend(fuzzy_matches)

        # Layer 3: Structural patterns
        structural_matches = self._check_patterns(
            input_text, self.STRUCTURAL_PATTERNS, layer="structural"
        )
        ...
        details = {
            ...
            "highest_risk_pattern": max(
                (p for p in detected), key=lambda x: x["risk_weight"], default=None
            ),
        }
    ...
    def _calculate_risk_score(self, detected_patterns: List[dict]) -> float:
        ...
        sorted_patterns = sorted(detected_patterns, key=lambda x: x["risk_weight"], reverse=True)
    ...
    def batch_validate(self, inputs: List[str], context: dict = None) -> List[dict]:
        ...
        """
        return [self.validate_user_input(input_text, context) for input_text in inputs]
