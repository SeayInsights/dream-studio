...
# Columns that a RELEASED-LATER migration adds to this table. A write that fails because
# one of these is missing is running against a recognised older schema; any other missing
# column is a schema this store does not know, which is a fault rather than a stage.
_MIGRATION_ADDED_COLUMNS = ("instance_key",)
...
def _is_stale_schema(exc: sqlite3.OperationalError) -> bool:
    ...
    ONLY A COLUMN A MIGRATION ADDS COUNTS. Accepting any missing column meant a table
    whose shape is wrong for some other reason -- hand-edited, half-restored from a
    backup, a foreign table of the same name -- was indistinguishable from
    144-without-152 and degraded forever, which an independent review measured with a
    dropped ``updated_at`` (WO b302834b task 1633f16e). A schema this store does not
    recognise is a fault; a schema it recognises as older is not.
    """
    text = str(exc).lower()
    if "no such table" in text:
        return True
    if "no such column" in text or "has no column named" in text:
        return any(column in text for column in _MIGRATION_ADDED_COLUMNS)
    return False
...
def set_wo_artifact(
    ...
    *,
    ...
    if not borrowed:
        ...
        # AN UNOPENABLE DATABASE IS NOT AN UNRELEASED MIGRATION. This returned False for
        # any connect error, so a missing parent directory or a typo'd `db_path` read as
        # "the artifact table does not exist yet" and routed every artifact to disk with a
        # fallback reason saying "authority write no-op". Measured by an independent
        # review (WO b302834b task 1633f16e).
        #
        # AND IT DOES NOT CREATE THE AUTHORITY. Adding a `mkdir` here would make a typo'd
        # `db_path` quietly produce a new empty database at the wrong location -- an
        # artifact write is never the thing that should bring an authority into being.
        # `bootstrap_database` and `_require_db` own that. A connect failure raises: "I
        # could not open the authority" is a fault, and the caller's disk fallback is for
        # a schema that predates the write, not for a database that is not there.
        #
    ...
    try:
        ...
        # is released). That is the documented no-op.
        #
        # EVERY OTHER IntegrityError RAISES. The comment here used to name a FOREIGN KEY
        # failure as the counter-example; this table declares no FOREIGN KEY in migrations
        # 144/152/154, and the INSERT uses ON CONFLICT DO UPDATE, so neither an FK nor a
        # UNIQUE violation is reachable -- an independent review checked the DDL and found
        # the claim describing a constraint that does not exist (WO b302834b task
        # 1017632d). The reachable one is `content NOT NULL`. The narrowing is what
        # matters either way: swallowing anything but the CHECK would report a stored
        # artifact that no reader can ever find.
...
def backfill_wo_artifacts(
    planning_root: Path, *, db_path: Path | None = None
) -> tuple[int, list[str]]:
    """Copy ``.planning/work-orders/<id>/*.{md,json}`` ceremony artifacts into the
    authority. Returns ``(written, failures)``.

    Idempotent (upsert), so it is safe to re-run after ``ds migrate activate`` releases a
    newer artifact migration. Files are left in place; the authority takes precedence on
    read, so a stale disk copy is shadowed rather than consulted.

    IT SURVIVES A DEGRADED AUTHORITY, WHICH IS ITS WHOLE JOB. This looped ~171
    ``set_wo_artifact`` calls with no ``try``, so once that function learned to RAISE on a
    lock, any busy moment aborted the loop mid-way and discarded the running count -- the
    operator running the repair learned neither how far it got nor why it stopped. The one
    command built to work against an unhealthy authority was the least protected caller of
    the change that made writes raise (WO b302834b task f9b6e787).

    So each artifact is attempted independently and a failure is COLLECTED rather than
    thrown: 170 recovered and one locked is a better outcome than nothing recovered, and
    the caller reports both numbers. ``failures`` carries one line per artifact that did
    not land, which is what makes a partial recovery re-runnable with intent instead of
    hopefully.
    """
    ...
    for wo_dir in sorted(wo_root.iterdir()):
        ...
        for fname, kind in filename_to_kind.items():
            ...
            try:
                ...
                failures.append(f"{wo_dir.name}/{fname}: unreadable ({exc})")
                continue
            try:
                if set_wo_artifact(wo_dir.name, kind, content, db_path=db_path):
                    written += 1
                else:
                    # The documented no-op: a schema older than the write. Not a failure
                    # to report per file -- it is the same fact for every file, and the
                    # caller's zero-written check names it once.
                    continue
            except Exception as exc:  # noqa: BLE001 - one bad artifact must not end the recovery
                failures.append(f"{wo_dir.name}/{kind}: {type(exc).__name__}: {exc}")
