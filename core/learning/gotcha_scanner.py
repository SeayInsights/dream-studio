...
def _try_db_search(keyword: str) -> list[dict] | None:
    ...
    try:
        from core.event_store.studio_db import search_gotchas_db  # noqa: PLC0415
...
def _try_db_gotchas_for_skill(skill_id: str) -> list[dict] | None:
    ...
    """
    try:
        from core.event_store.studio_db import get_gotchas_for_skill as db_get  # noqa: PLC0415
...
def _try_db_recent(skill_id: str | None, limit: int) -> list[dict] | None:
    ...
    try:
        ...
        if skill_id:
            rows = conn.execute(
                "SELECT * FROM reg_gotchas WHERE skill_id=? " "ORDER BY discovered DESC LIMIT ?",
                (skill_id, limit),
            ).fetchall()
        else:
            rows = conn.execute(
...
def _walk_all(plugin_root: Path | None = None) -> list[dict]:
    ...
    try:
        if plugin_root is None:
            from core.config import paths as _paths  # noqa: E402, PLC0415
