...
def suggest_optimizations(operation: str, usage_history: list[dict]) -> list[str]:
    ...
    models_used = [
        h.get("context", {}).get("model") for h in op_history if h.get("context", {}).get("model")
    ]
    model_counts = {}
    ...
    if len(op_history) >= 3:
        ...
        if len(set(filter(None, recent_ops))) < len(recent_ops):
            suggestions.append("Use caching for repeated queries (60% token reduction)")

    # Suggestion 3: Progressive loading
    if avg_tokens > 100000:
        suggestions.append("Use progressive loading with Context7 (40% token reduction)")
