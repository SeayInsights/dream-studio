...
class WorkflowGraphIntegration:
    ...
    def migrate_wave_to_graph(self, pi_wave_id: str, project_node_id: str) -> Optional[str]:
        """
        Migrate an existing pi_wave to the execution graph.

        Reads wave data from pi_waves table and creates corresponding
        execution graph nodes.

        Args:
            pi_wave_id: Wave ID from pi_waves table
            project_node_id: Parent project node

        Returns:
            str: Created wave node_id or None
        """
        # Read wave data
        with get_connection(read_only=True) as conn:
            cursor = conn.execute("SELECT * FROM pi_waves WHERE wave_id = ?", (pi_wave_id,))
            wave_row = cursor.fetchone()

            if not wave_row:
                logger.error(f"Wave not found: {pi_wave_id}")
                return None

            wave_data = dict(wave_row)

            # Read tasks for this wave
            cursor = conn.execute(
        ...
        # Create wave node
        wave_node_id = self.create_wave_node(
            wave_id=pi_wave_id,
            wave_title=wave_data.get("description", f"Wave {pi_wave_id}"),
            project_node_id=project_node_id,
            metadata={
                "analysis_run_id": wave_data.get("analysis_run_id"),
                "parallel_groups": wave_data.get("parallel_groups", 1),
            },
        )

        status = wave_data.get("status", "pending")
        if status == "running":
            self.graph.update_status(wave_node_id, to_db_value(ExecutionLifecycle.ACTIVE))
        elif status == "completed":
            duration = wave_data.get("duration_seconds")
            self.graph.update_status(
                wave_node_id, to_db_value(ExecutionLifecycle.COMPLETED), duration
            )

        # Create task nodes
        task_node_map = {}  # Map task_number -> node_id
        for task_row in task_rows:
            task_data = dict(task_row)
            task_num = task_data.get("task_number", 0)

            task_node_id = self.create_task_node(
                task_title=task_data.get("task_name", f"Task {task_num}"),
                wave_node_id=wave_node_id,
                task_description=task_data.get("task_description"),
                metadata={
                    "task_number": task_num,
                    "parallel_group": task_data.get("parallel_group", 1),
                    "skill_name": task_data.get("skill_name"),
                    "estimated_tokens": task_data.get("estimated_tokens"),
                },
            )

            task_node_map[task_num] = task_node_id

            task_status = task_data.get("status", "pending")
            if task_status == "in_progress":
                self.graph.update_status(task_node_id, to_db_value(ExecutionLifecycle.ACTIVE))
            elif task_status == "completed":
                self.graph.update_status(task_node_id, to_db_value(ExecutionLifecycle.COMPLETED))

        logger.info(
            f"Migrated wave {pi_wave_id} to graph: "
            f"wave_node={wave_node_id}, tasks={len(task_node_map)}"
        )
