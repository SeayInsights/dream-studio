...
from control.session.parser import extract_blockers, parse_handoff, parse_recap, scan_sessions
...
def disable_parser_event_bridge(monkeypatch: pytest.MonkeyPatch) -> None:
    """Parser behavior tests should not emit events to the native runtime DB."""
    monkeypatch.setattr("control.session.parser._get_bridge", lambda: None)
...
class TestParseHandoffJson:
    ...
    def test_skills_inferred_from_phase(self, tmp_path: Path) -> None:
        ...
        assert "ds-core" in result["skills_used"]
...
class TestExtractBlockersCountsFrequency:
    ...
    def test_duplicate_within_same_session_counted_once(self) -> None:
        ...
        sessions = [self._make_handoff_session("feat-a", ["flaky test", "flaky test"])]
        result = extract_blockers(sessions)
    ...
    def test_recap_risk_flags_counted_as_blockers(self) -> None:
        ...
        sessions = [self._make_recap_session("feat-a", ["low test coverage"], "2026-05-01")]
        result = extract_blockers(sessions)
