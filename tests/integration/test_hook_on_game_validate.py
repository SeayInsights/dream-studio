...
# Import validation functions from domain_lib instead of hook module
...
class TestDetectProject:
    ...
    def test_returns_none_outside_project(self, tmp_path: Path):
        ...
        test_file.write_text("extends Node", encoding="utf-8")
        assert detect_project(test_file) is None
...
class TestClassifyPath:
    def test_gameplay_keywords(self):
        assert "gameplay" in classify_path("scripts/gameplay/player.gd")
        assert "gameplay" in classify_path("scripts/combat/sword.gd")
        assert "gameplay" in classify_path("scripts/mechanics/jump.gd")

    def test_ai_keywords(self):
        assert "ai" in classify_path("scripts/ai/enemy.gd")
        assert "ai" in classify_path("scripts/npc/villager.gd")
        assert "ai" in classify_path("scripts/enemies/boss.gd")

    def test_ui_keywords(self):
        assert "ui" in classify_path("scripts/ui/health_bar.gd")
        assert "ui" in classify_path("scenes/hud/minimap.gd")
        assert "ui" in classify_path("scripts/menu/main_menu.gd")

    def test_networking_keywords(self):
        assert "networking" in classify_path("scripts/multiplayer/lobby.gd")
        assert "networking" in classify_path("scripts/net/rpc_handler.gd")

    def test_no_match(self):
        assert classify_path("scripts/utils/helpers.gd") == set()

    def test_filename_keyword_match(self, tmp_path: Path):
        f = tmp_path / "combat_system.gd"
        f.write_text("extends Node", encoding="utf-8")
        assert "gameplay" in classify_path("scripts/combat_system.gd", f)

    def test_content_fallback(self, tmp_path: Path):
        f = tmp_path / "thing.gd"
        f.write_text("extends CharacterBody2D\nvar velocity = Vector2.ZERO", encoding="utf-8")
        assert "gameplay" in classify_path("scripts/thing.gd", f)
...
class TestValidateGdscript:
    ...
    def test_no_warnings_outside_gameplay(self, tmp_path: Path):
        f = tmp_path / "util.gd"
        f.write_text("extends Node\nconst MAX_RETRIES = 3\n", encoding="utf-8")
        result = validate_gdscript(f, set())
        assert not result.warnings
...
class TestValidateJsonData:
    ...
    def test_schema_consistency(self, data_dir: Path):
        ...
        result = validate_json_data(f, "data/units.json")
...
class TestValidateAssetNaming:
    def test_valid_prefix(self, tmp_path: Path):
        ...
        f.write_text("", encoding="utf-8")
        result = validate_asset_naming(f, "assets/chr_hero.glb")
        assert not result.warnings
    ...
    def test_missing_prefix(self, tmp_path: Path):
        ...
        f.parent.mkdir(parents=True)
        f.write_text("", encoding="utf-8")
        result = validate_asset_naming(f, "assets/hero.glb")
    ...
    def test_spaces_in_name(self, tmp_path: Path):
        ...
        f.parent.mkdir(parents=True)
        f.write_text("", encoding="utf-8")
        result = validate_asset_naming(f, "assets/my hero.glb")
    ...
    def test_ignores_non_asset_dirs(self, tmp_path: Path):
        ...
        f.parent.mkdir(parents=True)
        f.write_text("", encoding="utf-8")
        result = validate_asset_naming(f, "scripts/hero.glb")
...
class TestVersionStaleness:
    def test_current_version_no_warning(self, godot_project: Path):
        ctx = ProjectContext(root=godot_project, godot_version="4.3", has_standard_structure=True)
        assert check_version_staleness(ctx) == []

    def test_newer_version_warns(self, godot_project: Path):
        ctx = ProjectContext(root=godot_project, godot_version="4.5", has_standard_structure=True)
        warnings = check_version_staleness(ctx)
        assert len(warnings) == 1
        assert "outdated" in warnings[0]

    def test_unknown_version_no_warning(self, godot_project: Path):
        ctx = ProjectContext(
            root=godot_project, godot_version="unknown", has_standard_structure=True
        )
        assert check_version_staleness(ctx) == []
