...
# ── T2: token.consumed projects on the live emit-then-tick path ───────────────


class TestSyncTickProjectsTokens:
    """WO-TOKEN-BACKFILL T2: token.consumed must materialize on the live
    emit-then-tick path (sync_tick), not only the dormant projection daemon.
    Regression guard: before this fix sync_tick registered only the four SDLC
    projections, so token_usage_records never grew after the one-time backfill.
    """

    def test_sync_tick_projects_token_consumed_into_usage_records(self, tmp_path, monkeypatch):
        from core.config.database import DatabaseRuntime  # noqa: PLC0415
        from core.config.sqlite_bootstrap import bootstrap_database  # noqa: PLC0415
        ...
        try:
            ...
            try:
                conn.execute(
                    ...
                    (
                        ...
                        _NOW,
                        _SESSION_ID,
                        json.dumps({"project_id": _PROJECT_ID}),
            ...
            from core.projections.runner import sync_tick  # noqa: PLC0415
        ...
        assert row is not None, (
            "sync_tick must project token.consumed into token_usage_records "
            "(WO-TOKEN-BACKFILL T2)"
        )
