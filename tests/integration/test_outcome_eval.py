...
def test_failed_outcome_reopens_wo(tmp_path: Path) -> None:
    ...
    symptom = (
        "SQL-CHECK: SELECT 1 WHERE EXISTS (SELECT 1 FROM business_projects WHERE project_id='nope')"
    )
...
def test_end_to_end(tmp_path: Path) -> None:
    ...
    good = _seed_closed_wo(
        db, symptom="SQL-CHECK: SELECT 1 WHERE EXISTS (SELECT 1 FROM business_projects)"
    )
