...
class TestMigrationRunner:
    def test_fresh_db_applies_all_migrations(self, tmp_path):
        ...
        assert v == 8, f"Expected schema version 8, got {v}"
        conn.close()
    ...
    def test_migration_is_idempotent(self, tmp_path):
        ...
        assert rows == 7, f"Expected 7 migration records, got {rows}"
        conn2.close()

    def test_existing_db_gets_upgraded(self, tmp_path):
        """Simulates an existing DB at version 7 that needs migration 8."""
        db = tmp_path / "test.db"
        conn = _connect(db)
        conn.close()
        conn = sqlite3.connect(str(db))
        conn.execute("DELETE FROM _schema_version WHERE version = 8")
        conn.commit()
        conn.close()
        conn2 = _connect(db)
        v = conn2.execute("SELECT MAX(version) FROM _schema_version").fetchone()[0]
