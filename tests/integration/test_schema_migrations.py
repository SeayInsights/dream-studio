...
def _expected_version() -> int:
    """Discover the highest migration version from canonical SQL files."""
    mdir = _migrations_dir()
    files = sorted(mdir.glob("[0-9]*.sql"))
    return max(int(f.stem.split("_")[0]) for f in files)


def _expected_migration_count() -> int:
    """Count the number of canonical migration SQL files."""
    mdir = _migrations_dir()
    return len(list(mdir.glob("[0-9]*.sql")))
...
class TestMigrationRunner:
    def test_fresh_db_applies_all_migrations(self, tmp_path):
        ...
        assert v == expected, f"Expected schema version {expected}, got {v}"
        conn.close()

    def test_schema_version_function(self, tmp_path):
        db = tmp_path / "test.db"
        _connect(db).close()
        assert schema_version(db) == _expected_version()

    def test_migration_is_idempotent(self, tmp_path):
        expected_v = _expected_version()
        expected_rows = _expected_migration_count()
        db = tmp_path / "test.db"
        conn1 = _connect(db)
        conn1.close()
        conn2 = _connect(db)
        ...
        assert rows == expected_rows, f"Expected {expected_rows} migration records, got {rows}"
        conn2.close()

    def test_existing_db_gets_upgraded(self, tmp_path):
        """Simulates an existing DB that needs re-migration after a version is deleted."""
        expected = _expected_version()
        db = tmp_path / "test.db"
        conn = _connect(db)
        # Keep the downgrade simulation inside the isolated fixture database.
        conn.execute("DELETE FROM _schema_version WHERE version = ?", (expected,))
        conn.commit()
        conn.close()
        conn2 = _connect(db)
        v = conn2.execute("SELECT MAX(version) FROM _schema_version").fetchone()[0]
    ...
    def test_version_check_blocks_old_code(self, tmp_path):
        ...
        db = tmp_path / "test.db"
        conn = _connect(db)
        conn.execute("INSERT INTO _schema_version(version, applied_at) VALUES(999, '2099-01-01')")
        conn.commit()
        conn.close()
...
class TestForeignKeys:
    ...
    def test_fk_null_allowed(self, tmp_path):
        ...
        conn = _connect(db)
        conn.execute(
            "INSERT INTO raw_sessions(session_id, started_at) " "VALUES('s1', '2026-01-01')"
        )
        conn.commit()
...
class TestIndexes:
    EXPECTED_INDEXES = [
        ...
        # idx_pulse_date, idx_specs_path, idx_specs_project, idx_tasks_spec,
        # idx_tasks_project removed (dead tables in migration 128)
