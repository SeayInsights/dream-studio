...
def test_find_skills_by_trigger(tmp_path):
    db = tmp_path / "test.db"
    upsert_skill(
        "core:build", "core", "build", "s/b.md", triggers="build:,execute plan:", db_path=db
    )
    upsert_skill(
        "core:think", "core", "think", "s/t.md", triggers="think:,spec:,research:", db_path=db
    )
    upsert_skill(
        "quality:debug", "quality", "debug", "s/d.md", triggers="debug:,diagnose:", db_path=db
    )
...
def test_upsert_skill_is_idempotent(tmp_path):
    ...
    conn = _connect(db)
    count = conn.execute("SELECT COUNT(*) FROM reg_skills WHERE skill_id='core:build'").fetchone()[
        0
    ]
    conn.close()
    assert count == 1
...
def test_upsert_and_search_gotchas(tmp_path):
    ...
    upsert_gotcha(
        ...
        keywords="parallel,subagent,race,file",
        db_path=db,
    )
    upsert_gotcha(
        "migration-order",
...
def test_gotcha_preserves_hit_count_on_upsert(tmp_path):
    ...
    conn = _connect(db)
    conn.execute(
        "UPDATE reg_gotchas SET times_hit=5 WHERE gotcha_id='g1' AND skill_id='core:build'"
    )
    conn.commit()
    conn.close()
...
def test_upsert_and_query_workflows(tmp_path):
    db = tmp_path / "test.db"
    upsert_workflow(
        "daily-standup", "workflows/daily-standup.yaml", category="daily", node_count=5, db_path=db
    )
    upsert_workflow(
        "daily-close", "workflows/daily-close.yaml", category="daily", node_count=4, db_path=db
    )
    upsert_workflow("hotfix", "workflows/hotfix.yaml", category="feature", node_count=6, db_path=db)
...
def test_gotcha_scanner_search_uses_db(tmp_path, monkeypatch):
    ...
    found = any(
        r.get("gotcha_id") == "test-gotcha" or r.get("id") == "test-gotcha" for r in results
    )
...
def test_gotcha_scanner_falls_back_to_file_walk(tmp_path, monkeypatch):
    ...
    from core.learning.gotcha_scanner import search_gotchas, clear_cache  # noqa: E402
