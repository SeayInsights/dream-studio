...
from core.learning import micro_capture  # noqa: E402
...
def test_micro_capture_appends_to_today(isolated_home):
    ...
    assert len(time_part) == 5 and time_part[2] == ":", f"Expected HH:MM prefix, got: {time_part!r}"
...
def test_lesson_queue_list_empty(isolated_home, capsys):
    ...
    assert (
        "0" in captured.out or "no lessons" in captured.out.lower()
