...
class TestBackfillTaskStatus:
    def test_backfill_marks_tasks(self, test_db: Path) -> None:
        from core.event_store.studio_db import upsert_project, upsert_spec, upsert_task
        ...
        _seed_handoff(test_db, "sess-1", "test-proj", "test-spec", tasks_completed=2, tasks_total=3)

        from interfaces.cli.backfill_task_status import backfill_task_status
    ...
    def test_backfill_dry_run(self, test_db: Path) -> None:
        ...
        _seed_handoff(test_db, "sess-1", "test-proj", "test-spec", tasks_completed=1, tasks_total=2)
    ...
    def test_no_matching_spec(self, test_db: Path) -> None:
        _seed_handoff(
            test_db, "sess-1", "test-proj", "nonexistent-topic", tasks_completed=5, tasks_total=10
        )
