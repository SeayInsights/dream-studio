...
from interfaces.cli.studio_backup import (  # noqa: E402
    backup,
    restore,
    export,
    main,
...
from core.config.state import backup_db, _maybe_cloud_push  # noqa: E402
...
class TestLocalBackup:
    def test_backup_creates_bak(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("interfaces.cli.studio_backup.paths.state_dir", lambda: tmp_path)
        monkeypatch.setattr("interfaces.cli.studio_backup._db_path", lambda: db_path)
    ...
    def test_backup_missing_db_exits(self, tmp_path, monkeypatch):
        monkeypatch.setattr("interfaces.cli.studio_backup._db_path", lambda: tmp_path / "nope.db")
    ...
    def test_restore_invalid_file_exits(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("interfaces.cli.studio_backup._db_path", lambda: tmp_path / "studio.db")
        with pytest.raises(SystemExit):
            restore(bad)

    def test_restore_missing_bak_exits(self, tmp_path, monkeypatch):
        monkeypatch.setattr(
            "interfaces.cli.studio_backup._default_bak_path", lambda: tmp_path / "nope.bak"
        )
        with pytest.raises(SystemExit):
            restore()
...
class TestExport:
    ...
    def test_export_creates_backup_if_missing(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("interfaces.cli.studio_backup._db_path", lambda: db_path)
        monkeypatch.setattr(
            "interfaces.cli.studio_backup._default_bak_path", lambda: db_path.with_suffix(".db.bak")
        )
...
class TestCloudConfig:
    def test_save_and_load_config(self, tmp_path, monkeypatch):
        monkeypatch.setattr("interfaces.cli.studio_backup.paths.state_dir", lambda: tmp_path)
        monkeypatch.setattr(
            "interfaces.cli.studio_backup._backup_config_path",
            lambda: tmp_path / "backup-config.json",
        )
    ...
    def test_load_missing_config_returns_empty(self, tmp_path, monkeypatch):
        monkeypatch.setattr(
            "interfaces.cli.studio_backup._backup_config_path", lambda: tmp_path / "nope.json"
        )
    ...
    def test_cloud_auto_toggles(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("interfaces.cli.studio_backup._backup_config_path", lambda: cfg_path)
        monkeypatch.setattr(
            "interfaces.cli.studio_backup._load_backup_config",
            lambda: json.loads(cfg_path.read_text(encoding="utf-8")),
        )
        monkeypatch.setattr(
            "interfaces.cli.studio_backup._save_backup_config",
            lambda d: cfg_path.write_text(json.dumps(d), encoding="utf-8"),
        )
...
class TestCloudWithMockRclone:
    def test_cloud_push_calls_rclone(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("interfaces.cli.studio_backup.paths.state_dir", lambda: tmp_path)
        monkeypatch.setattr("interfaces.cli.studio_backup._db_path", lambda: db_path)
        monkeypatch.setattr("interfaces.cli.studio_backup._default_bak_path", lambda: bak_path)
        backup(db_path)
        ...
        monkeypatch.setattr("interfaces.cli.studio_backup._has_rclone", lambda: True)
        monkeypatch.setattr("interfaces.cli.studio_backup._rclone_run", lambda args: mock_result)

        from interfaces.cli.studio_backup import cloud_push
    ...
    def test_cloud_push_no_rclone_exits(self, monkeypatch):
        monkeypatch.setattr("interfaces.cli.studio_backup._has_rclone", lambda: False)
        with pytest.raises(SystemExit):
            from interfaces.cli.studio_backup import cloud_push
