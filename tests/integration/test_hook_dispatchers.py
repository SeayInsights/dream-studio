...
def isolated_dispatcher_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Keep real hook handlers invoked by dispatchers out of native runtime state."""
    home = tmp_path / "home"
    home.mkdir()
...
def _load_dispatcher(name: str):
    path = PLUGIN_ROOT / "runtime" / "hooks" / "meta" / f"{name}.py"
...
class TestPromptDispatcher:
    def test_calls_all_handlers(self, tmp_path: Path) -> None:
        ...
        # Import the lib module to mock
        sys.path.insert(0, str(Path(__file__).resolve().parents[2]))
        from control.execution import dispatch_tracking

        original_load = dispatch_tracking.load_module
        ...
        with patch.object(dispatch_tracking, "load_module", side_effect=tracking_load):
    ...
    def test_handler_failure_doesnt_block_others(self, tmp_path: Path) -> None:
        ...
        with patch.object(dispatch_tracking, "load_module", side_effect=failing_then_working):
            with patch.object(mod, "STATE_DIR", tmp_path):
