...
def test_harden_nudge_uses_project_root(isolated_home, tmp_path, monkeypatch, capsys, handler):
    ...
    monkeypatch.setattr(tool_tracking, "project_root", lambda _: tmp_path)
    mod.main()
...
def test_harden_nudge_suppressed_when_makefile_at_root(
    isolated_home, tmp_path, monkeypatch, capsys, handler
):
...
def test_security_suggest_fires_for_auth_file(
    isolated_home, tmp_path, monkeypatch, capsys, handler
):
...
def test_security_suggest_suppressed_after_first(
    isolated_home, tmp_path, monkeypatch, capsys, handler
):
