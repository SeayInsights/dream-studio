...
is loaded as a CLASSIC script — not type="module" — so the global functions the 71
inline event handlers depend on stay on window. These tests pin the structural
contract; behavior preservation follows from the faithful extraction + classic-script
load.
"""
...
def test_static_assets_served():
    ...
    assert css.content, "dashboard.css served empty"
    assert js.content, "dashboard.js served empty"
...
def test_shell_has_no_inline_style_or_script_blocks():
    """The monolithic inline blocks must be gone — only external <script src> refs
    (CDN + dashboard.js) and the <link> remain."""
    html = (FRONTEND / "dashboard.html").read_text(encoding="utf-8")
    assert "<style>" not in html, "inline <style> block still present"
    # An inline opening script tag (no src) would be '<script>'; extracted refs all have src=.
    assert "<script>" not in html, "inline <script> block still present"


def test_dashboard_js_loaded_as_classic_script():
    """dashboard.js must NOT be a module — the 71 inline onclick= handlers resolve
...
def test_shell_is_substantially_smaller_than_extracted_js():
    """Sanity: the ~394KB JS moved out; the HTML shell is a fraction of the JS."""
