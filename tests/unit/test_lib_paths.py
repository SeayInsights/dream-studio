"""Tests for core/config/paths.py — plugin_root, plugin_version, writability, version mismatch."""
...
import core.config.paths as lp  # noqa: E402
from core.config.paths import user_data_dir_writable, warn_version_mismatch  # noqa: E402
...
def test_warn_version_mismatch_detects_cache_mismatch(tmp_path, monkeypatch):
    ...
    (plugin_dir / "plugin.json").write_text(json.dumps({"version": "0.9.0"}), encoding="utf-8")
...
def test_warn_version_mismatch_inner_exception_swallowed(tmp_path, monkeypatch):
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setattr(lp, "plugin_version", lambda: "1.0.0")
    bad_dir = tmp_path / ".claude" / "plugins" / "cache" / "dream-studio" / "bad" / "x"
