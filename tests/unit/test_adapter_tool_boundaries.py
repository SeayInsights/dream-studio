"""Phase 10B adapter/tool edge boundary guardrails."""
...
ADAPTER_FORBIDDEN_IMPORT_PREFIXES = PROVIDER_OR_NETWORK_IMPORT_PREFIXES | {
    "sqlite3",
    "subprocess",
...
LEGACY_ADAPTER_IMPORT_EXCEPTIONS: set[str] = set()
...
def test_adapter_interface_layer_is_pure_normalization_only():
    offenders: list[str] = []

    for path in _python_files(ADAPTER_ROOT):
        modules = _imported_modules(_read(path))
        for module in sorted(modules):
            if _matches_any_prefix(module, ADAPTER_FORBIDDEN_IMPORT_PREFIXES):
...
def test_github_and_ci_execution_tools_remain_explicit_method_only_adapters():
    ...
    for path in [github_adapter, ci_collector]:
        ...
        if "core.events" not in modules and "core.events.types" not in modules:
...
def test_runtime_model_metadata_defaults_are_provider_neutral():
    ...
    assert 'model: str = "unspecified"' in studio_db
...
def test_legacy_top_level_adapter_import_residue_is_absent_after_phase10c():
    ...
    assert top_level_imports == LEGACY_ADAPTER_IMPORT_EXCEPTIONS
