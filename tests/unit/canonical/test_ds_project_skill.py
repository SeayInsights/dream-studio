...
def test_skill_contains_one_question_at_a_time_rule():
    text = _skill_text()
    assert (
        "one question at a time" in text.lower()
...
def test_skill_contains_no_placeholders_rule():
    text = _skill_text()
    assert (
        "TBD" in text or "No placeholders" in text or "no placeholder" in text.lower()
...
def test_skill_contains_scope_assessment_rule():
    text = _skill_text()
    assert (
        "multi-subsystem" in text.lower() or "scope assessment" in text.lower()
...
def test_packs_yaml_includes_ds_project():
    packs = _packs()
    assert "ds-project" in packs.get("packs", {}), "packs.yaml missing ds-project entry"
...
def test_packs_yaml_ds_project_skill_field():
    packs = _packs()
    ds_project = packs.get("packs", {}).get("ds-project", {})
    assert ds_project.get("skill") == "ds-project", "ds-project pack must have skill: ds-project"
...
def test_skill_contains_resume_mode_section():
    text = _skill_text()
    assert (
        "Resume Mode" in text or "resume mode" in text.lower()
...
def test_resume_mode_contains_plain_english_output_rule():
    text = _skill_text()
    assert (
        "plain English" in text or "plain english" in text.lower()
...
def test_resume_mode_contains_never_show_uuid_rule():
    text = _skill_text()
    assert (
        "UUID" in text or "uuid" in text.lower()
...
def test_resume_mode_contains_one_question_at_a_time_rule():
    ...
    assert (
        "one question at a time" in text.lower() or "One question at a time" in text
...
def test_resume_metadata_contains_whats_next_trigger():
    ...
    combined = " ".join(trigger_strs)
    assert (
        "what" in combined.lower() and "next" in combined.lower()
