"""Slice 6d: code-health:audit SKILL.md + packs.yaml integrity tests."""
...
AUDIT_SKILL_MD = REPO_ROOT / "canonical" / "skills" / "code-health" / "modes" / "audit" / "SKILL.md"
...
def test_packs_yaml_quality_includes_audit():
    ...
    code_health_modes = packs_data["packs"]["code-health"]["modes"]
    assert "audit" in code_health_modes


def test_audit_mode_dir_registered_in_quality_modes_dir():
    audit_dir = REPO_ROOT / "canonical" / "skills" / "code-health" / "modes" / "audit"
    assert audit_dir.is_dir(), f"audit mode directory not found at {audit_dir}"
    packs_data = yaml.safe_load(PACKS_YAML.read_text(encoding="utf-8"))
    code_health_modes = set(packs_data["packs"]["code-health"]["modes"])
    mode_dirs = {d.name for d in audit_dir.parent.iterdir() if d.is_dir()}
    unlisted = mode_dirs - code_health_modes
