...
def test_old_agents_dir_removed():
    assert not (
        REPO_ROOT / "agents"
    ).exists(), "agents/ should not exist at repo root after migration (moved to canonical/agents/)"
...
def test_canonical_skills_contains_existing_bootstrap():
    assert (
        CANONICAL / "skills" / "ds-bootstrap" / "SKILL.md"
    ).is_file(), "canonical/skills/ds-bootstrap/SKILL.md must still exist after migration"
