"""Tests for core/telemetry/audit.py and core/telemetry/telemetry.py."""
...
from core.telemetry.audit import log_event  # noqa: E402
from core.telemetry.telemetry import capture_exception, init_sentry  # noqa: E402
...
def test_log_event_appends(tmp_path, monkeypatch):
    ...
    lines = (
        (tmp_path / ".dream-studio" / "audit.jsonl")
...
def test_log_event_no_session_id(tmp_path, monkeypatch):
    ...
    record = json.loads(
        (tmp_path / ".dream-studio" / "audit.jsonl").read_text(encoding="utf-8").strip()
    )
