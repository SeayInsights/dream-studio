...
def test_unopenable_database_returns_false_as_characterized(tmp_path):
    """A path that is a DIRECTORY cannot be opened at all.

    The connect failure is the one remaining False that is not a missing table, and it
    is the same class of mistake one level up. Recorded as a characterization, not an
    endorsement: if this ever starts raising, that is an improvement and this test
    should be updated to expect it rather than deleted.
    """
    d = tmp_path / "not-a-db"
    d.mkdir()
    assert set_wo_artifact("wo-3", "review_verdict", "{}", db_path=d) is False
