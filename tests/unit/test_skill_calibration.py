...
from core.event_store.studio_db import get_skill_summaries, import_buffer
...
class TestSkillSummaries:
    """get_skill_summaries computes its rollup live from raw_skill_telemetry
    (via effective_skill_runs) — sum_skill_summary was dropped in migration 140
    as pure derived state. No separate rebuild step is needed."""
    ...
    def test_repeated_calls_are_stable(self, tmp_path):
        ...
        db = tmp_path / "t.db"
        assert len(get_skill_summaries(db_path=db)) == 1
        assert len(get_skill_summaries(db_path=db)) == 1
