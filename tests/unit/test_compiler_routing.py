...
def test_content_between_markers_is_generated_table():
    compile_pack = _get_compile_pack()
    pack = compile_pack()
    claude_md = pack["files"]["CLAUDE.md"]
...
def test_ds_project_row_contains_scope_and_resume_keywords():
    """The live routing table for ds-project must include both scope and resume."""
    compile_pack = _get_compile_pack()
    pack = compile_pack()
    claude_md = pack["files"]["CLAUDE.md"]
    begin_idx = claude_md.find("<!-- BEGIN AUTO-ROUTING -->")
    ...
    assert "scope" in between, "routing table missing 'scope' for ds-project"
    assert "resume" in between, "routing table missing 'resume' for ds-project"


def test_ds_project_resume_triggers_from_metadata_yml():
    """Live compiled routing table contains 'start building' from resume metadata.yml."""
    compile_pack = _get_compile_pack()
    pack = compile_pack()
    claude_md = pack["files"]["CLAUDE.md"]
    begin_idx = claude_md.find("<!-- BEGIN AUTO-ROUTING -->")
...
def test_meta_pack_workflow_keyword_in_live_compile():
    """Live compiled routing table contains 'workflow:' for the meta/workflow pack."""
    compile_pack = _get_compile_pack()
    pack = compile_pack()
    claude_md = pack["files"]["CLAUDE.md"]
    begin_idx = claude_md.find("<!-- BEGIN AUTO-ROUTING -->")
