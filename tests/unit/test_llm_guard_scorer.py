...
from guardrails.scanners.llm_guard_scorer import score_output_risk


def test_safe_output():
    """Test that safe output gets low risk score."""
    result = score_output_risk(
        output="The weather is sunny today.", prompt="What's the weather?", context={}
    )
...
def test_toxic_output():
    ...
    result = score_output_risk(
        output="You are an idiot and don't deserve help.", prompt="Can you help me?", context={}
    )
...
def test_block_recommendation():
    ...
    result = score_output_risk(
        output="Kill yourself, you worthless piece of garbage.", prompt="I need advice.", context={}
    )
