"""Unit tests for control.research.memory."""
...
def _index_conn(ms: MemorySearch) -> sqlite3.Connection:
...
def test_build_index_creates_db(mem_dir: Path) -> None:
    ...
    ms.build_index()
    assert ms.db_path == mem_dir / "memory.db"
    conn = _index_conn(ms)
    count = conn.execute("SELECT COUNT(*) FROM memory_meta").fetchone()[0]
    conn.close()
    assert count == 3, f"Expected 3 files indexed in isolated memory_meta, got {count}"
    ms.close()
