...
_LIB_DIR = Path(__file__).resolve().parents[2] / "control" / "execution" / "workflow"
if str(_LIB_DIR.parents[3]) not in sys.path:
    sys.path.insert(0, str(_LIB_DIR.parents[3]))

from control.context.handoff import HANDOFF_PCT, URGENT_PCT  # noqa: E402
...
class TestFileLock:
    ...
    def test_stale_lock_force_acquired_on_timeout(self, tmp_path: Path) -> None:
        ...
        with patch("control.execution.workflow.engine.time.monotonic", side_effect=fast_clock):
...
class TestCoerceQualityScore:
    def test_reads_score_from_file(self, tmp_path: Path) -> None:
        ...
        with patch("control.execution.workflow.engine.paths.meta_dir", return_value=tmp_path):
...
class TestCheckContextBudget:
    def test_no_context_file_returns_none(self, tmp_path: Path) -> None:
        with patch("control.execution.workflow.engine.paths.state_dir", return_value=tmp_path):
...
class TestComputeReadyNodesEdgeCases:
    def test_condition_exception_skips_node(self) -> None:
        ...
        with patch("control.execution.workflow.engine._evaluate", side_effect=RuntimeError("oops")):
