...
def test_natural_english_generation_claims_are_detected():
    """The regex was a fixed idiom list -- DO NOT EDIT, AUTO-GENERATED, GENERATED
    BY/FROM/FILE, a leading GENERATED, Regenerate with -- and three real, ordinary
    phrasings of the exact same claim evaded all of it: no AUTO- prefix, no
    GENERATED BY/FROM/FILE, no line-initial GENERATED. A worklist that only ever
    sees banner idioms it was already told about is not finding the next file
    phrased slightly differently -- it is re-confirming the ones it already knows."""
    from core.gates.generated_artifacts import _BANNER

    for banner in (
        # No "do not edit" wording on purpose: it would pass under the OLD
        # pattern too via the pre-existing `DO NOT EDIT` alternative, hiding
        # whether the new self-referential "IS A ... GENERATED" alternative is
        # what actually caught it.
        "This file is a generated artifact, produced from the packs registry.",
        "Machine-generated -- do not hand-edit.",
        "This file is produced automatically by a script; edits will not persist.",
    ):
        assert _BANNER.search(banner), f"missed a real banner: {banner!r}"


def test_passing_mention_of_generation_is_still_not_a_banner():
    """Broadening the detector for natural phrasing must not start flagging prose
    that merely discusses generated artifacts as a concept. CLAUDE.md classifies
    "product source, generated artifact, test fixture, local state" as file
    categories; file-structure-authority-policy.md observes that "a generated
    artifact in a canonical source location does not become canonical by placement
    alone" -- both about the concept, naming no specific file as itself generated,
    and neither in the self-referential "is a generated ..." shape a real banner
    uses."""
    from core.gates.generated_artifacts import _BANNER

    assert not _BANNER.search(
        "Classify what you are touching: product source, generated artifact, "
        "test fixture, local state, external target."
    )
    assert not _BANNER.search(
        "A generated artifact in a canonical source location does not become "
        "canonical by placement alone."
    )
    assert not _BANNER.search(
        "12+ checked-in artifacts are generated and one now has a drift check."
    )
