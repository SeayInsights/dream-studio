...
class TestRewrittenViews:
    def test_vw_activity_timeline_reads_canonical_events(self, tmp_path):
        ...
        assert "activity_log" not in row[0], (
            "vw_activity_timeline must not reference activity_log"
        )
...
class TestMigrationIntegrity:
    ...
    def test_migration_062_contains_canonical_events_insert(self):
        ...
        assert "INSERT OR IGNORE INTO CANONICAL_EVENTS" in sql, (
            "Migration 062 must contain the backfill INSERT into canonical_events"
        )
        assert "DROP TABLE IF EXISTS CANONICAL_EVENTS" not in sql, (
            "Migration 062 must not drop canonical_events"
        )
        assert "DROP TABLE CANONICAL_EVENTS" not in sql, (
            "Migration 062 must not drop canonical_events"
        )
    ...
    def test_migration_063_drops_activity_log_and_indexes(self):
        ...
        sql = sql_path.read_text(encoding="utf-8").upper()
        assert "DROP TABLE IF EXISTS ACTIVITY_LOG" in sql, (
            "Migration 063 must drop activity_log"
        )
        for idx in [
            "IDX_ACTIVITY_TYPE_TIME",
            "IDX_ACTIVITY_STREAM",
            ...
            assert f"DROP INDEX IF EXISTS {idx}" in sql, (
                f"Migration 063 must drop index {idx}"
            )
        assert "DROP TABLE IF EXISTS CANONICAL_EVENTS" not in sql, (
            "Migration 063 must not drop canonical_events"
        )
        assert "DROP TABLE IF EXISTS HOOK_EXECUTIONS" not in sql, (
