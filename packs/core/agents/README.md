...
Agent files are referenced by name in `SKILL.md` files via the `subagent_type` field (e.g., `ds:director`). The filename (without `.md`) is the subagent type identifier.
