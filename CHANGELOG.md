...
- `test_close_proceeds_on_unreviewable_grader` in `tests/unit/test_wo_verify.py`: mocks `_collect_grader` to return no-summary; asserts `close_work_order` returns `ok=True` without `force=True` and the verdict JSON records `unreviewable_graders` (WO-VERIFY-NOSUMMARY, PR #341).
