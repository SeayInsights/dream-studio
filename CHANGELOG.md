...
### Fixed
- `ds eval queue show` and `ds eval queue run` now filter on `pending_rerun = 1` instead of `friction_flag = 1` (WO d1f3e656). `pending_rerun` is the explicit queue-membership flag; `friction_flag` may remain set on targets whose re-run failed. `queue show` SELECT also surfaces the `pending_rerun` column.
