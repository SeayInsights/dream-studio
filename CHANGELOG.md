...
### Added
- **Two blocking pre-push gates for claims nothing checks (WO-GITIGNORE-PHANTOM `bab28970`, WO-EVIDENCE-BACKED-OUTPUT `f7af0888`):** both enforce one principle from opposite ends — a thing asserted must be a thing verifiable. **`gitignore-phantom`** refuses a push whose changed files read or assert a path git will not ship. Green locally, where the file is on disk; broken on a fresh checkout for everyone else. Observed twice before this existed: nine `verify_*.py` split siblings untracked under the `.gitignore` rule `verify_*.py`, so a clean clone raised `ModuleNotFoundError`; and a review-rules profile authored under `.dream-studio/`, which `.gitignore` excludes as private runtime state, asserted by a committed test that passed locally. Both were found by someone remembering to look — `git ls-files` and `git check-ignore` answer it exactly, so it belongs in a gate. It caught two defects in itself: `candidate.relative_to(root)` raised for a relative root, so the loop skipped **every** literal and the gate reported OK on the exact case it was built for — all twelve fixture tests passed against that, because each hands it an absolute `tmp_path`, and only a run against the real repo exposed it; and `assert Path(x).is_file()` matched both the read-call and assert shapes, so one defect printed twice. 15 tests in `tests/unit/test_gitignore_phantom.py`. **`evidence-backed-output`** implements the operator ruling that anything pushed to GitHub must be backed by evidence, show it, and be verified, so no reader is guessing. A claim is backed when it carries a command and its output, a test count with its unit, a `file.py:line`, a commit sha, a run id, or a named artifact. Measured when written: all four PR bodies authored that day carried `- [x] PR smoke is expected to pass` — a forecast inside a **checked** box — and three of those pull requests were already merged. Its first citation window accepted the **preceding** line, which made it nearly useless: in a document carrying 46 citations almost every hedge has a cited neighbour, so an unrelated citation laundered the claim beside it; now the claim's own line or an output block beneath it. It audits what a push actually publishes — commit messages and added `CHANGELOG.md` lines — because a PR body is not in the repository, and the manifest entry says so rather than claiming a reach it lacks. A hedge inside a quotation is exempt, since quoting the offending line is how a document explains the rule; multi-line quotation turned out to be the normal case, because commit messages wrap at 72 characters. That exemption is a real evasion hole and is asserted as a test rather than buried in a docstring. 16 tests in `tests/unit/test_evidence_backed_output.py`. Both wired into `canonical/workflows/pre-push.yaml` (23 gates), with a test that reads the manifest the runner actually loads — a test inspecting a different file can pass while the gate never fires. The standard is also written into `canonical/skills/core/git.md` where PR bodies are authored, so it is followed by default rather than caught after the fact.

### Fixed
- **A locked authority is not a missing artifact table (WO-ARTIFACT-LOCK-FALLBACK `fd981a32`, fdadbf1b/f272e251/f3c69ceb/be6a0792):** measured on the live authority 2026-09-02 — of August's review verdicts, **154 landed in `.planning` and 23 in the authority**; `SELECT COUNT(*) FROM business_work_order_artifacts WHERE kind='review_verdict'` returned **24** while `.planning/work-orders/*/review-verdict.json` held **170**, of which **162** existed nowhere else. Every one was invisible to the `independent_review` close gate and to `ds project state`, which read the authority only. **The disk fallback was not the bug** — it is deliberate, for an authority whose artifact migration is unreleased. The bug is that `set_wo_artifact` returned `False` for **every** `sqlite3.OperationalError`, and all five artifact writes in verify run inside an already-open `with _connect(db_path)` (four `_persist_review_verdict` sites plus `_persist_unverified_ledger`) — so a second connection to that file got `database is locked`, returned `False`, and the caller read that as "the artifact table does not exist". `_persist_unverified_ledger`'s own docstring already named this defect and worked around it by writing to disk. `set_wo_artifact` now takes an optional `conn` so a write joins the transaction the caller already holds, `False` means only **a schema older than the write**, and every other `OperationalError` raises. **Two defects found by the tests rather than by reading:** narrowing to `no such table` alone broke a migration-144-without-152 authority, where SQLite words it `table business_work_order_artifacts has no column named instance_key` — raising there turns a database that merely needs `ds migrate activate` into a crash mid-verify; and `tests/unit/test_wo_verify.py:141` asserted the verdict was **on disk**, which passed only *because* the authority write was failing, so it now reads through `_artifact_with_envelope`, the same dual-store reader `core/work_orders/close_gates.py:385` uses. **A fallback that does fire is now counted** through `record_artifact_fallback` (rule `artifact_disk_fallback`, via `record_observation`, so `observations_report` groups it with no new event type) — verified end to end on the live authority: `artifact_disk_fallback: 2 -> 5` after `sync_tick()`. Firing 154 times without a countable trace is why this survived a month; the unit test for it asserts only that the call happened, and paired with it is an integration test running the real spool → `sync_tick` → report path, because the call-level test passed while `db_path` was unthreaded and the count was landing in the **wrong** authority. **Four callers discarded the `False`,** each with a deferred invisible cost: `carry_over` reported `ok: True` when the split record did not land, leaving the original permanently unclosable with its tasks already moved; `record_exception` returned `False` while both callers catch only `ValueError`, so `--accept-structure` printed success and stored nothing; `record_commit_ownership` returned `[]`, indistinguishable from "this task produced no commits", so the range stayed wide; and `close_work_order` held the boundary-end pin and the ownership record in **one** `except Exception: pass`, so a pin that raised skipped the ownership record entirely and neither failure reached the operator — despite the comment above them promising a boundary that cannot be pinned "keeps its open range AND says so". **And the recovery had no caller:** `backfill_wo_artifacts` has existed since WO-FILESDB-P1 invoked only from `tests/unit/test_wo_artifacts_db.py:154`, so the repair for what had already fallen through was unreachable — the `reachability` gate would refuse it today. New `ds work-order backfill-artifacts [--planning-root <path>] [--dry-run]` wraps it and exits 1 on zero written, because "nothing to do" and "the artifact table is absent so every write no-op'd" print the same count. Run against the live authority it wrote **171 artifacts**, taking `review_verdict` rows **24 → 186** with **0** disk-only verdicts remaining. **12 tests** in `tests/unit/test_artifact_lock_fallback.py`, **1** in `tests/integration/test_enforcement_tiers.py`, and **4** across `tests/unit/test_wo_artifact_cli.py`, `tests/unit/test_wo_lifecycle_surface.py` and `tests/unit/test_delivery_boundary.py`.
- **Agent output can no longer become shell syntax in a completion check (WO `e4e85949` tasks `52f2c484` / `f0dce86f`):** `completion_check` supports `{{node.field}}` templates so a check can name what it is checking — "does the PR exist" needs the PR number the previous node printed — and it is then run with `shell=True` at `control/execution/workflow/runner.py`. The resolved values are prior nodes' output: agent-generated text, or whatever a command node captured. They went into that string **raw**, so a value containing `; rm -rf ~`, `$(...)` or backticks became shell syntax in a command the runner executes unattended during an `--execute` run. `resolve_templates` now takes a `transform` and `_verify_completion` passes `shlex.quote`, so each substituted value is exactly one argument — asserted by tokenising the result and requiring the whole hostile payload to survive as `tokens[3]`, because a string comparison would pass on a payload that had merely been rearranged. **Two things are deliberately not quoted:** the template around the values, which is authored in the repo and whose pipes are intended (every real check pipes into `grep`, and quoting the whole string breaks all of them); and `completion_contains`, which is compared as a substring against the check's output and never executed, so quoting it would add literal quote characters to the thing being matched. `transform` defaults to none because the PROMPT path resolves the same templates and wants raw text. An unresolved reference stays literal rather than being quoted, since quoting `{{a.b}}` would mask a broken reference as a value that had resolved. No shipped `completion_check` uses a template today — the templates in `execute-work-orders.yaml` are all in `condition:`, a different evaluator — so this closes the path the feature's own docstring invites rather than one already in use. **59 tests** in `tests/unit/test_workflow_runner.py`.

- **Commit ownership survives the squash that makes the work permanent (WO `e439f287` / `80c0e61b`, b0523f30/a2172a92/af112858):** `record_commit_ownership` records BRANCH shas and this repo squash-merges every PR, so the moment a branch lands, the commits a work order claimed stop being ancestors of HEAD — `git merge-base --is-ancestor 4a5221cf main` returns non-zero because #687 squashed 18 commits into `14b8693c`. The attribution shipped in #689 was therefore correct for in-flight work and **inert for merged work**, silently. **The mapping is read, not inferred:** GitHub's squash writes each original commit's subject into the merge body as a `* <subject>` bullet, so `resolve_squashed` finds the commit that absorbed a given one by searching for its own recorded subject — verified on the real case, all three of `1db6de49`'s commits resolve to `14b8693c` with none lost, narrowing its graded range from 10 commits to 3. The consequence that mattered is on the **exclusion** side: a neighbour's claim evaporated the moment their branch merged, so a range with neighbours read identically to one with none. **Ambiguity refuses to answer** — two merge commits carrying the same subject bullet cannot both be it, so `resolve_squashed` returns `None` and the commit is reported unmappable rather than guessed. Two correctness violations an independent review found in the first cut are fixed in the two commits after it: `_squash_aware_index` dropped a neighbour's unreachable-and-unmappable commits on the floor while `attribute_range` reported "none of the N commits is recorded as belonging to another work order" — an absence it had not established, and the same diff's `reachable_ownership` insisted on naming exactly that distinction for the work order's own side. Candid on one side and silent on the other is worse than either, because the silence is invisible beside the candour. A `Dockerfile.test-suite` bundled into the attribution commit was also removed from this branch's net diff — a test harness and a verify-plane change that cannot be reverted independently.

- **The spool guard names the test that polluted (WO `efe2ce9d`, e5517119):** the autouse guard aborts the whole session when a test writes to the operator's real `~/.dream-studio/events` — correctly, that is its job — but it said only `Test modified real ~/.dream-studio/events`, and a session that ends at test 2,400 of 5,928 with no identifier leaves the reader to bisect by hand. Its teardown already knew which test it was; the identifier simply was not in the message, so taking `request` and interpolating `request.node.nodeid` is the whole change. Verified out-of-band with a throwaway probe that really does write to the real spool: `FATAL: tests/unit/test_zz_guard_probe.py::test_this_one_pollutes_the_real_spool modified real ~/.dream-studio/events`; the probe was then removed, and the permanent test asserts against `tests/conftest.py`'s source, because a test proving this by polluting for real would be the very thing the guard exists to stop. The task's acceptance criterion was **repointed** — it named `TestSetupReadOnly::test_canonical_setup_check`, the SUSPECT that pollutes rather than the FIX, and that test passes standalone whether or not the guard names anything, so it could never have failed for the reason the task exists.

- **The orchestrator's nodes are observed from outside, and its rules run (WO `1db6de49` / `e4e85949`, 7f0d9c19/dd167f65→52c5faf5/01b00950/f73c1f91/ea24ee71):** 6 of `execute-work-orders`' 14 nodes declare a `completion_check` that observes state OUTSIDE the runner and the other 8 record why they cannot. Run against this repo the loop reports `implement-tasks blocked — 15 task(s) still pending on the active work order` and stops there, instead of marching to "completed". **Every check was run before it was written, because the previous three attempts were not:** the first declared `completion_contains` tokens the runner can never see (`_invoke_skill` loads a prompt and never holds the agent's output); the second invented `ds work-order tasks-remaining --active --quiet`, which does not exist; and the third asked `ds project state` for the substring `"pending_tasks": 0,` — which appears **30 times** in that output — replaced by `scripts/active_wo_tasks_complete.py` reading the scoped field. **An executing node cannot push:** `remote_head` is read before and after execution and the node fails with the violation named if the remote-tracking ref moved, because telling an agent not to push is prose. **Retry budget 2, then diagnose — never move on:** on exhaustion a fresh reviewer is invoked rather than another execution attempt, walking the 8-class falsification taxonomy in `core/work_orders/scenario_taxonomy.py` by name, and the diagnosis is registered in the authority via `prescribe()` — one finding becomes a task, several a work order. **And the rules themselves were prose until `ea24ee71`:** six predicates existed in `control/execution/workflow/autonomy.py`, a test asserted each fires on its own case and passed, while `evaluate_operator_rules` had **no call site** — only `stance_brief()` was wired, handing the rules' TEXT to a reviewing agent. That is the exact substitution they were written to prevent, in a module whose second line is GATES, NOT PROSE; the `reachability` gate refused the push and was right. They now run in `_execute_wave` where a node's status is decided, and a violation on a node that reported `completed` makes it `blocked`. Wiring them exposed that all 8 unobservable nodes would have been flipped to blocked at once: each carried its reason as a YAML **comment**, which `yaml.safe_load` drops, so `absence_is_not_clean` could not see a single stated reason — and the test guarding that gap searched the raw file text for the comment and passed throughout. The reason is a `completion_unobservable` field now, refused under 12 characters, the same shape as `--accept-structure` at close. Scoped to `--execute` runs, because a dry run marks nodes completed having executed nothing: judging those flagged every node and broke `test_dry_run_marks_nodes_completed` immediately. **57 tests** in `tests/unit/test_workflow_runner.py`.


- **A generated projection no longer destroys hand-written CLAUDE.md content (WO-CLAUDEMD-CLOBBER, 3e6cf265):** operator report -- "when it builds it rewrites all of claude.md." Measured 2026-08-21: `~/.claude/CLAUDE.md`, the operator's **global personal instruction file for every project**, was a 101-line full generated copy written by the installer, with no protected region -- anything hand-written there was destroyed by `ds integrate install` or `ds doctor --fix`. The installer emitted `op="create"` carrying the whole generated file, and the apply loop `atomic_write`s anything with `source_content`; the `op` label never reaches the write. **The correct contract already existed and the installer bypassed it** -- `generate_routing.update_claude_md` replaces only the span between the two `AUTO-ROUTING` markers and refuses a file that has neither. That contract now lives in `integrations/compiler/claude_code.py::merge_claude_md`, which the installer already imported from, and follows the `settings.json` precedent exactly: **merge at plan time, write whole at apply time** (`merge_settings` has always done this), so the write path needed no change. Three outcomes -- no file yet writes the projection whole (nothing to lose, and the generated content carries the markers, so the file is self-maintaining after); both markers present splices **only** that span, every byte outside preserved; neither marker (or one, or reversed order) **refuses**, emitting `op="skip"` with a reason naming the two markers to add. Appending to an unmarked file would also preserve content and was considered, but an unmarked file may already hold a hand-pasted copy of an older projection, and appending would silently install a second conflicting set of instructions -- refusing is the only option that cannot make the operator's instruction file wrong. The repo-root 205-line `CLAUDE.md` previously survived only because a *different* code path happened to refuse it; that was an accident of routing, and is now a rule. 11 tests in `tests/unit/test_claude_md_projection.py`, driving `ClaudeCodeInstaller.install("execute")` rather than the helper alone -- a helper-only test would pass with the installer still bypassing it, which *is* the defect -- and asserting the content **outside** the markers byte for byte, because the old writer updated the generated region correctly and destroyed everything around it. One test asserts the converse, since a writer that preserves everything by writing nothing would pass every preservation test. The old `safety_notes` claimed "Existing file is backed up before write"; a claim in a string is not a backup, so the backup is now asserted on disk, holding the pre-write bytes, and named in the on-disk manifest.
- **The verify plane no longer creates work faster than it can be closed (WO-GAP-FANOUT, 736e6127):** operator report — "as we do work something else always gets surfaced by the review agent ... that is why we can't get through the work that is scheduled." Measured on the live authority: **118 work orders created 08-19..08-21 against 43 closed**, and 19 of the 25 auto-spawns were the *same two findings* (×11 "Add missing adversarial tests…", ×8 "Add missing test coverage"). `_gap_key` returned `{reviewed_work_order_id}::{category}`, so a generic finding got a different key on every reviewed work order — the eleven duplicates carried eleven distinct keys, identical after the `::`. Dedup worked exactly as written; the key was one field too specific. A prior fix (`_ADVISORY_PROJECT_WIDE_CATEGORIES`) held **one** entry while two unlisted phrasings fanned out, so the list is extended **and** backed by a rule needing no prediction: a category with ≥2 open spawns across different work orders becomes project-wide whatever it is called. **Gaps now belong to the work order they were found in** — five of ten open spawns were findings *about the work order under review*, and spawning a sibling declared it complete while routing around the `tasks_done` gate; a gap on an open, incomplete work order becomes a task on it, and only a closed one spawns a sibling. Attaching requires the verdict to say the work is **not** done, because a passing verdict's advisory gap must not block the close it just approved. The attach loop is bounded and visible: attached tasks carry their gap key, and after three distinct rounds the verdict, the verify CLI and close all say so and name carry-over. **An unreviewable finding is not work** — the correctness grader reported "independent review unverifiable — no diff provided" and the spawner turned that into work order `58e21003` with the task "Fix N/A: independent review unverifiable — no diff provided in N/A"; a violation naming no locatable rule *or* file is the grader reporting its own inability. Queue drained 126 → 116 via `ds work-order drain-gaps <project>` (preview by default, `--apply` to cancel, idempotent, records what absorbed each duplicate) — which shipped in the first push attempt with **no call site at all**, described in its own docstring as "the repeatable replacement for a one-off script" while reachable only from its tests. The `reachability` gate added earlier in this milestone refused the push for exactly that reason and was right. 29 tests in `tests/unit/test_gap_fanout.py`.

- **Fan out across milestones: dependencies constrain, milestone order informs (WO-WO-LIFECYCLE-SURFACE, 877af544):** operator — "why can't we utilize a fan out approach across milestones or work orders that are separate. walking down a straight line does not always make sense." The straight line was **imposed, not derived**: the dependency graph was already correct and already honoured (88 edges via `NOT EXISTS (... dep_wo.status != 'closed')`), and two things sat on top of it. `start_work_order` **refused** while any earlier milestone had open work — it refused to let the work order that removes the constraint start ("17 work order(s) in earlier milestones are incomplete"). And `get_next_work_order` filtered to `m.order_index = (SELECT MIN(...))` then `LIMIT 1`, so 45 open milestones produced one answer. Both gone: a declared dependency blocks (new `blocking_dependency_count`), and out-of-milestone-order work is reported as advisory `out_of_milestone_order` naming `ds work-order add-dep`. New `ready_work_orders()` returns the **set** across all milestones with in-progress entries marked — measured after the change: **43 work orders across 6 milestones, 3 already in progress**, where one was visible before. It reports what it does not know: all 43 have no declared dependencies, so each reads "unblocked; no declared dependencies — **independence NOT verified**". Overlapping declared `Module boundary:` clauses are flagged advisory (parsed by the same `runtime.lib.enforcement.boundary_globs` the on-edit hook and verify share — there is no `module_boundary` column). Each entry also says whether its deliverable is **in this checkout**, distinguishing "not written yet" from "work exists on another branch", because *finished* and *closable from where you are standing* are different facts. 10 tests in `tests/unit/test_wo_lifecycle_surface.py`.

- **A project resolves to every root its code lives in (WO-MULTIROOT-REVIEW, 0a7abdfe, tasks 1–2):** operator — "one folder is not feasible or reasonable for actual development." `business_projects.project_path` was a single column read by a resolver returning a single `Path`. Fulcrum's declared path has **no `.git` of its own**: it is a working folder holding 6 repositories and 29 worktrees. `_collect_git_commits` returned `None` there and found commits in `Fulcrum/platform` — so **all 28 open Fulcrum work orders were graded with no diff at all**, which is precisely what produced the `Fix N/A in N/A` work order. The single-root model, the "reviews against Dream Studio" complaint, and that nonsense work order were **one defect with three symptoms**. New `core/work_orders/project_roots.py` resolves declared *and* discovered roots; `resolve_project_root` delegates to it, so all six existing call sites benefit. **A worktree is not a repository** — a repository has a `.git` DIRECTORY, a worktree a `.git` FILE reading `gitdir: <repo>/.git/worktrees/<name>` — and conflating them turned 6 grading targets into 35; worktrees are recorded as *checkouts* mapped to their repository. **A root need not be a repository at all**, because not everything is pushed to GitHub; `versioned` says which so a caller can pick an evidence strategy instead of being handed nothing. A container root is **described** ("not itself a repository; it holds 6 repositories: …") rather than returned as `None`, since `None` is what became "Fix N/A in N/A". Single-repo projects verified unchanged against the live authority.

- **Nine defects caught in the above, seven of them by reading the diff rather than running it — the suite was green through all seven.** The severe ones: attached gap tasks were written with **no canonical event** while `business_tasks` is a projection whose default `pre_rebuild` does `DELETE FROM business_tasks`, so a rebuild would have permanently deleted every attached task and its work order's remaining work; the unreviewable filter **never worked** because the live grader writes the sentinel as the start of a *sentence* (`rule = "N/A: independent review unverifiable - no diff provided"`) and the test used a simplified value that had been *invented* rather than read from the stored artifact; and `primary` returning `None` for a container sent every caller through `or source_root` to grade Fulcrum **against the Dream Studio repo** — the original complaint, reintroduced by the fix meant to avoid guessing. Also: a closed project-wide tracker silently swallowed new findings of its class; `attachment_pressure` was computed, stored and printed nowhere; and the verify CLI announced "Gap work orders created" for gaps that were attached, naming work orders that did not exist.

### Changed
- **main-CI status is cacheable, opt-in, and never looks live (WO-MAINCI-CACHE, c14c2eea):** WO-MAINRED-VISIBILITY put main's post-merge verdict in `ds doctor` and `ds project state` — the right surface, since main sat red across eight merges because nothing volunteered it — but charged a `gh` round trip to commands that are otherwise local, on every invocation, with `ds project state` running on every resume. Typical ~1s; worst case the full 25s timeout, which is what an offline or unauthenticated operator paid *every time*. `main_ci_status` now accepts `max_age_seconds`, backed by `ds_config` through the existing `core/runtime_state.py` helpers (no new table, no migration, no disk file). **Caching is opt-in and the default is a live read:** the first cut defaulted it on and broke ten existing tests, because they call the reader with no `db_path` and so consulted the operator's *real* authority DB and were served a row an earlier real invocation had written — a default-on cache silently turns a stateless reader into one that reads shared global state, so any caller can be handed an answer produced by an unrelated earlier call. `ds doctor` and `ds project state` pass `CACHE_MAX_AGE_SECONDS` explicitly; `close` passes `0` because declaring work done must not be told about main by a cache. The key is repo-scoped (one shared row would serve repo A's status for repo B — a wrong answer indistinguishable from a right one, quality rule 7's class). Every return carries `as_of` and `age_seconds`, and the operator warning states its age, so a four-minute-old red cannot read as current. `unknown` is cached deliberately — an unreachable `gh` is the most expensive case. Every unusable cache shape (absent table, malformed payload, wrong types, unparseable *or future-dated* timestamp, unavailable DB) is a miss that falls through to a live read and never raises — WO-MAINRED-GH-NONSTR's lesson applied while writing the code rather than after main went red. New `clear_main_ci_cache()`; 13 tests in `tests/unit/test_main_ci_cache.py`.

### Added
- **A blocking gate for code that arrives with no caller (WO-DEAD-ON-ARRIVAL, 52a5951e):** five verify findings across 2026-08-19/21 were one shape — a mechanism written, its wiring omitted, and the mechanism then described as the outcome (`merge_readiness`/`record_merge_override` with no call site; `declare_reviewed_no_change` documented as an operator remedy while existing only as a function; `currency_evidence`, still zero callers; `_envelope_required` claiming three gates while touching one). Every one was caught by an LLM grader *after* the code shipped, and "is this name referenced in production code" is a **computation** — the first instance of the operator directive that anything verifiable deterministically should be. **Not a vulture threshold change:** `core/gates/leanness.py` already runs vulture at 80% advisory and printed "dead symbols: 8" on ~26 consecutive pushes while those functions sat unreachable at 60%; at 60 it also flags `currency_failure`, which IS called via a lazy import inside a function body, and DS uses lazy imports deliberately (a module-level import freezes the reference and silently defeats `patch(...)`), so tightening would punish an idiom the codebase requires. New `core/gates/reachability.py` is instead **diff-scoped** (only symbols this change set adds, so the 387 pre-existing advisory findings stay advisory) and **reference-based over AST identifier uses** (Name/Attribute at any depth, import aliases, and string constants such as `__all__` entries all count — making the lazy-import idiom safe by construction). Module-level publics only; decorated definitions skipped, since a decorator IS the registration mechanism. Blocking in `pre-push.yaml` with an inline `# reachability-gate: intentional: <reason>` escape the gate prints on every run. **Two defects it found in itself, both by being run rather than read:** the first cut was a text search and its own docstring — which names `currency_evidence()` as an example of dead code — was counted as a call site, so it reported the corpus clean while the symbol it cited sat unreferenced (a grep cannot tell a mention from a use, the exact substitution the gate exists to stop); and `git diff` never reports an untracked file, so the first dry run said "nothing to check" while two brand-new modules sat in the tree, the files most likely to hold an uncalled mechanism. Proven end-to-end: a throwaway unreferenced function in `core/gates/` exits 1 with the symbol named and exits 0 once removed. 22 tests in `tests/unit/test_reachability_gate.py`.

### Fixed
- **The stop hook can see the authority writes that actually happened (WO-HOOK-WRITE-ACCOUNTING, 4703cc38):** the enforcement Stop hook blocked sessions that had complied, two ways. `authority_write_since` asked `business_tasks` for `status = 'done'` while `ds work-order task-done` writes `'complete'` — 2,058 complete rows against 27 legacy `done` (last written 2026-06-29), so that fallback had matched nothing for months, and four consecutive successful task-done calls on befde290 were reported as "no authority write was recorded this session". Both literals are accepted now, not one swapped for the other. And `ds work-order affirm-impact` stores an `impact_affirmation` artifact and satisfies the `change_impact_affirmed` close gate, but did not count — so a session whose only honest remaining write was an affirmation had **no truthful way** to satisfy the hook (every task complete, close blocked on a gate); hit on 758fbedd, c14c2eea, 66e7ebc8. Deliberately narrow: a review verdict is evidence *about* work, not a record that work was completed, and a test pins that it still does not count, alongside in-window negative cases for every widened path (non-`complete` task statuses, non-counting event types, non-counting artifact kinds). Durable row state is now the primary signal with canonical events demoted to reinforcement, because events arrive only once spool ingestion runs — the old order made the check partly measure "did ingestion keep up" rather than "did the operator record work". The hook fails SAFE, so nothing false-done got through; the cost of a false violation is that it teaches operators toward `DS_ENFORCE=0`. Drift cannot silently return: the accepted-status set is asserted against what the **real writer** produces (`TaskProjection` handling `task.completed`), since this module is copied verbatim into the hook trees and may use only the stdlib, so it cannot import a shared constant from `core`. 18 tests in `tests/unit/test_hook_write_accounting.py`.
- **Tests and evals are certified by someone other than their author, and the substrate says so (WO-SEPARATE-TEST-RUNNER, ee03d360 + gap e3a17189):** operator directive, plus the correction that shaped it — a rule kept in operator-local agent memory binds one install and does nothing on anyone else's, and Dream Studio must work out of the box for every user. The rule now ships in `canonical/skills` (core `build`/`verify`, ds-workorder `execute`/`close`) and in `dist/plugin`. Made checkable rather than merely stated: every TEST-CHECK result records whether it **ran** (`executed`, plus a reason on the timeout / missing-command / unparseable paths); the verdict carries `test_execution.basis` (`executed` | `not_run_at_verify` | `none_registered`); and one shared builder (`core/gates/merge_readiness.py::execution_caveat`) states the caveat at `ds work-order merge-check`, at close (`test_execution_warning`, advisory, never blocks) and in `ds project state`. Close **measures** execution rather than assuming it — the count of TEST-CHECKs the always-on AC gate actually ran — after two wrong versions: asserting it unconditionally (a forced close bypasses the gate and would have gone silent), then inferring it from the WO type's `all_tests_pass` gate, which most types do not list and which printed a *false* caveat on this feature's own clean close. `all_tests_pass` executing rather than trusting a report is now pinned, including that a stored artifact reading "PASSED" cannot satisfy it. The measured reason it exists: fourteen self-run tests passed against an invented review-verdict shape while the code under them softened every genuine failure into "re-run verify". 29 tests in `tests/unit/test_separate_test_runner.py`.
- **The advisory main-CI reader can no longer raise into its caller (WO-MAINRED-GH-NONSTR, 758fbedd):** `main` went red on two runs (32302685616, 32302680563) on one test out of 5,386 — `test_doctor_fix_calls_install_when_skills_missing`, with `TypeError: the JSON object must be str, bytes or bytearray, not MagicMock`. WO-MAINRED-VISIBILITY had added `checks.main_ci` to `ds doctor`, so `_doctor_status` now shells out to `gh`; that test blanket-patches `subprocess.run` with `MagicMock(returncode=0)` — a legitimate shape for a test about the **install** call — so `.stdout` is a truthy non-string and `stdout or "[]"` handed it to `json.loads`. `TypeError` is neither a `JSONDecodeError` nor a `ValueError`, so the guard never saw it. The engine was wrong, not the test: this reader's documented contract is that an unavailable / unauthenticated / timed-out / non-JSON / empty `gh` reply yields `status="unknown"` **with a reason** and `red=False`, and it is an **advisory** check documented as never blocking a close, never failing the doctor, and never altering a gate outcome — so an escaping exception violates that regardless of which `except` clause was listed. `_gh_json` now normalises both streams to text before reading them, validates `returncode`, and catches `TypeError` alongside the rest. Empty output stays distinct from unreadable output (`""` means "gh answered, no runs"; a non-string means "we could not read the answer"). Tests assert the class rather than this one mock shape: bytes stdout, bytes stderr on a failure exit, a pre-parsed list, `None`, and a non-int `returncode` each return a well-formed unknown with `main_ci_warning` staying silent.
- **Every subprocess read names a codec; a new blocking gate keeps it that way (WO-LOCALE-DECODE-SILENT-LOSS, d771b2da):** `subprocess(..., text=True)` without `encoding=` decodes the child with `locale.getpreferredencoding(False)` — cp1252 on Windows — and one byte cp1252 leaves unmapped (`0x81/0x8D/0x8F/0x90/0x9D`, which are UTF-8 continuation bytes of the cross mark `U+274C`, the left arrow `U+2190`, and the emoji variation selector `U+FE0F`, all present in this repo's own gate output; 53 files contain one) kills subprocess's reader **thread**. Measured on win32/CPython 3.12.8, the result is neither a crash nor a hang: `run()` returns in 0.08s with `returncode=0` and `stdout=None`, `check_output()` returns `None` without raising, and the thread's traceback lands on the parent's stderr where a git hook swallows it. Found when a push printed a bare reader-thread traceback and no gate output at all — `core/gates/pre_push.py` did `completed.stdout or ""`, so the failing gate's diagnostic was empty while its exit code still carried the verdict; **no gate verdict ever flipped, the reason did**. Audited per consumer rather than assumed: `verify_executor` decides on `returncode` (verdict safe, evidence blank); `rubric_immutability_gate` / `test_fixture_resurrection_guard` / `migration_risk` / `leanness` raise on the `None` and fail closed with a misleading traceback; `main_ci` already degraded to unknown-with-reason; `secret_scan` was never affected (it already named a codec). An AST sweep — not grep, since `text=True` occurs in strings and docstrings and a multi-line call has no single line to match — found 118 sites lacking `encoding=` (62 product across 37 files, 56 in tests) against 19 already compliant; all 137 now pass `utf-8` with `errors="replace"`. **The edits are the cleanup; the gate is the fix:** new `core/gates/locale_decode_gate.py` blocks in `pre-push.yaml` and its tests run on all three platforms via the pr-smoke focused list. Its escape hatch (needed by its own test, which must write the defect to prove it) is an inline `# locale-decode-gate: intentional` marker the gate **prints on every run** — an exemption nobody can see is the same shape as the defect it exempts.
- **A failing gate shows both streams, so a failure arrives with its reason (WO-LOCALE-DECODE-SILENT-LOSS task 6):** `format_report` used `if stderr_tail ... elif stdout_tail`, so any stderr content suppressed stdout entirely. For a pytest gate that is backwards — the failing assertion is on stdout while stderr carries only an unrelated `RequestsDependencyWarning`. A `pin-tests` failure therefore printed its hint and nothing else, twice on one branch, and the only way to learn which pin had drifted was to re-run the gate by hand.
- **The falsification stage is hardened against the worst cases it named in itself (gap WOs 66e7ebc8 + 72b19987):** the analyst read the diff of the falsification stage and named five error-severity scenarios that were testable but untested; all five were real defects. (1) `read_unverified_ledger` was authority-first, so a run that reached the authority followed by one that fell back to disk (the documented `fd981a32` lock case) served the **stale** ledger — now newest-wins by envelope `created_at` vs disk mtime. (2) A grader returning prose instead of scenario objects raised `AttributeError` inside verify's **open authority transaction**; non-dict entries are now skipped and the skipped count is **reported**, so a malformed reply narrows the enumeration visibly. (3) Remediation evidence is appended after the WO's commits, so a newest-first diff budget could spend the whole budget on evidence and hand the analyst a change set whose own diff it never saw — commits are budgeted first, original order restored for the reader. (4) The truncation caveat lived only in the verdict, so close and `ds project state` reported a PARTIAL enumeration as complete — the ledger now carries it and close says the list may be incomplete. (5) The ledger carries the run's `verified_at`, making a ledger paired with an earlier run's verdict detectable, since the two-store write window cannot be closed without a cross-store transaction.

### Changed
- **Independent review is default-on at close; grader gains adversarial rules 7–8 (WO-GRADER-ADVERSARIAL, f076f130):** previously auto-verify and the `independent_review` gate ran only for WO types whose post-gate named it — `api_endpoint`/`ui_*`/`saas_feature`/`data_pipeline` WOs closed with zero independent review. Close now runs verify for every non-`documentation` WO and enforces the gate; `--skip-verify` opts out and is **recorded** as a `gate.bypassed` event. The quality rubric gains rule 7 `DURABLE_STATE_ADVERSARIAL` (durable state a read path trusts must carry a crash/race/version-skew test; a silent persist-key vs read-key mismatch is an error — the gw#619/#535 class) and rule 8 `CONFIG_AS_PROXY` (a secret gated on a config signal while a different knob controls actual reachability is an error — trace what the secret is valid *against*; the 2026-08-18 Fulcrum magic-link class). Test suite runs hermetically via a conftest `DREAM_STUDIO_VERIFY_MOCK` default (real-grader tests pop it explicitly). ds-core build/verify skill texts carry the adversarial-first discipline.
- **Verify resolves gaps against the authority; every enforcement bypass leaves a queryable mark (WO-VERIFY-GAP-RESOLUTION #650, gap WO d6e7b4c0 #651, WO-BYPASS-TELEMETRY #652, gap WO 08298665 #653):** a coverage gap whose spawned remediation WO is `closed` (exactly closed — blocked/cancelled never discount) resolves the original verdict with a `resolved_gaps` annotation instead of failing it forever. DS_ENFORCE=0 short-circuits, enforcement fail-opens (lib import failure, missing/unqueryable authority DB), `MIGRATION_RISK_ACKNOWLEDGED`, and `Docs-Reviewed-No-Change` trailer consumption all emit recorded events (hatches still work — invisibility was the defect); `ds doctor` gains `checks.bypass_audit` and `ds project state` gains `bypass_summary` (additive).
...
- **Work-order close is now report-only — no auto-start pile-up (WO-CLOSE-REPORT-ONLY, 34e79ccb):** `core/work_orders/close.py::close_work_order` auto-started the next/remediation WO on every close, piling up dangling `in_progress` WOs during directed (non-autonomous) sessions. Close is now **report-only**: it still advertises `next_work_order` (the `get_next_work_order` ready-set pick — startable, cross-milestone-correct), `next_command`, `next_block`, `gaps_block`, and `spawned_work_orders`, but no longer calls `start_work_order`; the `auto_started`/`auto_start_error`/`auto_start_message` result keys are removed. The autonomous `execute-work-orders` workflow previously relied on the engine auto-start to have started the next WO before its (start-less) `implement-tasks` node — its `next-iteration` node now runs `ds work-order start <next_id>` explicitly before re-invoking, so the loop still chains without the hidden side effect. `ds-workorder` close/main SKILL.md contract updated to report-only (interactive close surfaces `next_command` and stops; only the workflow chains).
- `tests/unit/test_work_order_close_exec_loop.py` + `tests/integration/test_close_autostart_parity.py`: rewritten to the report-only contract — close never sets `auto_started`, never calls `start_work_order`, and advertises `next_work_order` == the ready-set pick (not the naive same-milestone WO); gap paths still report `gaps_block`/`spawned_work_orders`/`next_command`.
- **Eval runner `--live` confirmed as an opt-in escape hatch, guarded (WO-EVAL-LIVE-GATE, d176fb5c):** `core/eval/runner.py` `--live` mode shells out to `claude --print`. Decision: this is an *intentional developer-only* escape hatch, not dead code — it is default-off (`run_case`/`run_all` default `live=False` → fixture mode), gated by the `claude` CLI being in PATH (raises `NotImplementedError` otherwise), scored by the SAME deterministic matcher as fixture mode (it only *generates* events — it is not the LLM judge WO-N2 removed), and never used by CI (the pre-push eval gate runs `pytest tests/evals/`, never `ds eval --live`). Clarified the `run_case`/`_run_case_live`/class docstrings and the `--live` CLI help to say so, and added `tests/unit/test_eval_live_optin_guard.py` (5 guards) locking the boundary: the default path never calls `subprocess.run`/`shutil.which`, never reaches `_run_case_live`; `live=True` is required to route there; and both the runner params and the CLI flag default to `False`.
- **`ds doctor` hook-projection freshness check (WO 46fe128b, follow-on to WO-HOOK-ENFORCE-EXEC-STATS):** the two blocking enforce hooks (`on-edit-enforce`, `on-stop-enforce`) are wired directly in `hooks.json` and copied into the install tree, but `ds update` is version-gated so a canonical edit does not auto-propagate — the deployed copies had silently gone stale (pre-refactor versions with no `_enforce()` wrapper / `log_hook_execution` emission), so plugin-installed sessions fired uninstrumented enforce hooks and the WO-HOOK-ENFORCE-EXEC-STATS telemetry never emitted. The verification test drove the canonical files directly and missed it. New `core/health/doctor.py::_check_hook_freshness` compares each deployed entry-hook copy (`<claude>/hooks/runtime/hooks/meta/on-*-enforce.py`) against canonical (CRLF-normalized) and surfaces `checks.hook_freshness` in `ds doctor`; `--fix` now also re-projects when hooks are stale. Regenerated the stale copies (project + user scope). Note: `ds integrate install` defaults to **project** scope, so user-scope deployed copies must be refreshed with `--scope user` — the new check catches exactly this gap.
- `tests/unit/health/test_doctor_hook_freshness.py`: +4 hermetic tests (clean; detects a stale copy; CRLF-insensitive; skips absent deployed copies).
- **`ds doctor` skill-sync false positives (WO-DOCTOR-DRIFT, a4b3e21d):** the doctor reported drift on a *clean* install because two checks were unaware of how skills are actually installed. (1) `core/health/doctor.py::_check_skill_freshness` hashed raw canonical vs installed, but the installer prepends `synthesize_skill_frontmatter()` to every routable pack's top-level `SKILL.md` (canonical ships frontmatter-less by design), so those skills always diverged → permanent false "stale". The canonical side now applies the same synthesis before hashing (new `_synthesized_skill_transform`, reusing the installer's own function; genuine body drift / orphan files / stale synthesized descriptions still diverge). (2) `_check_pack_mode_coverage` looked under `ds-<pack>/modes`, ignoring `packs.yaml` `skill_path`; `website`/`fullstack` (`skill_path: canonical/skills/domains/modes/*`) install their modes under `ds-domains/modes/<sub>/modes/<mode>`, so all 13 (9 website + 4 fullstack) read as missing though present — new `_installed_modes_dir` resolves the `skill_path`-derived location. Live doctor: `pack_modes_missing` 13→0; `stale` 10→5 (the 5 pure-frontmatter false positives cleared; the remaining 5 are genuine local-install drift, correctly still flagged). No release gate consumed these fields, so no push was ever blocked — the harm was misleading output.
- `tests/unit/health/test_doctor_skill_sync.py`: +2 regression tests (synthesized-frontmatter parity: not stale when installed = canonical + synthesized frontmatter, still stale on genuine body drift; `skill_path`-aware pack coverage: modes under the owning pack resolve, a genuinely-absent mode still flags).

### Added
- **Brownfield offer-to-invoke + findings aggregation (WO-BROWNFIELD-OFFER-INVOKE, 7f8618a4):** follow-on to WO-BROWNFIELD-ADAPTIVE (#522), which surfaced `recommended_dispatches` present-only. This closes the aggregation gap without breaking the operator-approval boundary (Rule 2 — never auto-run). `core/projects/acquisition.py` adds `aggregate_findings(findings, dispatches)` (pure) and `aggregate_readiness(project_id, dispatches=…)` (reads persisted findings via `get_scan_summary` over the `security_events` spine); both fold the findings that operator-approved `ds-quality` audits have **already persisted** into a per-project readiness report (finding count, severity breakdown, provenance audits, the findings) and a severity-ordered stabilization scope (one item per non-empty severity bucket, critical first, with affected files). The engine only CONSUMES persisted results — it never triggers an audit. `ds-project:brownfield` SKILL.md gains Step 8 (confirmation-gated offer to run the recommended audits, then `aggregate_readiness` to fold results into the readiness report / stabilization scope) and Rule 5 (recommended audits are present-only; aggregation consumes operator-approved results only).
- `tests/unit/test_brownfield_offer_invoke.py`: a web+db project routes to backend-api+database and its findings aggregate into both the readiness report and the scope; empty findings yield an empty scope; `aggregate_readiness` folds `get_scan_summary` findings without running an audit; unknown severities sort last.

### Removed
- **audit_runs dropped (WO-SCHEMALEAN, migration 149):** `audit_runs` (0 rows) was a designed external-tool write endpoint — `POST /api/v1/audits/runs` — that **nothing in-repo ever called**: there is no internal writer since the initial commit, and its GET routes read already-dropped security tables (`sec_sarif_findings`/`sec_cve_matches`, tombstoned long ago), so the "Security Audits" dashboard subtab it fed was permanently empty. Its purpose is delivered live by the `/api/v1/security/*` surface over `security_events` + `scan_runs`, making `audit_runs` duplicative + unwired. Removed with the table: the entire `projections/api/routes/audits.py` module (`/api/v1/audits/*`) + its `main.py` import/registration, its boundary-test allowed-write-exception entries, and the migration-143 stale-FK test (`TestMigration143StaleFk` — both of its tables, `capability_route_records`/147 and `audit_runs`/149, are now dropped). The dead dashboard audit-history tab + Security-audits subtab degrade gracefully (existing try/catch → empty/error) and are removed in a follow-up dead-UI cleanup WO. Tombstone (+`audit_runs`) + keeplist (KEEP→DROP) updated; no live data lost. `.released_version` NOT bumped — 149 unreleased until `ds migrate activate`. Vetting: `.planning/audits/schema-lean-vetting-2026-07-15.md`.
- **preflight stack dropped (WO-SCHEMALEAN, migration 148):** `preflight_events` + `business_work_order_preflights` (both 0 rows) were an unwired aspirational loop — `spec_ingestor.ingest_specs` (never called from any CLI/route/workflow, test-only) → `mutations.create_preflight` → `preflight_events` → `PreflightProjection` → `business_work_order_preflights` → `start.py::_check_preflight_gate` (a live but permanent no-op — the table was always empty). Nothing external ever wrote a finding, and the "pre-work risk" purpose is already delivered live by the CI blast-radius/hanging-detector gate (WO-BLAST-RADIUS-GATE), so the stack was duplicative + unwired. Removed: both tables, `core/preflight/mutations.py`, `core/preflight/spec_ingestor.py`, `core/projections/preflight_projection.py` (+ its runner registration), and the `_check_preflight_gate` start-gate (+ call site). `core/preflight/capability_probe.py` (unrelated github/MCP probing) is kept. Tombstone + keeplist (KEEP→DROP) + `tests/unit/test_preflight_layer.py` deleted; no live data lost.

### Removed
- **capability_route_records dropped (WO-SCHEMALEAN, migration 147):** dead persist=False writer table (0 rows) — the only production caller of `recommend_capability_route` (`GET /api/shared-intelligence/capability-routes/recommendation`) passes `persist=False`, so `record_capability_route`'s INSERT was never reached (same pattern as the dropped `compliance_review_flags`). Removed with the table: `record_capability_route` + its orphaned `_select_records`/`_table_has_column`/`_int_bool` helpers (authority.py), `capability_route_summary`/`_decode_route`/`_loads` (capability_routing.py), the `GET /capability-routes` route + dashboard surface entry (shared_intelligence.py), the `capability_routes` section of the `installed_adapter_router` read model (installed_runtime.py), and `capability_route_records` from `REQUIRED_SHARED_INTELLIGENCE_TABLES` + `CAPABILITY_SOURCE_TABLES`. The recommendation **preview** (`recommend_capability_route(persist=False)`) is kept — it does not touch the table. Tombstone + keeplist (KEEP→DROP) updated; no live data lost. Vetting: `.planning/audits/schema-lean-vetting-2026-07-15.md`.

### Changed
- **Production-readiness dashboard wired to readiness_events (WO-SCHEMALEAN, acca5184):** `core/production_readiness/controls.py::production_readiness_dashboard_summary` was permanently returning "unavailable" — it read `production_readiness_assessment_runs`, dropped in migration 112, so its early-return always fired. Repointed it to reconstruct the summary from the `readiness_events` spine (the `assessment.started` event body → status/confidence/scores/missing_evidence/blocking_factors; child `control_result.recorded` events → controls + counts) — the same spine `ds analytics-ingest` already writes. No response-shape change; `source_tables` now names `readiness_events`. Activates the production-readiness dashboard section that had been dead since migration 112. Vetting: `.planning/audits/schema-lean-vetting-2026-07-15.md`.
- `tests/unit/test_readiness_events_reader.py`: reconstruct-from-spine + honest empty-state; `tests/unit/test_analytics_only_ingestion.py` now asserts the ingested assessment surfaces on the dashboard.
- **Singleton runtime-state JSON moved into the authority (WO-FILESDB-P2 task 2, 6dc7c844-1174-4e27-ad64-2239b3660071):** the active skill (`active_skill.json`), active task (`active_task.json`), and platform profile (`platform.json`) were loose `~/.dream-studio/state/*.json` files. Migration 146 (`146_runtime_state.sql`) adds `raw_runtime_state` (key PK, value TEXT JSON, updated_at) — one `key -> JSON` row per state kind. `core/runtime_state.py` provides `db_write_runtime_state` / `db_read_runtime_state` / `db_clear_runtime_state` (resolving the DB via `_default_db_path` so runtime state lives in the same authority DB callers use). Repointed callers: `core/skills/invocation.py` + `core/telemetry/token_capture.py` (active_skill write/read), `core/sdlc/active_task.py` (set/get/clear), `core/config/platform.py` (ensure/get). All prefer the authority row and fall back to the legacy JSON file when the table is absent; an explicit `profile_path` / `DS_PLATFORM_PROFILE_PATH` / `DS_ACTIVE_TASK_PATH` override still uses the file. Keeplist: `raw_runtime_state` = KEEP. `.released_version` NOT bumped — 146 stays unreleased on the live authority DB until `ds migrate activate`; fresh installs and CI apply it immediately. Sibling of migration 145 (raw_session_token_accumulators) under the same files-in-database directive.
- `tests/unit/test_runtime_state_db.py`: store roundtrip + overwrite + clear, absent-table fallback (write False / read None / clear False), platform read/write through the authority row (no JSON written), explicit-path still writes JSON, and active_task get/clear through the authority row.
