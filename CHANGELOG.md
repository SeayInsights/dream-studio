...
### Fixed
- **A skill change reaches installs, and shipped skills stop naming files no install has (WO 789df02b, WO f1290b5c, `e6ef2c79`/`44493a98`):** operator report — another Dream Studio install "said it couldn't do the reviews because it didn't have some of the files needed". **Two independent causes.** First, `ds update` compared VERSION stamps and, when they matched, consulted only `_canonical_hook_drift`, which filters manifest entries to `"hooks" in p and "meta" in p and p.endswith(".py")` — skills were outside its scope, so a canonical skill change without a version bump printed `already_current` and installed nothing. Measured on the live manifest: `_canonical_hook_drift` returned **0** findings where hooks+skills returned **2**, one of them `canonical/skills/core/modes/review/SKILL.md` — the review rule added in this same branch, which therefore would not have shipped. The manifest already recorded `content_hash` for **616** skill entries and nothing read it. The check drives the installer's own `plan()` rather than re-deriving the canonical-to-installed path mapping, since a second copy of that mapping would drift from the first; and it plans against the scope the **manifest** records, because from inside the repo `detect_claude_code()` reports `project` while a user-scope manifest records `~/.claude`, and comparing those trees lines up no path — the first draft did that and reported all **616** skill files drifted. Second, and this is the reported symptom exactly: `dist/plugin/skills/ds-core/modes/review/SKILL.md` told the reader to open `core/git.md`, `core/quality.md`, `core/orchestration.md` and `core/format.md` — `orchestration.md` holding the reviewer prompt template and the review loop — while projection renames `canonical/skills/core` to `ds-core`, so **no install has ever had a `core/` directory**. Measured on the shipped tree: **111** such references across **3** packs, **0** resolving; **111 → 0** after the sweep. It works in this repo by accident, because `core/git.md` resolves from `canonical/skills/`, which is why it survived. **70** references became relative paths (projection-invariant — a relative path never names the pack directory, the only thing projection renames) and the **7** cross-pack ones name the owning pack in prose, since no path is true in both trees. Also `_collect_skill_dir_ops`'s `rglob("*")` installed **20** `__pycache__/*.pyc` files as skill content and recorded them in the manifest; a `.pyc` is rewritten on import so its hash never settles, which would leave any content-hash drift check permanently dirty. **Five independent review rounds, and the first four each found a real defect — three of them in the verification apparatus rather than the product.** The drift check reported 0 drift on a tree holding an uncommitted skill edit because an argument mismatch raised and a broad `except` ate it. A "planned nothing" sentinel was proved unreachable (a tree with no skills fails in `compile_pack` first) and deleted as dead code that read like a guarantee. The path gate's first draft matched by basename and flagged **127** references to the user's own project; its second demanded an exact trailing-segment match, which silenced the false positives and the true positives together — nothing shipped ends in `("core","git.md")`, so the exact shape the gate existed to catch was classified "not a module reference" and dropped, and a mutation run reintroduced all four broken review references with the suite still green. `_locate` now models the rename explicitly and sorts its matches: leaving order to `os.scandir` left the answer undefined across the three CI platforms and let the test pinning the same-pack preference pass by accident. **13** tests in `tests/unit/test_skill_module_paths_resolve.py` (six driving the resolver against a synthetic tree, so the rule stays falsifiable once the real tree is clean) and **7** in `tests/unit/test_ds_update_skill_drift.py`; every one was driven red by mutation before being trusted. Both wired into the BLOCKING `pin-tests` gate in `canonical/workflows/pre-push.yaml` and into pr-smoke's focused list in `.github/workflows/ci.yml` — a guard living only in full-ci is the documented reason a stale projection shipped before. Pre-push: **23/23 gates PASS**.
- **A locked authority is not a missing artifact table (WO-ARTIFACT-LOCK-FALLBACK `fd981a32`, `b302834b`):** measured on the live authority 2026-09-02 — of August's review verdicts, **154 landed in `.planning` and 23 in the authority**; `SELECT COUNT(*) FROM business_work_order_artifacts WHERE kind='review_verdict'` returned **24** while `.planning/work-orders/*/review-verdict.json` held **170**, of which **162** existed nowhere else. Every one was invisible to the `independent_review` close gate and to `ds project state`, which read the authority only. **The disk fallback was not the bug** — it is deliberate for an authority whose artifact migration is unreleased. The bug is that `set_wo_artifact` returned `False` for **every** `sqlite3.OperationalError`, and all five artifact writes in verify run inside an already-open `with _connect(db_path)`, so a second connection got `database is locked`, returned `False`, and the caller read that as "the table does not exist". `set_wo_artifact` now takes the caller's `conn`, `False` means only **a schema older than the write**, and every other `OperationalError` raises. A fallback that does fire is counted via `record_artifact_fallback` (rule `artifact_disk_fallback`, through `record_observation`, so `observations_report` groups it) — verified end to end, `artifact_disk_fallback: 2 -> 5` after `sync_tick()`. Firing 154 times without a countable trace is why it survived a month.
- **Four callers discarded that `False`, each with a deferred invisible cost:** `carry_over` reported `ok: True` with the split unrecorded and the tasks already moved, leaving the original permanently unclosable — and its recovery advice prescribed a re-run that cannot work, because `_emit_task_deleted` has already fired and `_open_tasks` filters the deleted tasks; `record_exception` returned `False` while both callers catch only `ValueError`, so `--accept-structure` printed success and stored nothing (and once the write learned to raise, crashed with a traceback on a busy authority — every storage failure is a `ValueError` now, converted at the source); `record_commit_ownership` returned `[]`, indistinguishable from "this task produced no commits"; and `close_work_order` held the boundary pin and the ownership record in **one** `except Exception: pass`, so a pin that raised skipped the ownership record entirely and neither failure reached the operator — on five return paths including `if gate_failures and not force`, the normal blocked-close outcome.
- **The recovery had no caller.** `backfill_wo_artifacts` existed since WO-FILESDB-P1 invoked only from a test, so the repair for what had already fallen through was unreachable — the `reachability` gate would refuse it today. New `ds work-order backfill-artifacts [--planning-root <path>] [--dry-run]`; run against the live authority it took `review_verdict` rows **24 → 186** with **0** disk-only verdicts remaining. It reports a **partial** recovery rather than aborting, because the loop ran ~171 raising writes with no `try`, and exits 1 on zero written — "nothing to do" and "the table is absent so every write no-op'd" print the same count.
- **Commit ownership survives the squash that makes the work permanent (WO `e439f287` / `80c0e61b`):** `record_commit_ownership` records BRANCH shas and this repo squash-merges every PR, so a work order's claimed commits stop being ancestors of HEAD the moment it lands — `git merge-base --is-ancestor 4a5221cf main` returns non-zero because #687 squashed 18 commits into `14b8693c`. The attribution shipped in #689 was correct for in-flight work and **inert for merged work**, silently. The mapping is *read* from the `* <subject>` bullets a squash writes, never inferred; all three of `1db6de49`'s commits resolve to `14b8693c` with none lost, narrowing its graded range from 10 commits to 3. Ambiguity returns `None` rather than guessing. And the neighbour caveat now fires only when a neighbour could plausibly be in range — a sha FOLLOWED to a commit outside it is accounted for, not unknown, where before 11 long-merged commits produced "a neighbour's work may be graded here" on every verify.
- **The spool guard names the test that polluted (WO `efe2ce9d`):** the autouse guard aborts the session when a test writes to the operator's real `~/.dream-studio/events` — correctly — but said only `Test modified real ~/.dream-studio/events`, and a session ending at test 2,400 of 5,928 with no identifier leaves the reader to bisect by hand. Its teardown already knew; interpolating `request.node.nodeid` is the whole change. Verified out-of-band with a throwaway probe that really does pollute, then removed.
- **`gitignore-phantom`: an assertion of absence is not a dependency.** The gate collected every string literal under an `assert` as "this source relies on the path existing", which is backwards for `assert ".planning" not in str(p)` — and asserting absence is how a zero-disk rule gets tested, so the gate refused the tests enforcing the rule it supports. Negated assertions are exempt, keyed by `(literal, lineno)`: a first cut used bare strings and one absence assertion exempted every real dependency on that string in the whole file, which is a hole through a blocking gate. **81 tests** across `tests/unit/test_artifact_lock_fallback.py`, `test_wo_verify.py`, `test_wo_artifact_cli.py`, `test_wo_artifacts_db.py`, `test_gitignore_phantom.py` and `tests/integration/test_enforcement_tiers.py`.
