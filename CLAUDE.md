...
**Why this matters — the runtime/subset gap:** The pre-push gate runs `tests/evals/` only (a subset). The full unit suite takes **~35 minutes** on Windows and does NOT OOM (6,044 passed / 25 failed / 35m33s, measured 2026-09-21; the earlier ~79-minute figure was stale, and `docs/operations/lightweight-github-ci-strategy.md` separately claimed it OOMed and could not run locally at all — it can, and at 35 minutes it can run before a push). Passing the pre-push gate is necessary but not sufficient. Local-green ≠ CI-green — not because of subtle platform quirks alone, but because the local gate runs less. The remote 3-platform `pr-smoke` matrix runs 4 focused gate test files + format/lint (NOT the full suite). The full suite runs ubuntu-only, post-merge in `full-ci.yml`. The matrix watch is the sole merge-authorization gate; it verifies what the PR smoke actually covers (gate tests + format/lint).
