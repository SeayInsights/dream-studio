...
class RepoAnalyzer:
    ...
    def analyze_skill_file(
        ...
        has_response_contract = (
            "response contract" in content_lower or "output contract" in content_lower
        )
        ...
        for analysis in self.skill_analyses:
            ...
            if analysis.has_frontmatter and analysis.frontmatter:
                ...
                self.patterns_found["frontmatter_patterns"].append(
                    {"repo": repo, "file": analysis.path, "keys": list(fm_keys)}
                )
        ...
        for repo in stats["by_repo"]:
            ...
            if total > 0:
                ...
                stats["averages"][repo] = {
                    ...
                    "avg_decision_tables_per_skill": sum(
                        a.decision_table_count for a in self.skill_analyses if a.repo == repo
                    )
                    / total,
                    "avg_code_blocks_per_skill": sum(
                        a.code_block_count for a in self.skill_analyses if a.repo == repo
                    )
                    / total,
                }
