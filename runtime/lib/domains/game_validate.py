...
UI_REFERENCE_PATTERNS = [
    re.compile(
        r"\$.*(?:Label|Button|Panel|HBox|VBox|Container|TextureRect|ProgressBar|LineEdit|TextEdit|RichTextLabel|OptionButton|CheckBox|SpinBox)"
    ),
...
VELOCITY_PATTERN = re.compile(
    r"\bvelocity(?:\.\w+)?\s*[+\-*/]?=.*(?:SPEED|speed|gravity|GRAVITY|acceleration|ACCELERATION)"
)
...
def classify_path(rel_path: str, file_path: Optional[Path] = None) -> set[str]:
    ...
    if not domains and file_path and file_path.suffix.lower() == ".gd":
        try:
            ...
            if any(
                kw in head
                for kw in ("CharacterBody2D", "CharacterBody3D", "move_and_slide", "velocity")
            ):
                domains.add("gameplay")
            if any(
                kw in head
                for kw in ("NavigationAgent", "AStarGrid", "behavior_tree", "state_machine")
            ):
                domains.add("ai")
            if any(
                kw in head
                for kw in ("Control", "Panel", "Label", "Button", "HBoxContainer", "VBoxContainer")
            ):
                domains.add("ui")
            if any(
                kw in head for kw in ("@rpc", "multiplayer", "MultiplayerAPI", "MultiplayerSpawner")
            ):
                domains.add("networking")
        except Exception:
            pass
    ...
    try:
        if file_path.stat().st_size > MAX_VALIDATE_FILE_SIZE:
            return ValidationResult(
                [], [], [f"  Skipped: file too large (>{MAX_VALIDATE_FILE_SIZE // 1024 // 1024}MB)"]
            )
        ...
        for i, line in enumerate(lines, 1):
            ...
            for pattern in UI_REFERENCE_PATTERNS:
                if pattern.search(line):
                    ...
                    if ui_ref_count <= 3:
                        warnings.append(
                            f"  Line {i}: gameplay code references UI — emit signals instead"
                        )
                    break
        if ui_ref_count > 3:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if in_physics_func and VELOCITY_PATTERN.search(line):
                if "delta" not in line.lower() and not _is_suppressed(lines, i - 1):
                    warnings.append(
                        f"  Line {i}: velocity change in {current_func.split('(')[0].replace('func ', '')} — missing delta?"
                    )
    ...
    if is_net:
        for i, line in enumerate(lines, 1):
            ...
            if stripped.startswith("#"):
                continue
            if (
                re.search(r"\.position\s*=", line)
                and "is_multiplayer_authority" not in content[: content.find(line)]
            ):
                if "set_position" not in line and "@rpc" not in line:
                    info.append(
                        f"  Line {i}: direct position set in network code — verify authority check exists"
                    )
            if "@rpc" in line and "any_peer" in line:
                info.append(
                    f"  Line {i}: @rpc(any_peer) — ensure server validates all data from this RPC"
                )
    ...
    if is_ui:
        ...
        for i, line in enumerate(lines, 1):
            if state_mutations.search(line):
                warnings.append(
                    f"  Line {i}: UI code mutating game state — use signals or autoload methods"
                )
...
def validate_json_data(file_path: Path, rel_path: str) -> ValidationResult:
    ...
    if isinstance(data, dict) and len(data) > 2:
        bad_keys = [
            k for k in data.keys() if k != k.lower() and not k.startswith("_") and not k.isupper()
        ]
        if bad_keys and len(bad_keys) > len(data) * 0.3:
            warnings.append(
                f"  Keys should be snake_case (found: {', '.join(bad_keys[:3])}{'...' if len(bad_keys) > 3 else ''})"
            )
        ...
        for key, val in data.items():
            if isinstance(val, list) and len(val) >= 2 and all(isinstance(v, dict) for v in val):
                ...
                for idx, ks in enumerate(key_sets[1:], 1):
                    ...
                    if missing:
                        warnings.append(
                            f"  {key}[{idx}] missing keys vs [0]: {', '.join(sorted(missing))}"
                        )
                        break
                    if extra:
                        info.append(
                            f"  {key}[{idx}] has extra keys vs [0]: {', '.join(sorted(extra))}"
                        )
                        break
...
def validate_asset_naming(file_path: Path, rel_path: str) -> ValidationResult:
    ...
    if " " in name:
        errors.append(
            f"  '{name}' contains spaces — use snake_case (Godot import breaks with spaces in some contexts)"
        )
...
def validate_shader(file_path: Path) -> ValidationResult:
    ...
    for i, line in enumerate(lines, 1):
        ...
        if re.search(r"(?:color|albedo|emission)\s*=\s*vec[34]\([\d.]+", line, re.IGNORECASE):
            if "uniform" not in line:
                warnings.append(
                    f"  Line {i}: hardcoded color/visual value — use a uniform with hint_color or hint_range"
                )
    ...
    for i, line in enumerate(lines, 1):
        if re.match(r"^void\s+(vertex|fragment|light)\s*\(", line.strip()):
            ...
            if length > 50:
                warnings.append(
                    f"  {func_name}() is {length} lines — consider extracting to helper functions (<50 lines recommended)"
                )
            func_start = None
